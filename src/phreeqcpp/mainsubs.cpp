...
/* ---------------------------------------------------------------------- */
{
   ...
   *   Allocate space
   */
  cell_data.resize((size_t)count_cells +
                   2); // initialized by global_structures.h
...
/* ---------------------------------------------------------------------- */
{
  ...
  //{
  // for (size_t nn = 0; nn < Rxn_new_solution.size(); nn++)
  for (std::set<int>::const_iterator nit = Rxn_new_solution.begin();
       nit != Rxn_new_solution.end(); nit++) {
    std::map<int, cxxSolution>::iterator it = Rxn_solution_map.find(*nit);
    if (it == Rxn_solution_map.end()) {
      assert(false);
    }
    ...
    if (solution_ref.Get_new_def()) {
      ...
      if (print == TRUE) {
        snprintf(token, sizeof(token), "Initial solution %d.\t%.350s",
                 ...
                 solution_ref.Get_description().c_str());
        dup_print(token, FALSE);
      }
      use.Set_solution_ptr(&solution_ref);
      LDBLE d0 = solution_ref.Get_density();
      ...
      use.Get_solution_ptr()->Set_viscos_0(viscos_0);
      if (use.Get_surface_ptr() != NULL && dl_type_x != cxxSurface::NO_DL)
        use.Get_surface_ptr()->Set_DDL_viscosity(
            viscosity(use.Get_surface_ptr()));
      add_isotopes(solution_ref);
      punch_all();
      print_all();
      density_iterations = 0;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::initial_exchangers(int print)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Go through list of exchangers, make initial calculations
  ...
  // Rxn_exchange_map.find(Rxn_new_exchange[nn]);
  for (std::set<int>::const_iterator nit = Rxn_new_exchange.begin();
       nit != Rxn_new_exchange.end(); nit++) {
    std::map<int, cxxExchange>::iterator it = Rxn_exchange_map.find(*nit);
    if (it == Rxn_exchange_map.end()) {
      assert(false);
    }
    ...
    if (exchange_ptr->Get_solution_equilibria()) {
      ...
      if (print == TRUE) {
        snprintf(token, sizeof(token), "Exchange %d.\t%.350s",
                 exchange_ptr->Get_n_user(),
                 exchange_ptr->Get_description().c_str());
        dup_print(token, FALSE);
      }
      use.Set_exchange_ptr(exchange_ptr);
      use.Set_solution_ptr(Utilities::Rxn_find(Rxn_solution_map,
                                               exchange_ptr->Get_n_solution()));
      if (use.Get_solution_ptr() == NULL) {
      ...
      /* free_model_allocs(); */
      if (converge == ERROR || converge1 == ERROR) {
        error_msg("Model failed to converge for initial exchange calculation.",
                  STOP);
      }
    }
    for (i = n_user + 1; i <= last; i++) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::initial_gas_phases(int print)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Go through list of gas_phases, make initial calculations
  ...
  // Rxn_gas_phase_map.find(Rxn_new_gas_phase[nn]);
  for (std::set<int>::const_iterator nit = Rxn_new_gas_phase.begin();
       nit != Rxn_new_gas_phase.end(); nit++) {
    std::map<int, cxxGasPhase>::iterator it = Rxn_gas_phase_map.find(*nit);
    if (it == Rxn_gas_phase_map.end()) {
      assert(false);
    }
    ...
    if (gas_phase_ptr->Get_solution_equilibria()) {
      ...
      if (print == TRUE) {
        snprintf(token, sizeof(token), "Gas_Phase %d.\t%.350s",
                 ...
                 gas_phase_ptr->Get_description().c_str());
        dup_print(token, FALSE);
      }

      /* Try to obtain a solution pointer */
      use.Set_solution_ptr(Utilities::Rxn_find(
          Rxn_solution_map, gas_phase_ptr->Get_n_solution()));
      prep();
      k_temp(use.Get_solution_ptr()->Get_tc(),
             use.Get_solution_ptr()->Get_patm());
      set(TRUE);
      ...
      if (converge == ERROR || converge1 == ERROR) {
        /* free_model_allocs(); */
        error_msg("Model failed to converge for initial gas phase calculation.",
                  STOP);
      }
      use.Set_gas_phase_ptr(gas_phase_ptr);
      gas_phase_ptr->Set_total_p(0);
      ...
      for (size_t i = 0; i < gas_phase_ptr->Get_gas_comps().size(); i++) {
        ...
        if (phase_ptr->in == TRUE) {
          ...
          phase_ptr->p_soln_x = exp(lp * LOG_10);
          gas_phase_ptr->Set_total_p(gas_phase_ptr->Get_total_p() +
                                     phase_ptr->p_soln_x);
          phase_ptr->moles_x = phase_ptr->p_soln_x *
                               gas_phase_ptr->Get_volume() /
                               (R_LITER_ATM * tk_x);
          gc_ptr->Set_moles(phase_ptr->moles_x);
      ...
      if (fabs(gas_phase_ptr->Get_total_p() -
               use.Get_solution_ptr()->Get_patm()) > 5) {
        snprintf(token, sizeof(token),
                 "WARNING: While initializing gas phase composition by "
                 "equilibrating:\n%s (%.2f atm) %s (%.2f atm).\n%s.",
                 "         Gas phase pressure",
                 (double)gas_phase_ptr->Get_total_p(),
                 "is not equal to solution-pressure",
      ...
      if (PR /*&& use.Get_gas_phase_ptr()->total_p > 1.0*/)
        warning_msg(
            "While initializing gas phase composition by equilibrating:\n"
            "         Found definitions of gas` critical temperature and "
            "pressure.\n"
            "         Going to use Peng-Robinson in subsequent "
            "calculations.\n");
      xgas_save(n_user);
      punch_all();
      /* free_model_allocs(); */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::initial_surfaces(int print)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Go through list of surfaces, make initial calculations
  ...
  // Rxn_surface_map.find(Rxn_new_surface[nn]);
  for (std::set<int>::const_iterator nit = Rxn_new_surface.begin();
       nit != Rxn_new_surface.end(); nit++) {
    std::map<int, cxxSurface>::iterator it = Rxn_surface_map.find(*nit);
    if (it == Rxn_surface_map.end()) {
      assert(false);
    }
    ...
    if (surface_ptr->Get_solution_equilibria()) {
      ...
      if (print == TRUE) {
        std::ostringstream msg;
        msg << "Surface " << n_user << ".\t"
            << surface_ptr->Get_description().c_str();
        dup_print(msg.str().c_str(), FALSE);
      }
      use.Set_surface_ptr(surface_ptr);
      dl_type_x = use.Get_surface_ptr()->Get_dl_type();
      use.Set_solution_ptr(
          Utilities::Rxn_find(Rxn_solution_map, surface_ptr->Get_n_solution()));
      if (use.Get_solution_ptr() == NULL) {
...
/* ---------------------------------------------------------------------- */
{
  ...
  for (reaction_step = 1; reaction_step <= count_steps; reaction_step++) {
    ...
    if (use.Get_kinetics_in() == TRUE) {
      kinetics_ptr = Utilities::Rxn_find(Rxn_kinetics_map, -2);
      kin_time = kinetics_ptr->Current_step((incremental_reactions == TRUE),
                                            reaction_step);
    }
    if (incremental_reactions == FALSE ||
        (incremental_reactions == TRUE && reaction_step == 1)) {
  ...
  //	{
  //		cxxGasPhase *gas_phase_ptr = use.Get_gas_phase_ptr();
  //		for (size_t i = 0; i < gas_phase_ptr->Get_gas_comps().size();
  //i++)
  //		{
  //			cxxGasComp *gc_ptr =
  //&(gas_phase_ptr->Get_gas_comps()[i]); 			int k; 			class phase *phase_ptr =
  //phase_bsearch(gc_ptr->Get_phase_name().c_str(), &k, FALSE);
  //			assert(phase_ptr);
  //			phase_ptr->pr_in = false;
  //		}
  //	}
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (save.pp_assemblage == TRUE) {
    ...
    Utilities::Rxn_copies(Rxn_pp_assemblage_map, save.n_pp_assemblage_user,
                          save.n_pp_assemblage_user_end);
  }
  ...
  if (save.ss_assemblage == TRUE) {
    ...
    Utilities::Rxn_copies(Rxn_ss_assemblage_map, save.n_ss_assemblage_user,
                          save.n_ss_assemblage_user_end);
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xexchange_save(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  snprintf(token, sizeof(token), "Exchange assemblage after simulation %d.",
           simulation);
  ...
  for (i = 0; i < count_unknowns; i++) {
    if (x[i]->type == EXCH) {
      const cxxExchComp *comp_ptr =
          use.Get_exchange_ptr()->Find_comp(x[i]->exch_comp);
      if (!comp_ptr) {
        assert(false);
      }
      ...
      for (j = 0; j < species_list.size(); j++) {
        if (species_list[j].master_s == x[i]->master[0]->s) {
          add_elt_list(species_list[j].s->next_elt, species_list[j].s->moles);
      ...
      /* debug
                              output_msg(sformatf( "Exchange charge_balance:
         %e\n", charge));
       */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xgas_save(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xss_assemblage_save(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xpp_assemblage_save(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (int j = 0; j < count_unknowns; j++) {
    ...
    cxxPPassemblageComp *comp =
        temp_pp_assemblage.Find(x[j]->pp_assemblage_comp_name);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xsolution_save(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  // the subroutine is called at the start of a new simulation, and the
  // following 2 go wrong since s_x is not updated
  ...
  temp_solution.Set_cb(
      cb_x); /* cb_x does not include surface charge after sum_species */
  ...
  for (int i = 0; i < (int)master.size(); i++) {
    if (master[i]->s->type == EX || master[i]->s->type == SURF ||
        master[i]->s->type == SURF_PSI)
      continue;
    ...
    if (master[i]->in != FALSE) {
      temp_solution.Get_master_activity()[master[i]->elt->name] =
          master[i]->s->la;
    }
  ...
  for (it = temp_solution.Get_isotopes().begin();
       it != temp_solution.Get_isotopes().end(); it++) {
    class master *iso_master_ptr =
        master_bsearch(it->second.Get_elt_name().c_str());
    if (iso_master_ptr != NULL) {
      ...
      error_string = sformatf("Ignoring failed attempt to interpret %s as an "
                              "isotope of element %s.",
                              it->second.Get_isotope_name().c_str(),
                              it->second.Get_elt_name().c_str());
  ...
  if (this->save_species) {
    ...
    for (int i = 0; i < (int)this->s_x.size(); i++) {
      if (s_x[i]->type <= H2O) {
        temp_solution.Get_species_map()[s_x[i]->number] =
            s_x[i]->moles / temp_solution.Get_soln_vol();
      }
    }
    // saves gamma
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xsurface_save(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (int i = 0; i < count_unknowns; i++) {
    if (x[i]->type == SURFACE) {
      ...
      if (comp_ptr == NULL)
        continue; // appt in transport with different mobile and stagnant
                  // surfaces
      assert(comp_ptr);
      ...
      cxxSurfaceCharge *charge_ptr =
          temp_surface.Find_charge(x[i]->surface_charge);
      if (charge_ptr == NULL)
        continue; // appt in transport with different mobile and stagnant
                  // surfaces
      assert(charge_ptr);
      ...
      if (dl_type_x != cxxSurface::NO_DL) {
        charge_ptr->Set_charge_balance(
            (charge_ptr->Get_sigma0() + charge_ptr->Get_sigma1() +
             charge_ptr->Get_sigma2() + charge_ptr->Get_sigmaddl()) *
  ...
  if (!(dl_type_x == cxxSurface::NO_DL)) {
    ...
    for (size_t i = 0; i < surface_ptr->Get_surface_charges().size(); i++) {
      ...
      for (int j = 0; j < (int)this->s_x.size(); j++) {
        ...
        charge_ref.Get_dl_species_map()[s_x[j]->number] =
            moles_surface / mass_water_surface;
        double g = charge_ref.Get_g_map()[s_x[j]->z].Get_g();
        // double moles_excess = mass_water_aq_x * molality * (g *
        // s_x[j]->erm_ddl + 	mass_water_surface / 	mass_water_aq_x *
        //(s_x[j]->erm_ddl - 1));
        ...
        double c =
            (mass_water_surface * molality + moles_excess) / mass_water_surface;
...
/* ---------------------------------------------------------------------- */
{
   ...
   */
  if (use.Get_pp_assemblage_in() == TRUE) {
    Utilities::Rxn_copy(Rxn_pp_assemblage_map, use.Get_n_pp_assemblage_user(),
                        i);
   ...
   */
  if (use.Get_ss_assemblage_in() == TRUE) {
    Utilities::Rxn_copy(Rxn_ss_assemblage_map, use.Get_n_ss_assemblage_user(),
                        i);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::step_save_exch(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::step_save_surf(int n_user)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (use.Get_surface_ptr() == NULL)
    return (OK);
  Utilities::Rxn_copy(Rxn_surface_map, use.Get_surface_ptr()->Get_n_user(),
                      n_user);
   ...
   *   Update grams
   */
  if ((surface_ptr->Get_type() == cxxSurface::DDL ||
       surface_ptr->Get_type() == cxxSurface::CCM ||
       surface_ptr->Get_type() == cxxSurface::CD_MUSIC) &&
...
/* ---------------------------------------------------------------------- */
int Phreeqc::copy_entities(void)
/* ---------------------------------------------------------------------- */
{
  int return_value;
  return_value = OK;
  for (size_t j = 0; j < copy_solution.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_solution_map, copy_solution.n_user[j]) !=
        NULL) {
  ...
  for (size_t j = 0; j < copy_pp_assemblage.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_pp_assemblage_map,
                            copy_pp_assemblage.n_user[j]) != NULL) {
      for (size_t i = copy_pp_assemblage.start[j];
           i <= copy_pp_assemblage.end[j]; i++) {
        if (i == copy_pp_assemblage.n_user[j])
          continue;
        Utilities::Rxn_copy(Rxn_pp_assemblage_map, copy_pp_assemblage.n_user[j],
                            (int)i);
      }
    }
  }
  ...
  for (size_t j = 0; j < copy_reaction.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_reaction_map, copy_reaction.n_user[j]) !=
        NULL) {
  ...
  for (size_t j = 0; j < copy_exchange.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_exchange_map, copy_exchange.n_user[j]) !=
        NULL) {
  ...
  for (size_t j = 0; j < copy_temperature.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_temperature_map, copy_temperature.n_user[j]) !=
        NULL) {
      for (size_t i = copy_temperature.start[j]; i <= copy_temperature.end[j];
           i++) {
        if (i != copy_temperature.n_user[j]) {
          Utilities::Rxn_copy(Rxn_temperature_map, copy_temperature.n_user[j],
                              (int)i);
        }
      }
    }
  ...
  for (size_t j = 0; j < copy_pressure.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_pressure_map, copy_pressure.n_user[j]) !=
        NULL) {
      for (size_t i = copy_pressure.start[j]; i <= copy_pressure.end[j]; i++) {
        if (i != copy_pressure.n_user[j]) {
          Utilities::Rxn_copy(Rxn_pressure_map, copy_pressure.n_user[j],
                              (int)i);
        }
      }
    }
  ...
  for (size_t j = 0; j < copy_gas_phase.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_gas_phase_map, copy_gas_phase.n_user[j]) !=
        NULL) {
      for (size_t i = copy_gas_phase.start[j]; i <= copy_gas_phase.end[j];
           i++) {
        if (i == copy_gas_phase.n_user[j])
          continue;
        Utilities::Rxn_copy(Rxn_gas_phase_map, copy_gas_phase.n_user[j],
                            (int)i);
      }
    }
  }
  ...
  for (size_t j = 0; j < copy_kinetics.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_kinetics_map, copy_kinetics.n_user[j]) !=
        NULL) {
  ...
  for (size_t j = 0; j < copy_ss_assemblage.n_user.size(); j++) {
    if (Utilities::Rxn_find(Rxn_ss_assemblage_map,
                            copy_ss_assemblage.n_user[j]) != NULL) {
      for (size_t i = copy_ss_assemblage.start[j];
           i <= copy_ss_assemblage.end[j]; i++) {
        if (i == copy_ss_assemblage.n_user[j])
          continue;
        Utilities::Rxn_copy(Rxn_ss_assemblage_map, copy_ss_assemblage.n_user[j],
                            (int)i);
      }
    }
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_database(void)
/* ---------------------------------------------------------------------- */
{
  simulation = 0;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::run_simulations(void)
/* ---------------------------------------------------------------------- */
{
  char token[MAX_LENGTH];
  ...
  try {
    ...
    for (simulation = 1;; simulation++) {
#ifdef TEST_COPY_OPERATOR
      {
        ...
        {
          std::map<int, SelectedOutput>::iterator so_it =
              this->SelectedOutput_map.begin();
      ...
      screen_msg(token);
#endif
      snprintf(token, sizeof(token), "Reading input data for simulation %d.",
               simulation);
