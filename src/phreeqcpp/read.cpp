...
#define LAST_C_KEYWORD 61
  ...
  while ((i = check_line("Subroutine Read", FALSE, TRUE, TRUE, TRUE)) !=
         KEYWORD) {
  ...
  for (;;) {
    ...
    case Keywords::KEY_NONE: /* Have not read line with keyword */
      error_string = sformatf("Unknown input, no keyword has been specified.");
      warning_msg(error_string);
      while ((j = check_line("No keyword", FALSE, TRUE, TRUE, TRUE)) !=
                 KEYWORD &&
             j != EOF) {
        warning_msg(error_string);
      }
    ...
    case Keywords::KEY_EQUILIBRIUM_PHASES_RAW:
      Utilities::Rxn_read_raw(Rxn_pp_assemblage_map, Rxn_new_pp_assemblage,
                              this);
      break;
    ...
    case Keywords::KEY_EQUILIBRIUM_PHASES_MODIFY:
      Utilities::Rxn_read_modify(Rxn_pp_assemblage_map, Rxn_new_pp_assemblage,
                                 this);
      break;
    ...
    case Keywords::KEY_SOLID_SOLUTIONS_MODIFY:
      Utilities::Rxn_read_modify(Rxn_ss_assemblage_map, Rxn_new_ss_assemblage,
                                 this);
      break;
    ...
    case Keywords::KEY_REACTION_TEMPERATURE_MODIFY:
      Utilities::Rxn_read_modify(Rxn_temperature_map, Rxn_new_temperature,
                                 this);
      break;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_exchange_species(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (;;) {
    ...
    case 0: /* no_check */
      if (s_ptr == NULL) {
        error_string =
            sformatf("No reaction defined before option, %s.", opt_list[opt]);
    ...
    case 15: /* llnl_gamma */
      ...
      if (i < 1) {
        error_string = sformatf("Expecting activity-coefficient parameter, a.");
        warning_msg(error_string);
      }
      opt_save = OPTION_DEFAULT;
      break;
    ...
    case 17: /* add_log_k */
    {
      ...
      if (copy_token(token, &next_char, &i) == EMPTY) {
        input_error++;
        error_string = sformatf("Expected the name of a NAMED_EXPRESSION.");
        error_msg(error_string, CONTINUE);
        break;
      }
    ...
    case 18: /* add_constant */
    {
      ...
      if (i <= 0) {
        input_error++;
        error_string =
            sformatf("Expected the constant to add for log_K definition.");
        error_msg(error_string, CONTINUE);
        break;
      }
      /* set name */
      s_ptr->add_logk[count_add_logk].name = string_hsave("XconstantX");
      /* read coef */
      opt_save = OPTION_DEFAULT;
    } break;
    case 19: /* vm, molar volume */
      ...
      read_vm_only(next_char, &s_ptr->logk[vm0], &s_ptr->original_deltav_units);
      opt_save = OPTION_DEFAULT;
      break;
    ...
    case OPTION_DEFAULT:
       ...
       */
      {
        ...
        /*
         *   Get pointer to each species in the reaction, store new species if
         * necessary
         */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_exchange(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads exchange data
  ...
  for (;;) {
    ...
    case OPTION_DEFAULT: {
       ...
       */
      if (i != UPPER && token[0] != '[') { /* maybe a bit more clear? */
        error_string = sformatf("Expected exchanger name to begin with a "
                                "capital letter, but found:\n %s",
                                line_save);
        error_msg(error_string, CONTINUE);
        input_error++;
        break;
      ...
      if (i == DIGIT) {
        ...
        if (sscanf(token1.c_str(), SCANFORMAT, &conc) < 1) {
          error_string =
              sformatf("Expected concentration of exchanger, but found:\n %s",
                       line_save);
          error_msg(error_string, CONTINUE);
          input_error++;
          break;
        ...
        if (j == UPPER || j == LOWER) {
          ...
          if (copy_token(token1, &cptr) != DIGIT) {
            error_string = sformatf("Expected a coefficient to relate exchange "
                                    "to kinetic reaction, but found:\n %s",
                                    prev_next_char);
            error_msg(error_string, CONTINUE);
            input_error++;
            break;
        ...
        if (j != DIGIT) {
          if (token1[0] == 'K' || token1[0] == 'k') {
            ...
            error_string =
                sformatf("Character string expected to be 'equilibrium_phase' "
                         "or 'kinetics'\n to relate exchange to mineral or "
                         "kinetic reaction, but found:\n %s",
                         prev_next_char);
            error_msg(error_string, CONTINUE);
            input_error++;
            break;
        ...
        if (j != DIGIT) {
          error_string =
              sformatf("Expected a coefficient to relate exchanger to mineral "
                       "or kinetic reaction, but found:\n %s",
                       prev_next_char);
          error_msg(error_string, CONTINUE);
          input_error++;
          break;
        ...
        error_msg("Expected concentration of exchanger, mineral name, or "
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_exchange_master_species(void)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_gas_phase(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (;;) {
    ...
    case OPTION_DEFAULT: {
      ...
      if (j != 1) {
        error_msg("Expected partial pressure of gas in gas phase.", CONTINUE);
        error_msg(line_save, CONTINUE);
        input_error++;
      } else {
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (inverse[n].isotopes.size() > 1) {
    qsort(&inverse[n].isotopes[0], inverse[n].isotopes.size(),
          sizeof(class inv_isotope), inverse_isotope_compare);
  }

  if (inverse[n].i_u.size() > 1) {
    qsort(&inverse[n].i_u[0], inverse[n].i_u.size(),
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_inv_balances(class inverse *inverse_ptr, const char *cptr)
/* ---------------------------------------------------------------------- */
{
  int j, l;
  char token[MAX_LENGTH];
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_inv_isotopes(class inverse *inverse_ptr, const char *cptr)
/* ---------------------------------------------------------------------- */
{
  int i, j, l, l1, l2;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_inv_phases(class inverse *inverse_ptr, const char *cptr)
/* ---------------------------------------------------------------------- */
{
  int j, l;
  char token[MAX_LENGTH], token1[MAX_LENGTH];
  ...
  for (;;) {
    ...
    if (token1[0] == 'p') {
      ...
      if (copy_token(token, &cptr, &l) != DIGIT) {
        input_error++;
        error_string = sformatf("Expected numeric value for uncertainty in "
                                "isotope ratio, but found:\n %s",
                                prev_next_char);
        error_msg(error_string, CONTINUE);
        continue;
      }
  ...
  if (isotopes.size() > 0) {
    ...
    for (size_t i = 0; i < isotopes.size(); i++) {
      ...
      iso_ptr->isotope_name =
          string_hsave(isotopes[i].Get_isotope_name().c_str());
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_kinetics(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads kinetics data
  ...
  for (;;) {
    ...
    case 1: /* m */
      if (kinetics_comp_ptr == NULL) {
        ...
        if (copy_token(token, &next_char) == DIGIT) {
          ...
          error_string = sformatf("Expecting numerical value for moles of "
                                  "reactant (m), but found:\n %s",
                                  prev_next_char);
          error_msg(error_string, CONTINUE);
          input_error++;
        }
    ...
    case 2: /* m0 */
      if (kinetics_comp_ptr == NULL) {
        ...
        if (copy_token(token, &next_char) == DIGIT) {
          ...
          error_string = sformatf("Expecting numerical value for initial moles "
                                  "of reactant (m0), but found:\n %s",
                                  prev_next_char);
          error_msg(error_string, CONTINUE);
          input_error++;
        }
    ...
    case 7: /* parameters */
      if (kinetics_comp_ptr == NULL) {
        ...
        while ((j = copy_token(token, &next_char)) != EMPTY) {
          ...
          if (j == DIGIT) {
            char *ptr;
            kinetics_comp_ptr->Get_d_params().push_back(
      ...
      if (kinetics_comp_ptr == NULL) {
        ...
        while (copy_token(token, &cptr) != EMPTY) {
          ...
          if (isalpha((int)token[0]) || (token[0] == '(') ||
            ...
            if (!have_name) {
              error_string =
                  sformatf("No phase or chemical formula has been defined.");
              error_msg(error_string, CONTINUE);
              input_error++;
            }
            /*
          ...
          if (j == DIGIT) {
            ...
            if (Utilities::replace("*", " ", token)) {
              ...
              if (sscanf(token.c_str(), "%d" SCANFORMAT, &k, &step) == 2) {
                ...
                input_error++;
                error_msg(
                    "Format error in multiple, equal KINETICS "
            ...
            if (token.substr(0, 1) == "i") {
              ...
              if (temp_kinetics.Get_steps().size() != 1) {
                error_msg("To define equal time increments, only one total "
    ...
    case 10: /* rk */
    {
      ...
      if (j == DIGIT) {
        ...
        error_string = sformatf("Expecting order for Runge-Kutta method.");
        error_msg(error_string, CONTINUE);
        input_error++;
      }
    } break;
    ...
    case 14: /* cvode_order */
    {
      ...
      if (j == DIGIT) {
        ...
        error_string =
            sformatf("Expecting number of terms (order) used in cvode (1-5).");
        error_msg(error_string, CONTINUE);
        input_error++;
      }
    } break;
  ...
  for (size_t i = 0; i < temp_kinetics.Get_kinetics_comps().size(); i++) {
    cxxKineticsComp *kinetics_comp_ptr =
        &(temp_kinetics.Get_kinetics_comps()[i]);
    if (kinetics_comp_ptr->Get_namecoef().size() == 0) {
      kinetics_comp_ptr->Get_namecoef().add(
          kinetics_comp_ptr->Get_rate_name().c_str(), 1.0);
    }
  }
  /*
   *   Default 1 sec
  ...
  if (temp_kinetics.Get_steps().size() == 0) {
    ...
    for (it = temp_kinetics.Get_steps().begin();
         it != temp_kinetics.Get_steps().end(); it++) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_rate_parameters_pk(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads kinetics data
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_mean_gammas(void)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_gas_binary_parameters(void)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_rate_parameters_svd(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads kinetics data
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_rate_parameters_hermanska(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads kinetics data
...
/* ---------------------------------------------------------------------- */
bool Phreeqc::read_vector_doubles(const char **cptr, std::vector<double> &v)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
bool Phreeqc::read_vector_ints(const char **cptr, std::vector<int> &v,
                               int positive)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int *Phreeqc::read_list_ints_range(const char **cptr, int *count_ints,
                                   int positive, int *int_list)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Reads a list of int numbers until end of line is reached or
      ...
      if (replace("-", " ", token) == TRUE) {
        if (sscanf(token, "%d %d", &value1, &value2) != 2) {
          ...
          for (i = value1 + 1; i <= value2; i++) {
            (*count_ints)++;
            int_list = (int *)PHRQ_realloc(int_list,
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_list_ints_range(const char **cptr, bool positive,
...
/* ---------------------------------------------------------------------- */
bool Phreeqc::read_vector_t_f(const char **cptr, std::vector<bool> &v)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_log_k_only(const char *cptr_in, LDBLE *log_k)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Read log k
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_t_c_only(const char *cptr_in, LDBLE *t_c)
/* ---------------------------------------------------------------------- */
{
  *t_c = 0.0;
  std::string stds(cptr_in);
  ...
  if (sscanf(stds.c_str(), SCANFORMAT, t_c) < 1) {
    input_error++;
    error_msg("Expecting numeric value for critical temperature T_c (K)",
              CONTINUE);
    return (ERROR);
  }
  return OK;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_p_c_only(const char *cptr, LDBLE *p_c)
/* ---------------------------------------------------------------------- */
{
  *p_c = 0.0;
  ...
  if (sscanf(stds.c_str(), SCANFORMAT, p_c) < 1) {
    input_error++;
    error_msg("Expecting numeric value for critical pressure P_c (atm)",
              CONTINUE);
    return (ERROR);
  }
  return OK;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_omega_only(const char *cptr, LDBLE *omega)
/* ---------------------------------------------------------------------- */
{
  *omega = 0.0;
  std::string stds(cptr);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_aq_species_vm_parms(const char *cptr, LDBLE *delta_v)
/* ---------------------------------------------------------------------- */
{
  int j;
  /*
  ...
  if (j < 1) {
    input_error++;
    error_msg(
        "Expecting numeric values for calculating the species molar volume.",
        CONTINUE);
    return (ERROR);
  }
  /* multiply with factors. a1 is in cal/mol/bar. a2 in  cal/mol, a3, a4 in cal
    K/mol 41.84004 converts cal/mol/bar to cm3/mol. */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_vm_only(const char *cptr, LDBLE *delta_v, DELTA_V_UNIT *units)
/* ---------------------------------------------------------------------- */
{
  int j, l;
  char token[MAX_LENGTH];
  ...
  j = sscanf(cptr,
             SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_phase_vm(const char *cptr, LDBLE *delta_v,
                           DELTA_V_UNIT *units)
/* ---------------------------------------------------------------------- */
{
  int j, l;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_delta_h_only(const char *cptr_in, LDBLE *delta_h,
                               DELTA_H_UNIT *units)
/* ---------------------------------------------------------------------- */
{
  int j, l, kilo, joul;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_analytical_expression_only(const char *cptr, LDBLE *log_k)
/* ---------------------------------------------------------------------- */
{
  int j;
  ...
  if (j < 1) {
    input_error++;
    error_msg("Expecting numeric values for analytical expression.", CONTINUE);
    return (ERROR);
  }
  return (OK);
}
...
/* VP: Density Start */
/* ---------------------------------------------------------------------- */
int Phreeqc::read_millero_abcdef(const char *cptr, LDBLE *abcdef)
/* ---------------------------------------------------------------------- */
{
  int j;
  /*
  ...
  j = sscanf(cptr,
             SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT
                 SCANFORMAT,
             &(abcdef[0]), &(abcdef[1]), &(abcdef[2]), &(abcdef[3]),
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_viscosity_parms(const char *cptr, LDBLE *Jones_Dole)
/* ---------------------------------------------------------------------- */
{
  int j;
  /*
  ...
  j = sscanf(cptr,
             ...
             &(Jones_Dole[0]), &(Jones_Dole[1]), &(Jones_Dole[2]),
             &(Jones_Dole[3]), &(Jones_Dole[4]), &(Jones_Dole[5]),
             &(Jones_Dole[6]), &(Jones_Dole[7]), &(Jones_Dole[8]),
             &(Jones_Dole[9]));
  if (j < 1) {
    input_error++;
    error_msg("Expecting numeric values for viscosity calculation.", CONTINUE);
    return (ERROR);
  }
  return (OK);
}
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_incremental_reactions(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Define flow only
   ...
   *   find next keyword
   */
  while ((j = check_line("Subroutine Read", FALSE, TRUE, TRUE, FALSE)) !=
         KEYWORD) {
    /* empty, eof, keyword, print */
    if (j == EOF)
      return (EOF);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_master_species(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Reads master species data from data file or input file
  ...
  for (;;) {
    ...
    iss >> ename;
    if (ename != "e" && ename != "E" && ename != "Alkalinity" &&
        std::string::npos == sname.find(ename)) {
      input_error++;
      std::ostringstream oss;
      oss << "Master species, " << sname << " must contain the element, "
          << ename;
    ...
    if (i != 1) {
      input_error++;
      if (elts_ptr != NULL) {
        error_string = sformatf("Expected alkalinity for master species, %s, "
                                "in master species input.",
                                elts_ptr->name);
      } else {
        error_string = sformatf(
    ...
    if (i == DIGIT) {
      ...
      input_error++;
      if (elts_ptr != NULL) {
        error_string = sformatf("Expected gram formula weight for master "
                                "species, %s, in master species input.",
                                elts_ptr->name);
      } else {
        error_string = sformatf("Expected gram formula weight for master "
                                "species in master species input.");
      }
      error_msg(error_string, CONTINUE);
      continue;
    }
    ...
    if (strchr(master[count_master]->elt->name, '(') == NULL) {
      ...
      if (strcmp(master[count_master]->elt->name, "E") != 0) {
        ...
        if (i == DIGIT) {
          ...
          if (elts_ptr != NULL) {
            error_string =
                sformatf("Expected gram formula weight for element, %s.",
                         elts_ptr->name);
          } else {
            error_string =
                sformatf("Expected gram formula weight for element.");
          }
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (temp_mix.Get_mixComps().size() == 0) {
    input_error++;
    error_msg("Must define at least one solution number and mixing fraction "
              "for MIX input.",
              CONTINUE);
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_entity_mix(std::map<int, cxxMix> &mix_map)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Reads mixing fractions
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_number_description(const char *cptr, int *n_user,
...
/* ---------------------------------------------------------------------- */
{
    ...
    *n_user_end = 1;
    ...
    if (replace("-", " ", &token[1])) {
      ...
      if (n != 2) {
        ...
        if (next_keyword >= 0) {
          error_string =
              sformatf("Reading number range for %s.",
                       Keywords::Keyword_name_search(next_keyword).c_str());
        } else {
  ...
  if (*n_user < 0 && allow_negative == FALSE) {
    error_string =
        sformatf("Negative number in number range not allowed for keyword.");
    error_msg(error_string, CONTINUE);
    input_error++;
  }
  /*
...
/* ---------------------------------------------------------------------- */
{
  ...
  for (;;) {
    ...
    case 11: /* add_constant */
    {
      ...
      /* set name */
      phase_ptr->add_logk[count_add_logk].name = string_hsave("XconstantX");
      opt_save = OPTION_DEFAULT;
    } break;
    case 12: /* T_c */
      if (phase_ptr == NULL)
    ...
    case OPTION_DEFAULT: {
      ...
      /*
       *   Copy reaction to reaction for phase, first token (token[0]) is not
       * used except to check that coef of phase formula = 1.0
       */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_pp_assemblage(void)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_reaction(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads reaction data
  ...
  // copy if needed
  Utilities::Rxn_copies(Rxn_reaction_map, n_user,
                        temp_reaction.Get_n_user_end());

  return (return_value);
}

/* ---------------------------------------------------------------------- */
int Phreeqc::read_reaction_reactants(cxxReaction *reaction_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_reaction_steps(cxxReaction *reaction_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
    ...
    if (replace("*", " ", token)) {
      ...
      if (sscanf(token.c_str(), "%d" SCANFORMAT, &n, &value) == 2) {
        ...
        error_msg("Format error in multiple, equal REACTION steps.\nCorrect is "
                  "(for example): 0.2 4*0.1 2*0.5 0.3\n",
                  CONTINUE);
      }
    } else {
      LDBLE step;
  ...
  if (check_units(t1, false, false, NULL, false) == OK) {
    ...
    if (strstr(t1.c_str(), "Mol") == NULL) {
      error_string =
          sformatf("Units of steps not in moles, %s.", token.c_str());
      error_msg(error_string, CONTINUE);
      input_error++;
      return (ERROR);
    } else {
  ...
  if (reaction_ptr->Get_reaction_steps() != 1) {
    error_msg("To define equal increments, only one reaction increment should "
              "be defined.",
              CONTINUE);
    input_error++;
    return (ERROR);
...
/* ---------------------------------------------------------------------- */
{
  ...
  default:
    input_error++;
    error_msg("Expecting keyword solution, equilibrium_phases, exchange, "
              "surface, gas_phase, or solid_solutions.",
              CONTINUE);
    error_msg(line_save, CONTINUE);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_selected_output(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (;;) {
    ...
    case 45: /* selected_output */
             // warning_msg("Use PRINT; -selected_output, not SELECTED_OUTPUT;
      // -selected_output"); value = get_true_false(next_char, TRUE);
  ...
  if (temp_selected_output.Get_new_def() || so == SelectedOutput_map.end()) {
    ...
    if (punch_open(SelectedOutput_map[n_user].Get_file_name().c_str(),
                   n_user)) {
      if (phrq_io) {
        SelectedOutput_map[n_user].Set_punch_ostream(
            phrq_io->Get_punch_ostream());
        phrq_io->Set_punch_ostream(NULL);
      }
    } else {
      error_string =
          sformatf("Can`t open file, %s.",
                   SelectedOutput_map[n_user].Get_file_name().c_str());
      input_error++;
      error_msg(error_string, CONTINUE);
    }
  }
  ...
  //	{
  //		error_string = sformatf( "Can`t open file, %s.",
  //"selected.out"); 		input_error++; 		error_msg(error_string, CONTINUE);
  //	}
  // }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_solution(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads solution data
...
/* ---------------------------------------------------------------------- */
{
  ...
  const char *opt_list[] = {
      ...
      "viscosity" /* 23, b and d parms for viscosity, (b1 + b2 * exp(-b3 * tc))
                     * c + (d1 * exp(-d2 * tc)) * c ^ d3 */
  };
  ...
  for (;;) {
    ...
    case 19: /* tracer diffusion coefficient */
      ...
      s_ptr->dw_a_v_dif = 0;
      i = sscanf(next_char,
                 SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT SCANFORMAT
                     SCANFORMAT SCANFORMAT,
                 &s_ptr->dw, &s_ptr->dw_t, &s_ptr->dw_a, &s_ptr->dw_a2,
                 &s_ptr->dw_a_visc, &s_ptr->dw_a3, &s_ptr->dw_a_v_dif);
      if (i < 1) {
        input_error++;
        error_msg(
            "Expecting numeric values for the diffusion coefficient, its "
            "temperature dependence, and coefficients for the SC calculation.",
            CONTINUE);
        return (ERROR);
      }
  ...
  for (;;) {
    ...
    if (i == DIGIT) {
      ...
      if (strstr(token, "-") != NULL) {
        error_string =
            sformatf("USE does not accept a range of numbers, %s.", token);
        warning_msg(error_string);
        error_string = sformatf(
            "Only %s %d will be used in the batch-reaction calculation.",
            token1, n_user);
        warning_msg(error_string);
        error_string = sformatf("NOTE--USE is not needed for ADVECTION and "
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_surface_species(void)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
{
  ...
  for (;;) {
    ...
    case 5:
      ...
      break;
    case 6:
      temp_surface.Set_only_counter_ions(get_true_false(next_char, TRUE) ==
                                         TRUE);
      break;
      ...
      for (;;) {
        ...
        if (i == DIGIT) {
          ...
          if (token[0] == 'D' || token[0] == 'd') {
            if (thickness != 0) {
              error_msg("You must enter EITHER thickness OR Debye lengths "
                        "(1/k),\n	   and relative DDL viscosity, DDL "
                        "limit.\nCorrect is (for example): -donnan 1e-8 "
                        "viscosity 0.5 limit 0.9 correct_D true\n or (default "
                        "values):     -donnan debye_lengths 1 viscosity 1 "
                        "limit 0.8 correct_D false",
                        CONTINUE);
              error_msg(line_save, CONTINUE);
              input_error++;
              break;
            ...
            if (token1[0] == 'T' || token1[0] == 't' || token1[0] == 'F' ||
                token1[0] == 'f') {
              temp_surface.Set_correct_D(get_true_false(token1.c_str(), TRUE) ==
                                         TRUE);
              continue;
            } else {
              error_msg("Expected True or False for correct_D (which brings "
                        "co-ion concentrations closer to their integrated "
                        "double layer value).",
                        CONTINUE);
              error_msg(line_save, CONTINUE);
              input_error++;
            ...
            if (j == DIGIT) {
              ...
              error_msg("Expected number for maximum of DDL water as fraction "
                        "of bulk water.",
                        CONTINUE);
              error_msg(line_save, CONTINUE);
              input_error++;
            ...
            error_msg("Expected diffuse layer thickness (m) or Debye lengths "
                      "(1/k) for \n\tcalculating the thickness, and relative "
                      "DDL viscosity and DDL limit.\nCorrect is (for example): "
                      "-donnan 1e-8 visc 0.5\n or (default values):     "
                      "-donnan debye_lengths 1 visc 1 lim 0.8",
                      CONTINUE);
            error_msg(line_save, CONTINUE);
            input_error++;
            break;
    ...
    case 15: /* site_units */
    {
      ...
      if (j != EMPTY && (token1[0] == 'A' || token1[0] == 'a')) {
        ...
        error_msg("Character string expected to be 'Absolute' or 'Density' to "
                  "define the units of the first item in the definition of a "
                  "surface component.\n",
                  CONTINUE);
        input_error++;
        break;
    ...
    case 13: /* ccm */
      ...
      if (charge_ptr == NULL) {
        error_msg(
            "A surface must be defined before the capacitance is defined.\n",
            CONTINUE);
        input_error++;
        break;
      }
    ...
    case 17: /* Donnan_factors */
      ...
      for (;;) {
        ...
        if (i == DIGIT && i1 < 8) {
          ...
          error_msg(
              "Expected 4 numbers for the Donnan_factors of single and "
              "double-charged coounter- and co-ions,\n 	z1, z2, z_1, z_2",
              CONTINUE);
          error_msg(line_save, CONTINUE);
          input_error++;
          break;
        ...
        if (i == DIGIT) {
          ...
          if (j != DIGIT) {
            if (token1[0] == 'K' || token1[0] == 'k') {
              ...
              error_msg("Character string expected to be 'equilibrium_phase' "
                        "or 'kinetics' to relate surface to mineral or kinetic "
                        "reaction.\n",
                        CONTINUE);
              input_error++;
              break;
          ...
          /* read proportion */
          if (j != DIGIT) {
            error_msg("Expected a coefficient to relate surface to mineral or "
                      "kinetic reaction.\n",
                      CONTINUE);
            input_error++;
            break;
          ...
          error_msg("Expected concentration of surface, mineral name, or "
                    "kinetic reaction name.",
                    CONTINUE);
          error_msg(line_save, CONTINUE);
          input_error++;
          ...
          if (j != DIGIT) {
            ...
            if (dummy > 0) {
              ...
              if (temp_surface.Get_related_rate() ||
                ...
                error_msg("Can`t transport surfaces related to phases or rates "
                          "(yet).",
                          CONTINUE);
                input_error++;
              }
  ...
  /*
   * copy Dw > 0 to all comps with the same charge structure, check edl & dif
   * for surface transport
   */
  if (temp_surface.Get_transport()) {
    if (temp_surface.Get_type() <= cxxSurface::NO_EDL) {
      input_error++;
      error_msg("Must use default Dzombak and Morel or -cd_music for surface "
                "transport.",
                CONTINUE);
    }
   ...
   */
  if (temp_surface.Get_type() == cxxSurface::DDL ||
      temp_surface.Get_type() == cxxSurface::CCM ||
      temp_surface.Get_type() == cxxSurface::CD_MUSIC) {
    for (size_t i = 0; i < temp_surface.Get_surface_charges().size(); i++) {
      charge_ptr = &(temp_surface.Get_surface_charges()[i]);
      if (charge_ptr->Get_grams() * charge_ptr->Get_specific_area() <= 0) {
  ...
  if (temp_surface.Get_type() == cxxSurface::UNKNOWN_DL) {
    ...
    if (temp_surface.Get_dl_type() != cxxSurface::NO_DL) {
      error_string =
          sformatf("No electrostatic term calculations do not allow "
    ...
    if (temp_surface.Get_dl_type() == cxxSurface::BORKOVEK_DL) {
      error_string = sformatf(
          "Borkovec and Westall diffuse layer calculation is not allowed with "
          "a CD_MUSIC surface.\n\tUsing Donnan diffuse layer calculation.");
      warning_msg(error_string);
      temp_surface.Set_dl_type(cxxSurface::DONNAN_DL);
    }
    if (temp_surface.Get_debye_lengths() > 0) {
      error_msg("Variable DDL thickness is not permitted in CD_MUSIC. Fix DDL "
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_surface_master_species(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Reads master species data from data file or input file
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_psi_master_species(char *token)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_advection(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (;;) {
    ...
    case 8: /* timest */
      (void)sscanf(next_char, SCANFORMAT, &advection_kin_time);
      {
        ...
        if (j == UPPER || j == LOWER) {
          advection_kin_time =
              Utilities::convert_time(advection_kin_time, token, "s");
        }
      }
    ...
    case 15: /* initial_time */
        ...
        (void)sscanf(token, SCANFORMAT, &initial_total_time);
      {
        ...
        if (j == UPPER || j == LOWER) {
          initial_total_time =
              Utilities::convert_time(initial_total_time, stdtoken, "s");
        }
      }
      opt_save = OPTION_DEFAULT;
      break;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::check_key(const char *str)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::check_units(std::string &tot_units, bool alkalinity,
                         bool check_compatibility, const char *default_units,
                         bool print)
/* ---------------------------------------------------------------------- */
{
#define NUNITS (sizeof(units) / sizeof(char *))
   ...
   *	   default_units       character string of default units (check /L, /kg,
   *etc) print	       TRUE print warning messages Output: tot_units standard
   *form for unit
   */
  const char *units[] = {
      "Mol/l",    /* 0 */
  ...
  if (alkalinity && strstr(tot_units.c_str(), "Mol") != NULL) {
    if (print == TRUE) {
      error_string =
          sformatf("Alkalinity given in moles, assumed to be equivalents.");
      warning_msg(error_string);
    }
  ...
  if (print == TRUE) {
    error_string = sformatf("Units for master species, %s, are not compatible "
                            "with default units, %s.",
...
/* ---------------------------------------------------------------------- */
int Phreeqc::find_option(const char *item, int *n, const char **list,
                         int count_list, int exact)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::get_true_false(const char *string, int default_value)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::get_option(const char **opt_list, int count_opt_list,
                        const char **next_char)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (j == EOF) {
    ...
    if (find_option(&(stdoption.c_str()[1]), &opt, opt_list, count_opt_list,
                    FALSE) == OK) {
      j = opt;
    ...
    opt_ptr = line;
    copy_token(stdoption, &opt_ptr);
    if (find_option(&(stdoption.c_str()[0]), &opt, opt_list, count_opt_list,
                    TRUE) == OK) {
      j = opt;
      *next_char = opt_ptr;
    } else {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_user_print(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads basic code with which to calculate rates
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_user_punch(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *      Reads basic code with which to calculate rates
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_solid_solutions(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (;;) {
    ...
    case 5: /* activity coefficients */
      ...
      if (ss_ptr->Get_p().size() != 4) {
        error_string =
            sformatf("Expected 4 parameters to calculate a0 and a1 from two "
                     "activity coefficients, assemblage %d, solid solution %s",
                     n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
      }
    ...
    case 6: /* distribution coefficients */
      ...
      if (ss_ptr->Get_p().size() != 4) {
        error_string = sformatf(
            "Expected 4 parameters to calculate a0 and a1 from two "
            "distribution coefficients, assemblage %d, solid solution %s",
            n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
      }
    ...
    case 7: /* miscibility_gap */
      ...
      if (ss_ptr->Get_p().size() != 2) {
        error_string =
            sformatf("Expected 2 miscibility gap fractions of component 2 to "
                     "calculate a0 and a1, assemblage %d, solid solution %s",
                     n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
      }
    ...
    case 8: /* spinodal_gap */
      ...
      if (ss_ptr->Get_p().size() != 2) {
        error_string =
            sformatf("Expected 2 spinodal gap fractions of component 2 to "
                     "calculate a0 and a1, assemblage %d, solid solution %s",
                     n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
    ...
    case 9: /* critical point */
      ...
      if (ss_ptr->Get_p().size() != 2) {
        error_string = sformatf(
            "Expected fraction of component 2 and critical temperature to "
            "calculate a0 and a1, assemblage %d, solid solution %s",
            n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
    ...
    case 10: /* alyotropic point */
      ...
      if (ss_ptr->Get_p().size() != 2) {
        error_string = sformatf(
            "Expected fraction of component 2 and sigma pi at alyotropic point "
            "to calculate a0 and a1, assemblage %d, solid solution %s",
            n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
      }
    ...
    case 12: /* tempk */
      ...
      }
      {
        ...
        if (j != 1) {
          error_string =
              sformatf("Expected temperature (Kelvin) for parameters, "
                       "assemblage %d, solid solution %s, using 298.15 K",
                       n_user, ss_ptr->Get_name().c_str());
          warning_msg(error_string);
    ...
    case 13: /* tempc */
      ...
      }
      {
        ...
        if (j != 1) {
          error_string =
              sformatf("Expected temperature (Celsius) for parameters, "
                       "assemblage %d, solid solution %s, using 25 C",
                       n_user, ss_ptr->Get_name().c_str());
          warning_msg(error_string);
          ss_ptr->Set_tk(298.15);
        }
    ...
    case 14: /* Thompson and Waldbaum */
      ...
      if (ss_ptr->Get_p().size() != 2) {
        error_string =
            sformatf("Expected Wg2 and Wg1 Thompson-Waldbaum parameters to "
                     "calculate a0 and a1, assemblage %d, solid solution %s",
                     n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
    ...
    case 15: /* Margules */
      ...
      if (ss_ptr->Get_p().size() != 2) {
        error_string =
            sformatf("Expected alpha2 and alpha3 Margules parameters to "
                     "calculate a0 and a1, assemblage %d, solid solution %s",
                     n_user, ss_ptr->Get_name().c_str());
        error_msg(error_string, CONTINUE);
        input_error++;
  ...
  for (size_t i = 0; i < ss_v.size(); i++) {
    if (ss_v[i]->Get_p()[0] != 0.0 || ss_v[i]->Get_p()[1] != 0.0) {
      if (ss_v[i]->Get_ss_comps().size() != 2) {
        error_string = sformatf("Solid solution, %s, is nonideal. Must define "
                                "exactly two components (-comp1 and -comp2).",
...
/* ---------------------------------------------------------------------- */
int Phreeqc::next_keyword_or_option(const char **opt_list, int count_opt_list)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_named_logk(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (;;) {
    ...
    case 7: /* ln_alpha1000 */
      ...
      if (empty == FALSE) {
        error_string = sformatf(
            "Analytical expression previously defined for %s in "
    ...
    case 9: /* add_log_k */
    {
      ...
      logk_ptr->add_logk[count_add_logk].name = string_hsave(token);
      /* read coef */
      i = sscanf(next_char, SCANFORMAT,
...
/* ---------------------------------------------------------------------- */
{
  ...
  default:
    input_error++;
    error_msg("Expecting keyword solution, mix, kinetics, reaction, "
              "reaction_pressure, reaction_temperature, equilibrium_phases, "
              "exchange, surface, gas_phase, or solid_solutions, or cell.",
              CONTINUE);
    error_msg(line_save, CONTINUE);
    check_line("End of use", FALSE, TRUE, TRUE, TRUE);
    /* empty, eof, keyword, print */
  ...
  if (i == DIGIT) {
    ...
    error_msg("Source index number must be an integer.", CONTINUE);
    error_msg(line_save, CONTINUE);
    input_error++;
    return (ERROR);
  }
  ...
  if (i == DIGIT) {
    ...
    error_msg("Target index number must be an integer.", CONTINUE);
    error_msg(line_save, CONTINUE);
    input_error++;
    return (ERROR);
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_reaction_pressure(void)
/* ---------------------------------------------------------------------- */
{
  /*
   ...
   *         KEYWORD if keyword encountered, input_error may be incremented if
   *                    a keyword is encountered in an unexpected position
   *         EOF     if eof encountered while reading mass balance
   * concentrations ERROR   if error occurred reading data
   *
   */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_reaction_pressure_raw(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  Utilities::Rxn_copies(Rxn_pressure_map, atm.Get_n_user(),
                        atm.Get_n_user_end());

  return cleanup_after_parser(parser);
}
int Phreeqc::cleanup_after_parser(CParser &parser) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::read_temperature(void)
/* ---------------------------------------------------------------------- */
{
  /*
