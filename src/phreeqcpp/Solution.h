...
public:
  ...
  cxxSolution(std::map<int, cxxSolution> &solution_map, cxxMix &mx, int n_user,
              PHRQ_io *io = NULL);
  virtual ~cxxSolution();
  ...
  std::map<int, double> &Get_log_molalities_map(void) {
    return this->log_molalities_map;
  }
  std::map<std::string, cxxSolutionIsotope> &Get_isotopes(void) {
    return this->isotopes;
  }
  const std::map<std::string, cxxSolutionIsotope> &Get_isotopes(void) const {
    return this->isotopes;
  }
  void Set_isotopes(const std::map<std::string, cxxSolutionIsotope> &iso) {
    this->isotopes = iso;
  }
  ...
  void Add_isotopes(const std::map<std::string, cxxSolutionIsotope> &old,
  ...
  void Update(LDBLE h_tot, LDBLE o_tot, LDBLE charge, LDBLE tc, LDBLE patm, const cxxNameDouble &nd); // MDL reverted 20260219 , LDBLE massh2o, LDBLE viscosity, LDBLE density
  void Update_activities(const cxxNameDouble &original_tot);
  void Serialize(Dictionary &dictionary, std::vector<int> &ints,
                 std::vector<double> &doubles);
  void Deserialize(Dictionary &dictionary, std::vector<int> &ints,
