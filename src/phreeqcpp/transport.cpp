...
struct CURRENT_CELLS {
  LDBLE dif, ele,
      R; // diffusive and electric components, relative cell resistance
} *current_cells;
LDBLE sum_R, sum_Rd; // sum of R, sum of (current_cells[0].dif -
                     // current_cells[i].dif) * R
struct V_M // For calculating Vinograd and McBain's zero-charge, diffusive
           // transfer of individual solutes
{
  LDBLE grad, D, z, c, zc, Dz, Dzc;
  LDBLE b_ij; // harmonic mean of cell properties, with EDL enrichment
};
struct CT /* summed parts of V_M and mcd transfer in a timestep for all cells,
             for free + DL water */
{
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (multi_Dflag) {
    sol_D =
        (class sol_D *)PHRQ_malloc((size_t)(all_cells) * sizeof(class sol_D));
    if (sol_D == NULL)
      malloc_error();
    ...
    moles_added = (struct MOLES_ADDED *)PHRQ_malloc(
        (size_t)(count_moles_added) * sizeof(struct MOLES_ADDED));
    if (moles_added == NULL)
      malloc_error();
  ...
  try {
    ...
    if (!use.Get_solution_ptr()) {
      if (ishift == 1) {
        input_error++;
        error_string =
            sformatf("Solution 0 is needed for transport, but is not defined.");
        error_msg(error_string, CONTINUE);
      } else
    ...
    use.Set_solution_ptr(
        Utilities::Rxn_find(Rxn_solution_map, count_cells + 1));
    if (!use.Get_solution_ptr()) {
      if (ishift == -1) {
        input_error++;
        error_string =
            sformatf("Solution %d is needed for transport, but is not defined.",
                     count_cells + 1);
        error_msg(error_string, CONTINUE);
      } else
        Utilities::Rxn_copy(Rxn_solution_map, count_cells, count_cells + 1);
    } else {
      if ((cell_data[count_cells + 1].potV =
               use.Get_solution_ptr()->Get_potV()))
        dV_dcell = 1;
    }
    /*
    ...
    for (n = 1; n <= stag_data.count_stag; n++) {
      for (i = 1; i <= count_cells; i++) {
        ...
        if (n == 1 && implicit && use.Get_solution_ptr() == NULL) {
          input_error++;
          error_string = sformatf("Stagnant solution %d not found for implicit "
                                  "diffusion with 1 stagnant layer.\n       "
                                  "Please define it, or set -implicit false.",
                                  k);
          error_msg(error_string, CONTINUE);
        }
      }
    ...
    if (fix_current && !dV_dcell && !implicit) {
      warning_msg(
          "fix_current (A) was defined, but potential in a boundary cell was "
    ...
    if (dV_dcell) {
      ...
      if (!multi_Dflag) {
        ...
        if (bcon_first != 1) {
          bcon_first = 1;
          error_string =
              sformatf("Electrical Field (potential) was defined, assuming "
                       "constant boundary condition for first cell.");
        ...
        if (bcon_last != 1) {
          ...
          error_string =
              sformatf("Electrical Field (potential) was defined, assuming "
                       "constant boundary condition for last cell.");
          warning_msg(error_string);
        }
    ...
    if (implicit && interlayer_Dflag) {
      input_error++;
      error_string = sformatf("Interlayer diffusion cannot be calculated "
                              "implicitly. Please remove -implicit true.");
      error_msg(error_string, CONTINUE);
    }
    if (implicit && !multi_Dflag) {
      input_error++;
      error_string =
          sformatf("Implicit diffusion needs diffusion coefficients for "
                   "individual species. Please add -multi_d true.");
      error_msg(error_string, CONTINUE);
    }
    if (implicit && !timest) {
      error_string = sformatf(
    ...
    for (i = 0; i <= count_cells + 1; i++) {
      ...
      if (use.Get_surface_ptr() != NULL &&
          use.Get_surface_ptr()->Get_transport())
        transp_surf = TRUE;
      if (transp_surf && !multi_Dflag) {
        error_string =
            sformatf("-multi_d must be defined for surface transport");
        error_msg(error_string, CONTINUE);
      }
      if (multi_Dflag == TRUE) {
        fill_spec(cell_no, 0);
     ...
     *  for multicomponent diffusion, limit mixing by diffc_max (usually from
     * H+)
     */
    if (multi_Dflag)
      diffc_tr = diffc_max;
    if ((stag_data.exch_f > 0) && (stag_data.count_stag == 1)) {
      ...
      if (multi_Dflag == TRUE) {
        snprintf(token, sizeof(token),
                 "multi_D calc's and stagnant: define MIXing factors "
                 "explicitly, or \n\t give in -multi_D the Dw used for "
                 "calculating the mobile-immobile exchange factor.");
        warning_msg(token);
      }
     ...
     * structure stag_data.
     * MIX 'cell_no' in input file can be an alternative for the calculation
     * here.
     */
    if ((stag_data.exch_f > 0) && (stag_data.count_stag == 1)) {
      ...
      for (j = 1; j <= count_cells; j++) {
        ...
        water_imm =
            Utilities::Rxn_find(Rxn_solution_map, j_imm)->Get_mass_water();
        /*
      ...
      if (heat_nmix > 0) {
        /*
         * Assumption: D_e used for calculating exch_f in input file equals
         * diffc
         */
    ...
    if (implicit)
      snprintf(token, sizeof(token),
               "\nCalculating implicit transport: %d (mobile) cells, %d "
               "shifts, %d mixruns, max. mixf = %g.\n\n",
               count_cells, count_shifts - transport_start + 1, nmix, max_mixf);
    else
      snprintf(token, sizeof(token),
               "\nCalculating transport: %d (mobile) cells, %d shifts, %d "
               "mixruns...\n\n",
    ...
    for (transport_step = transport_start; transport_step <= count_shifts;
        ...
        for (j = 1; j <= floor((LDBLE)nmix / 2); j++) {
          ...
          for (i = 0; i <= count_cells + 1; i++) {
            if (!dV_dcell && (i == 0 || i == count_cells + 1) && !implicit) {
              if (j == nmix && stag_data.count_stag == 0 &&
                  (cell_data[0].print || cell_data[0].punch ||
                   cell_data[count_cells + 1].print ||
                   cell_data[count_cells + 1].punch))
                print_punch(i, false);
              continue;
            }
            ...
            if (ishift == 0 && j == nmix &&
                (stag_data.count_stag == 0 ||
                 (implicit && stag_data.count_stag == 1)))
              print_punch(i, true);
            if (i > 1)
          ...
          if (stag_data.count_stag > 0) {
            if (ishift == 0 && j == nmix)
              ...
              punch_boolean = FALSE;
            for (i = 0; i <= count_cells + 1;
                 i++) // allow for stagnant cell mixing with boundary cells
            {
      ...
      if (ishift != 0) {
        ...
        if (Utilities::Rxn_find(Rxn_kinetics_map, first_c) != NULL &&
            count_cells > 1) {
          cell_no = first_c;
        ...
        if (transp_surf) {
          for (i = last_c + ishift; i != (first_c - ishift); i -= ishift) {
            ...
            cxxSurface *surface_ptr =
                Utilities::Rxn_find(Rxn_surface_map, i - ishift);
            if (surface_ptr == NULL) {
            ...
            if (surface_ptr->Get_transport()) {
              cxxSurface *surface_ptr1 =
                  Utilities::Rxn_find(Rxn_surface_map, i);
      ...
      for (; j <= nmix; j++) // loop on j
      {
        ...
        rate_sim_time_start =
            ((double)transport_step - 1) * timest + ((double)j - 1) * kin_time;
        if (ishift != 0)
        ...
        if (!dV_dcell)
          Utilities::Rxn_copy(Rxn_solution_map, -2, count_cells);
        /* Stagnant zone mixing after completion of each diffusive/dispersive
         * step ... */
        rate_sim_time_start = ((double)transport_step - 1) * timest +
                              ((double)j - 1) * stagkin_time;
        rate_sim_time = rate_sim_time_start + stagkin_time;
        ...
        if ((it1 = neg_moles.find(i)) != neg_moles.end() &&
          ...
          for (it2 = els.begin(); it2 != els.end(); it2++) {
            for (int i1 = 0; i1 < count_moles_added; i1++) {
              if (moles_added[i1].name &&
                  !strcmp(moles_added[i1].name, it2->first.c_str())) {
    ...
    if (multi_Dflag && moles_added[0].moles > 0) {
      snprintf(token, sizeof(token),
               "\nFor balancing negative concentrations in MCD, added in total "
               "to the system:");
      ...
      for (i = 0; i < count_moles_added; i++) {
        if (!moles_added[i].moles)
          break;
        snprintf(token, sizeof(token), "\t %.4e moles %s.",
...
/* ---------------------------------------------------------------------- */
void Phreeqc::transport_cleanup(void)
/* ---------------------------------------------------------------------- */
{
  int i;
  /*
  ...
  if (change_surf_count > 0) {
    for (int k = 0; k < change_surf_count; k++) {
      if (change_surf[k].cell_no != i)
        break;
      reformat_surf(change_surf[k].comp_name, change_surf[k].fraction,
                    change_surf[k].new_comp_name, change_surf[k].new_Dw,
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (multi_Dflag) {
    if (dV_dcell)
      dV_dcell =
          (cell_data[count_cells + 1].potV - cell_data[0].potV) / count_cells;
    for (i = 1; i <= count_cells; i++) {
    ...
    if (bcon_last == 1) {
      mD = 2 * diffc_max * timest /
           (cell_data[count_cells].length * cell_data[count_cells].length);
      if (mD > maxmix)
        maxmix = mD;
      if (ishift != 0) {
        m1[count_cells] = 2 * cell_data[count_cells].disp /
                          cell_data[count_cells].length * corr_disp;
    ...
    if (maxmix == 0) {
      ...
      if (2.25 * maxmix + 1.0 > (double)INT_MAX) {
        ...
        char token[MAX_LENGTH];
        snprintf(token, sizeof(token),
                 "Calculated number of mixes %g, is beyond program "
                 "limit,\nERROR: please set implicit true, or decrease "
                 "time_step, or increase cell-lengths.",
                 2.25 * maxmix);
  ...
  } else // multi_D false
  {
    ...
    for (i = 1; i <= count_cells; i++) {
      if (i < count_cells) {
        ...
        // add diffusive mixf...
        m1[i] += diffc_here / (cell_data[i].length * cell_data[i].length +
                               cell_data[i].length * cell_data[i + 1].length);
      ...
      if (i > 1) {
        ...
        // add diffusive mixf...
        m[i] += diffc_here / (cell_data[i].length * cell_data[i].length +
                              cell_data[i].length * cell_data[i - 1].length);
        m[i] *= corr_disp; /* m[i] has mixf with lower numbered cell */
        if (m[i] != m1[i - 1] && !warning &&
            (!dav || m[i] / (2 / dav) > 1.00001)) {
          warning_msg("Unequal cell-lengths may give mass-balance error, "
                      "consider using -multi_D");
          warning = true;
        }
      }
      mf12 = m[i] + m1[i];
    ...
    if (bcon_last == 1) {
      m1[count_cells] = diffc_here / (cell_data[count_cells].length *
                                      cell_data[count_cells].length);
      if (ishift != 0)
        m1[count_cells] +=
            cell_data[count_cells].disp / cell_data[count_cells].length;
      mf12 = m[count_cells] + m1[count_cells];
      if (mf12 > maxmix)
        maxmix = mf12;
    }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::mix_stag(int i, LDBLE kin_time, int l_punch, LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
  int n, k;
  LDBLE t_imm;
  ...
  for (n = 1; n <= stag_data.count_stag; n++) {
    if (i == 0 || i == count_cells + 1) {
      ...
      if (use.Get_mix_ptr()) {
        for (std::map<int, LDBLE>::const_iterator it =
                 use.Get_mix_ptr()->Get_mixComps().begin();
             it != use.Get_mix_ptr()->Get_mixComps().end(); it++) {
          if (it->first > i && it->first < all_cells &&
              it->first != count_cells + 1) {
            k = it->first;
    ...
    if (ptr_imm != NULL) {
      if (n == 1) {
        if (heat_nmix > 0 &&
            (!implicit || (implicit && stag_data.count_stag > 1))) {
          ptr_m = Utilities::Rxn_find(Rxn_solution_map, i);
          t_imm = heat_mix_f_imm * ptr_m->Get_tc() +
                  (1 - heat_mix_f_imm) * ptr_imm->Get_tc();
          ptr_m->Set_tc(heat_mix_f_m * ptr_imm->Get_tc() +
                        (1 - heat_mix_f_m) * ptr_m->Get_tc());
        ...
        if (!implicit || (implicit && stag_data.count_stag > 1)) {
          ...
          if (l_punch)
            print_punch(i, true);
          saver(); // save solution i in -2, original can be used in other
                   // stagnant mixes
        }
      }
      ...
      if (multi_Dflag == TRUE)
        fill_spec(cell_no, i);
      saver(); // save solution k in -2 - k, original k can be used in other
               // stagnant mixes
  ...
  if (done_mixing) // after all mixing is done, the temporal solution becomes
                   // the original for the next timestep
  {
    for (n = 1; n <= stag_data.count_stag; n++) {
      k = i + 1 + n * count_cells;
      if (Utilities::Rxn_find(Rxn_solution_map, k) != 0) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::init_heat_mix(int l_nmix)
/* ---------------------------------------------------------------------- */
{
  ...
  if (l_heat_nmix == 0) {
    if (fabs(Utilities::Rxn_find(Rxn_solution_map, count_cells + 1)->Get_tc() -
             t0) > 1.0)
      l_heat_nmix = 1;
    for (n = 1; n <= stag_data.count_stag; n++) {
      for (i = 1; i < count_cells; i++) {
  ...
  if (maxmix == 0)
    ...
    if (implicit) {
      for (i = 1; i <= count_cells + 1; i++) {
        heat_mix_array[i - 1] =
            heat_mix_array[i] /
            l_nmix; /* for implicit, m[i] has mixf with higher cell */
        // heat_mix_array[i - 1] *= viscos_0_25 / ((sol_D[i - 1].viscos_0 +
        // sol_D[i].viscos_0) / 2); // 10/14/24: done in diffuse implicit
      }
    } else {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::heat_mix(int l_heat_nmix)
/* ---------------------------------------------------------------------- */
{
  int i, j;
  ...
  for (i = 1; i <= l_heat_nmix; i++) {
    for (j = 1; j <= count_cells; j++) {
      ...
      temp2[j] = heat_mix_array[j] * viscos_f * temp1[j - 1] +
                 heat_mix_array[j + 1] * viscos_f1 * temp1[j + 1] +
                 (1 - heat_mix_array[j] * viscos_f -
                  heat_mix_array[j + 1] * viscos_f1) *
                     temp1[j];
    }
    for (j = 1; j <= count_cells; j++)
      temp1[j] = temp2[j];
...
/* ---------------------------------------------------------------------- */
int Phreeqc::set_initial_moles(int i)
/* ---------------------------------------------------------------------- */
{
   ...
   */
  {
    cxxPPassemblage *pp_assemblage_ptr =
        Utilities::Rxn_find(Rxn_pp_assemblage_map, i);
  ...
  if (kinetics_ptr != NULL) {
    for (j = 0; j < (int)kinetics_ptr->Get_kinetics_comps().size(); j++) {
      cxxKineticsComp *kinetics_comp_ptr =
          &(kinetics_ptr->Get_kinetics_comps()[j]);
   ...
   */
  {
    cxxSSassemblage *ss_assemblage_ptr =
        Utilities::Rxn_find(Rxn_ss_assemblage_map, i);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::fill_spec(int l_cell_no, int ref_cell)
/* ---------------------------------------------------------------------- */
{
  ...
  // sol_D[l_cell_no].spec = (class spec *)
  // free_check_null(sol_D[l_cell_no].spec);
  if (sol_D[l_cell_no].spec == NULL) {
    sol_D[l_cell_no].spec = (class spec *)PHRQ_malloc(
        (species_list.size() + (size_t)size_xt) * sizeof(class spec));
    sol_D[l_cell_no].spec_size = (int)species_list.size() + size_xt;
  } else if ((int)species_list.size() + size_xt > sol_D[l_cell_no].spec_size) {
    sol_D[l_cell_no].spec = (class spec *)PHRQ_realloc(
  ...
  sol_D[l_cell_no].tk_x = l_tk_x =
      Utilities::Rxn_find(Rxn_solution_map, l_cell_no)->Get_tc() + 273.15;
  ...
  /*
   * correct diffusion coefficient for temperature Dw(TK) = Dw(298.15) *
   * exp(dw_t / TK - dw_t / 298.15), SC data from Robinson and Stokes, 1959 and
   * viscosity, D_T = D_298 * viscos_0_298 / viscos_0_tk
   */
  sol_D[l_cell_no].viscos = viscos =
      Utilities::Rxn_find(Rxn_solution_map, l_cell_no)->Get_viscosity();
  sol_D[l_cell_no].viscos_0 = viscos0 =
      Utilities::Rxn_find(Rxn_solution_map, l_cell_no)->Get_viscos_0();
  ...
  if (species_list.size() > 1) {
    qsort(&species_list[0], species_list.size(), sizeof(class species_list),
          sort_species_name);
  }
  if (correct_Dw)
    calc_SC();
  ...
  for (i = 0; i < (int)species_list.size(); i++) {
    ...
    if (s_ptr->type > HPLUS && !(s_ptr->type == EX && interlayer_Dflag))
      continue;
    ...
    if (s_ptr->type == EX) {
      if (s_ptr->lm > min_dif_LM) {
        ...
        if (strcmp(name, "X") != 0) {
          if (!warn_MCD_X) {
            snprintf(token, sizeof(token),
                     "MCD found more than 1 exchanger, uses X for interlayer "
                     "diffusion.");
            warning_msg(token);
            warn_MCD_X = 1;
          }
        ...
        sol_D[l_cell_no].spec[count_spec].a =
            dum2 * pow(10, sol_D[l_cell_no].spec[count_spec].lg);
        sol_D[l_cell_no].exch_total = master_ptr->total;
        if (transport_step == 0 && !x_max_done) {
          x_max_done = true;
          dum = master_ptr->total /
                Utilities::Rxn_find(Rxn_solution_map, l_cell_no)
                    ->Get_mass_water();
        ...
        if (s_ptr2->dw == 0) {
          ...
          if (s_ptr2->dw_t) {
            sol_D[l_cell_no].spec[count_spec].Dwt *=
                exp(s_ptr2->dw_t / l_tk_x - s_ptr2->dw_t / 298.15);
        ...
        // if (implicit) // && name_ret.second && (l_cell_no > 1 || (l_cell_no
        // == 1 && bcon_first != 2)))
        //{
    ...
    if (lm > min_dif_LM) {
      if (implicit /* && l_cell_no < count_cells + 2 */) {
        ...
        if (i2 > count_spec) {
          if (l_cell_no == 0) {
            ...
            i1 = 0;
            for (it = loc_spec_names.begin(); it != loc_spec_names.end();
                 it++) {
              if (*it == s_ptr->name)
                break;
              i1++;
            ...
            if (i3 + count_spec + 1 > sol_D[l_cell_no].spec_size) {
              sol_D[l_cell_no].spec = (class spec *)PHRQ_realloc(
                  sol_D[l_cell_no].spec,
                  ((size_t)i3 + count_spec + 1 + (size_t)size_xt) *
                      sizeof(class spec));
              if (sol_D[l_cell_no].spec == NULL)
                malloc_error();
            ...
            {
              // memmove(&sol_D[l_cell_no].spec[i1], &sol_D[ref_cell].spec[i1],
              // sizeof(class spec));
      ...
      if (s_ptr->dw == 0) {
        ...
        if (s_ptr->dw_t) {
          ...
          sol_D[l_cell_no].spec[count_spec].Dwt *=
              exp(s_ptr->dw_t / l_tk_x - s_ptr->dw_t / 298.15);
      ...
      if (s_ptr->dw_a_v_dif) {
        ...
        sol_D[l_cell_no].spec[count_spec].Dwt *=
            pow(viscos0 / viscos, s_ptr->dw_a_v_dif);
      } else
        sol_D[l_cell_no].spec[count_spec].dw_a_v_dif = 0.0;
      if (correct_Dw) {
        // calc_SC(); // removed that neutral species are corrected as if z = 1,
        // but is viscosity-dependent
        sol_D[l_cell_no].spec[count_spec].Dwt = s_ptr->dw_corr;
      }
      if (l_cell_no <= count_cells + 1 &&
        ...
        if (name_ret.second && l_cell_no) {
          ...
          for (i1 = 0; i1 < l_cell_no; i1++) {
            ...
            if (i2 > sol_D[i1].spec_size) {
              sol_D[i1].spec = (class spec *)PHRQ_realloc(
                  sol_D[i1].spec,
            ...
            // memmove(&sol_D[i1].spec[i2], &sol_D[l_cell_no].spec[i2],
            // sizeof(class spec));
  ...
  if (implicit /* && l_cell_no < count_cells + 2 */ &&
      loc_spec_names.size() < dif_spec_names.size()) {
  ...
  if (implicit && l_cell_no /* && l_cell_no < count_cells + 2 */) {
    for (i = 0; i < count_spec; i++) {
      ...
      if (strcmp(sol_D[l_cell_no].spec[i].name, sol_D[ref_cell].spec[i].name)) {
        error_string = sformatf("Implicit diffusion: species %s in cell %d "
                                "differs from species %s in previous cells.",
                                sol_D[l_cell_no].spec[i].name, l_cell_no,
                                sol_D[ref_cell].spec[i].name);
        error_msg(error_string, CONTINUE);
      }
    }
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::sort_species_name(const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
void Phreeqc::diffuse_implicit(LDBLE DDt, int stagnant)
/* ---------------------------------------------------------------------- */
{
  // The structure comes from example 11.3 in A&P and Appelo 2017, CCR 101, 102:
  // Ct2 is implicitly solved for the individual species, corrected for
  // electro-neutrality, this always gives Ct2 > 0. Version 3.5.2 includes
  // diffusion in 1 stagnant layer in the implicit calc'n. Transport of aqueous
  // species is summarized into master species. With electro-migration,
  // transport of anions and cations is calculated in opposite directions since
  // (sign) J = - z * dV. Only available moles are transported, thus are > 0,
  // but if concentrations oscillate, decrease max_mixf in input file: -implicit
  // true 0.7 # max_mixf = 0.7 (default = 1).
  //         or define time substeps: -time 0.5e6 year 20 # substeps = 20
  //         (default = 1).
  int i, icell, cp, comp;
  // ifirst = (bcon_first == 2 ? 1 : 0); ilast = (bcon_last == 2 ? count_cells -
  // 1 : count_cells);
  int ifirst, ilast;
  int i_1, i0, i1, i2 = 0;
  // double mfr, mfr1, max_b = 0, b, grad, dVc, j_0e, min_dif_M = pow(10,
  // min_dif_LM);
  ...
  std::map<int, std::map<std::string, double>>::iterator it1;
  std::map<std::string, double>::iterator it2;
  // c= count_cells, c1=(c+1)= end boundary-cell, c_1=(c-1), c2=(c+2)= first
  // stagnant cell, cc1=(c+c+1)= last stagnant cell
  int c = count_cells, c1 = c + 1, c2 = c + 2, c_1 = c - 1,
      cc = c + stagnant * c, cc1 = cc + 1;
  ...
  if (Ct2 == NULL)
    Ct2 = (LDBLE *)PHRQ_malloc(
        (count_cells + 2 + (size_t)stagnant * count_cells) * sizeof(LDBLE));
  if (Ct2 == NULL)
    malloc_error();
  if (l_tk_x2 == NULL)
    l_tk_x2 = (LDBLE *)PHRQ_malloc(
        (count_cells + 2 + (size_t)stagnant * count_cells) * sizeof(LDBLE));
  if (l_tk_x2 == NULL)
    malloc_error();
  ...
  if (comp + 2 > mixf_comp_size) {
    for (i = 0; i < count_cells + 2; i++) {
      if (mixf[i] == NULL)
        ...
        mixf[i] =
            (LDBLE *)PHRQ_realloc(mixf[i], ((size_t)comp + 2) * sizeof(LDBLE));
      if (mixf[i] == NULL)
        malloc_error();
      if (stagnant) {
        if (mixf_stag[i] == NULL)
          mixf_stag[i] =
              (LDBLE *)PHRQ_malloc(((size_t)comp + 2) * sizeof(LDBLE));
        else
          mixf_stag[i] = (LDBLE *)PHRQ_realloc(
              mixf_stag[i], ((size_t)comp + 2) * sizeof(LDBLE));
        if (mixf_stag[i] == NULL)
          malloc_error();
        for (cp = 0; cp < comp; cp++)
          mixf_stag[i][cp] = 0;
  ...
  for (i = 0; i <= count_cells + 1; i++) {
    ...
    if (stagnant) {
      ...
      if (use.Get_mix_ptr() == NULL) {
        i1 = 0;
        if (i == 0 &&
            (sptr1 = Utilities::Rxn_find(Rxn_solution_map, c2)) != NULL) {
          // the 1st boundary solution diffuses into 1st stagnant cell (=c2) by
          // the ratio immobile_kgw in cell c2 / mobile_kgw in cell 1
          find_J(0, 1, 1, 1, 0);
          ct[c2].kgw = sptr1->Get_mass_water();
          for (cp = 0; cp < comp; cp++) {
            if (!heat_nmix || cp < comp - 1)
              mixf_stag[i][cp] =
                  DDt * ct[i].v_m[cp].b_ij * ct[c2].kgw / ct[1].kgw;
            else if (heat_nmix && cp == comp - 1)
              mixf_stag[i][cp] =
                  mixf_stag[i][0] * heat_diffc / tempr / sol_D[i].spec[0].Dwt;
          }
          i1 = c2;
        } else if (i == c1 && (sptr1 = Utilities::Rxn_find(Rxn_solution_map,
                                                           cc1)) != NULL) {
          ...
          for (cp = 0; cp < comp; cp++) {
            if (!heat_nmix || cp < comp - 1)
              mixf_stag[i][cp] =
                  DDt * ct[c].v_m[cp].b_ij * ct[cc1].kgw / ct[c].kgw;
            else if (heat_nmix && cp == comp - 1)
              mixf_stag[i][cp] =
                  mixf_stag[i][0] * heat_diffc / tempr / sol_D[c].spec[0].Dwt;
          }
          i1 = cc1;
        }
        if (i1) {
  ...
  for (cp = 0; cp < comp; cp++) {
    ...
    if (heat_nmix && cp == comp - 1) {
      ...
      mfr1 = mixf[1][cp] = heat_mix_array[1] * viscos_0_25 /
                           ((sol_D[1].viscos_0 + sol_D[2].viscos_0) / 2);
    } else {
    ...
    if (bcon_first == 2) {
      ...
      if (stagnant) {
        ...
        if (mixf_stag[0][cp]) {
          if (dV_dcell) {
            A[0][0] += mixf_stag[0][cp];
            A[0][c2] = -mixf_stag[0][cp];
          }
    ...
    if (heat_nmix && cp == comp - 1) {
      mfr = mixf[c_1][cp] = heat_mix_array[c_1] * viscos_0_25 /
                            ((sol_D[c_1].viscos_0 + sol_D[c].viscos_0) / 2);
      mfr1 = mixf[count_cells][cp] =
          heat_mix_array[count_cells] * viscos_0_25 / sol_D[c].viscos_0;
    } else {
    ...
    if (bcon_last == 2) {
      if (stagnant) {
        A[count_cells][c_1] = -mfr;
        A[count_cells][count_cells] = 1 + mfr;
        A[c1][c1] = A[cc1][cc1] = 1;
        if (mixf_stag[count_cells][cp]) {
          A[count_cells][count_cells] += mixf_stag[count_cells][cp];
          A[count_cells][cc1] = A[cc1][count_cells] =
              -mixf_stag[count_cells][cp];
          A[cc1][cc1] += mixf_stag[count_cells][cp];
        }
      } else {
        A[count_cells][0] = -mfr;
        A[count_cells][1] = 1 + mfr;
        A[count_cells][2] = 0;
        A[c1][0] = 0;
        A[c1][1] = 1;
      }
      ...
      if (stagnant) {
        A[count_cells][c_1] = -mfr;
        A[count_cells][count_cells] = 1 + mfr + mfr1;
        A[count_cells][c1] = -mfr1;
        if (dV_dcell) {
          A[c1][count_cells] = -mfr1;
          A[c1][c1] = 1 + mfr1;
        } else
          A[c1][c1] = 1;
        A[cc1][cc1] = 1;
        ...
        if (mixf_stag[c1][cp]) {
          if (dV_dcell) {
            A[c1][c1] += mixf_stag[c1][cp];
            A[c1][cc1] = -mixf_stag[c1][cp];
          }
        ...
        A[count_cells][0] = -mfr;
        A[count_cells][1] = 1 + mfr + mfr1;
        A[count_cells][2] = -mfr1;
        if (dV_dcell) {
          A[c1][0] = -mfr1;
          A[c1][1] = 1 + mfr1;
        } else {
    ...
    for (i = 2; i < count_cells; i++) {
      if (heat_nmix && cp == comp - 1) {
        mfr = mixf[i - 1][cp] =
            heat_mix_array[i - 1] * viscos_0_25 /
            ((sol_D[i - 1].viscos_0 + sol_D[i].viscos_0) / 2);
        mfr1 = mixf[i][cp] = heat_mix_array[i] * viscos_0_25 /
                             ((sol_D[i].viscos_0 + sol_D[i + 1].viscos_0) / 2);
      } else {
      ...
      if (stagnant && mixf_stag[i][cp]) {
        A[i][i - 1] = -mfr;
        A[i][i] = 1 + mfr + mfr1 + mixf_stag[i][cp];
        A[i][i + 1] = -mfr1;
    ...
    // decompose full A in LU, bypass A[][] = 0...
    {
      ...
      for (i = 1; i <= cc1; i++) {
        ...
        if (i < c2) {
          ...
          // the i = count_cells + 2 and higher rows are filled from i0 onwards,
          // need to subtract the L*U terms for the ith column of L.
          for (i0 = i; i0 <= cc1; i0++) {
            s = 0;
            if (i0 == i) {
              i2 = (i == c2 ? 0 : i - c1);
    ...
    // Moles transported by concentration gradient from cell [i] to [i + 1] go
    // in tot1,
    //        moles by stagnant exchange from cell [i] to [i + c1] go in
    //        tot_stag
    ...
    for (i = ifirst; i <= ilast + 1; i++) {
      ...
      for (i0 = stagnant; i0 >= 0; i0--) {
        ...
        grad = (Ct2[i1] -
                Ct2[i]) /* * ct[i].v_m[cp].D*/; // .D has the d{lg(gamma)} /
                                                // d{lg(molal)} correction
        if (!i0) {
          ...
          if (cell_iter == cell_J_ij.end())
            continue;
          std::map<std::string, class J_ij_save>::iterator s_iter =
              cell_iter->second.find(sol_D[1].spec[cp].name);
    ...
    for (int k = 0; k < count_elts; k++) {
      if (!strcmp(elt_list[k].elt->name, "X"))
        continue;
  ...
  for (i = ifirst; i <= ilast; i++) {
    ...
    if (dV_dcell && !fix_current) {
      ...
      if (i > 1)
        sum_Rd +=
            (current_cells[0].dif - current_cells[i].dif) * current_cells[i].R;
    }
  }
  if (dV_dcell || fix_current) {
    if (fix_current) {
  ...
  for (i = ifirst + 1; i <= ilast; i++) {
    ...
    if ((dV_dcell && (dVc * j_0e > 0)) ||
        ((dV_dcell > 0) &&
         ((cell_data[i].potV + dVc) > cell_data[count_cells + 1].potV)) ||
        ((dV_dcell < 0) &&
         ((cell_data[i].potV + dVc) < cell_data[count_cells + 1].potV))) {
      dVc = (cell_data[count_cells + 1].potV - cell_data[i].potV) /
            ((double)count_cells + 1 - (double)i);
    }
  ...
  for (cp = 0; cp < comp; cp++) {
    ...
    for (i = ifirst; i <= ilast + stagnant; i++) {
      ...
      if (stagnant && ct[i].Dz2c_stag) {
        ct[i].J_ij[cp].tot_stag +=
            (mixf_stag[i][cp] * ct[i].J_ij_sum / ct[i].Dz2c_stag);
      }
    }
  }
  ...
  for (i = ifirst; i <= ilast + stagnant + (bcon_last == 2 ? 1 : 0); i++) {
    ...
    if (ct[i].m_s == NULL) {
      ...
      ct[i].m_s = (class M_S *)PHRQ_realloc(ct[i].m_s, (count_m_s + 5) *
                                                           sizeof(class M_S));
      ct[i].m_s_size = count_m_s + 5;
    }
    if (ct[i].m_s == NULL)
  ...
  for (cp = 0; cp < count_m_s; cp++) {
    ...
    for (icell = if1; icell != il1; icell += incr) {
      ...
      if (!strcmp(ct[icell].m_s[cp].name, "H")) {
        ...
        if (sptr_stag) {
          ...
          if (icell == c) {
            ...
            if (dV_dcell || fix_current)
              sptr2->Set_total_h(sptr2->Get_total_h() -
                                 ct[icell + 1].m_s[cp].tot_stag);
          }
      ...
      if (!strcmp(ct[icell].m_s[cp].name, "O")) {
        ...
        if (sptr_stag) {
          ...
          if (icell == c) {
            dummy += ct[icell + 1].m_s[cp].tot_stag;
            if (dV_dcell || fix_current)
              sptr2->Set_total_o(sptr2->Get_total_o() -
                                 ct[icell + 1].m_s[cp].tot_stag);
          }
      ...
      // check for negative moles, add moles from other redox states and the
      // donnan layer when necessary and available...
      if (dum1 - ct[icell].m_s[cp].tot1 - ct[icell].m_s[cp].tot_stag <
              min_mol &&
        ...
        if (ct[icell].dl_s > 1e-8) {
          ...
          if (s_ptr) {
            dum1 += moles_from_donnan_layer(s_ptr, ct[icell].m_s[cp].name,
                                            ct[icell].m_s[cp].tot1 +
                                                ct[icell].m_s[cp].tot_stag -
                                                dum1 + min_mol);
          }
        }
      ...
      if (dum2 + ct[icell].m_s[cp].tot2 - dum < min_mol &&
        ...
        if (ct[icell + 1].dl_s > 1e-8) {
          ...
          if (s_ptr) {
            dum2 += moles_from_donnan_layer(
                s_ptr, ct[icell].m_s[cp].name,
                -(ct[icell].m_s[cp].tot2 + dum2 - min_mol));
          }
        }
      ...
      if (dV_dcell || fix_current || (icell > 0 && icell <= ilast)) {
        ...
        dum1 -= dum;
        sptr1->Get_totals()[ct[icell].m_s[cp].name] =
            (dum1 > 0 ? dum1 : min_mol);
        if (dum1 < 0) {
          dum += dum1 - min_mol;
          if ((it1 = neg_moles.find(icell)) != neg_moles.end() &&
              (it2 = (els = it1->second).find(ct[icell].m_s[cp].name)) !=
                  els.end())
            dum1 += it2->second;
          els.clear();
      ...
      if (dV_dcell || fix_current || (icell >= 0 && icell < ilast) ||
        ...
        if (stagnant && icell == c && (dV_dcell || fix_current))
          dum -= ct[c1].m_s[cp].tot_stag;
        dum2 += dum;
        sptr2->Get_totals()[ct[icell].m_s[cp].name] =
            (dum2 > 0 ? dum2 : min_mol);
        if (dum2 < 0) {
          dum -= dum2 - min_mol;
      ...
      if (sptr_stag) {
        ...
        if (dum_stag + dum < 0) {
          ...
          if (ct[i1].dl_s) {
            ...
            if (s_ptr) {
              dum_stag += moles_from_donnan_layer(s_ptr, ct[icell].m_s[cp].name,
        ...
        dum_stag += dum;
        sptr_stag->Get_totals()[ct[icell].m_s[cp].name] =
            (dum_stag > 0 ? dum_stag : min_mol);
        if (dum_stag < 0) {
          dum -= dum_stag - min_mol;
      ...
      // reduce oscillations in the column-boundary cells, but not for H and O,
      // and current_A is not adjusted...
      if ((dV_dcell /* || fix_current*/) && icell == il1 - incr &&
          dV_dcell * ct[0].m_s[cp].charge < 0 &&
          strcmp(ct[0].m_s[cp].name, "H") && strcmp(ct[0].m_s[cp].name, "O") &&
          c > 3 && mixrun > 1) {
        dummy = Utilities::Rxn_find(Rxn_solution_map, 0)
                    ->Get_totals()[ct[0].m_s[cp].name] /
                ct[0].kgw * (1 - ct[0].dl_s);
        if (dummy > 1e-6) {
          sptr1 = Utilities::Rxn_find(Rxn_solution_map, 1);
          sptr2 = Utilities::Rxn_find(Rxn_solution_map, 2);
          dum1 = sptr1->Get_totals()[ct[0].m_s[cp].name] / ct[1].kgw *
                     (1 - ct[1].dl_s) -
                 dummy;
          dum2 = sptr2->Get_totals()[ct[0].m_s[cp].name] / ct[2].kgw *
                     (1 - ct[2].dl_s) -
                 dummy;
          if (dum1 / dum2 < 0 || dum1 / dum2 > 1) {
            dum = cell_data[1].mid_cell_x / cell_data[2].mid_cell_x;
            // ct[1].J_ij_sum -= sptr1->Get_totals()[ct[0].m_s[cp].name] *
            // ct[1].m_s[cp].charge;
        ...
        dummy = Utilities::Rxn_find(Rxn_solution_map, c1)
                    ->Get_totals()[ct[0].m_s[cp].name] /
                ct[c1].kgw * (1 - ct[c1].dl_s);
        if (dummy > 1e-6) {
          sptr1 = Utilities::Rxn_find(Rxn_solution_map, c);
          sptr2 = Utilities::Rxn_find(Rxn_solution_map, c_1);
          dum1 = sptr1->Get_totals()[ct[0].m_s[cp].name] / ct[c].kgw *
                     (1 - ct[c].dl_s) -
                 dummy;
          dum2 = sptr2->Get_totals()[ct[0].m_s[cp].name] / ct[c_1].kgw *
                     (1 - ct[c_1].dl_s) -
                 dummy;
          if (dum1 / dum2 < 0 || dum1 / dum2 > 1) {
            dum = (cell_data[c].mid_cell_x - cell_data[c_1].mid_cell_x) /
                  (cell_data[c1].mid_cell_x - cell_data[c_1].mid_cell_x);
            // ct[c].J_ij_sum -= sptr1->Get_totals()[ct[0].m_s[cp].name] *
            // ct[c].m_s[cp].charge;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::moles_from_redox_states(cxxSolution *sptr, const char *name)
/* ---------------------------------------------------------------------- */
{
  int length, length2;
  cxxNameDouble::iterator kit;
  ...
  for (kit = sptr->Get_totals().begin(); kit != sptr->Get_totals().end();
       kit++) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::add_MCD_moles(LDBLE moles, LDBLE min_mol, int i,
                             cxxSolution *sptr, const char *name)
/* ---------------------------------------------------------------------- */
{
  cxxNameDouble::iterator kit;
  std::map<int, std::map<std::string, double>>::iterator it1;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::moles_from_donnan_layer(cxxSurface *sptr, const char *name,
                                       LDBLE moles_needed)
/* ---------------------------------------------------------------------- */
{
  cxxNameDouble::iterator kit;
  ...
  for (size_t j = 0; j < sptr->Get_surface_charges().size(); j++) {
    if (sptr->Get_dl_type() == cxxSurface::DONNAN_DL) {
      ...
      for (kit = charge_ptr->Get_diffuse_layer_totals().begin();
           kit != charge_ptr->Get_diffuse_layer_totals().end(); kit++) {
        if (strcmp(kit->first.c_str(), "H") == 0 ||
            strcmp(kit->first.c_str(), "O") == 0)
          continue;
        if (strcmp(kit->first.c_str(), name) == 0) {
          if (kit->second > moles_needed) {
            dum += moles_needed;
            kit->second -= moles_needed;
          } else {
            dum += kit->second;
            kit->second = 0;
          }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::multi_D(LDBLE DDt, int mobile_cell, int stagnant)
/* ---------------------------------------------------------------------- */
{
  /*
  * 1. determine mole transfer (mol/s) of solute species for the interface
  between 2 cells.
  ...
  * If an electrical field is applied (dV_dcell != 0), the currents j_i =
  current_cells[i].ele + dif (C * s) are calculated for all cells. Then the ele
  part from cell 0 -> 1 is calculated: current_x = (j_0d + j_0e) = j_0 = j_1 =
  ... = j_i j_0e * (R0 + R1 + ...) + (j_0d - j_1d) * R1 + ... + (j_0d - j_id) *
  Ri = Vtot or j_0e * Sum_R + Sum_Rd = Vtot. Ri = dV_dcell / j_ie, the relative
  cell resistance. Solve j_0e, find (V1 - V0) = j_0e * R0. j_1e = current_x -
  j_1d, find (V2 - V1) = j_1e * R1, etc.
  */
  ...
  if (dV_dcell)
    find_current = loop_f_c =
        1; // calculate J_ij once for all cells, loop to dV_dcell2.
  else
    find_current = loop_f_c = 0;

  for (int f_c = 0; f_c <= loop_f_c; f_c++) {
    for (n = 0; n <= (stagnant ? stag_data.count_stag : 0);
         n++) // allow for stagnant cell mixing with higher cells in the layer
    {
      ...
      for (i = first_c; i <= last_c; i++) {
        if (stagnant) {
          ...
          if (jcell >= all_cells || jcell < 0) {
            error_string = sformatf("Multi_D asked for diffusion from cell %d "
                                    "to %d, %d is beyond the number of cells",
                                    icell, jcell, jcell);
            error_msg(error_string, CONTINUE);
          }
          if (jcell > last_c2)
        ...
        if (find_current) {
          if (i < last_c)
            ...
            LDBLE dVc, j_0e;
            // distribute dV_dcell according to relative resistance, calculate
            // current_x if not fixed
            j_0e = (dV_dcell * count_cells - sum_Rd) / sum_R;
        ...
        if (!il_calcs) {
          ...
          count_m_s = (ct[icell].J_ij_count_spec < count_moles_added
                           ...
                           : count_moles_added);
          m_s = (class M_S *)PHRQ_malloc((size_t)count_m_s * sizeof(class M_S));
          if (m_s == NULL)
            malloc_error();
          for (i1 = 0; i1 < count_m_s; i1++) {
            m_s[i1].name = NULL;
        ...
        if (dV_dcell || (icell != 0 && icell != count_cells + 1)) {
          ...
          use.Get_solution_ptr()->Set_total_h(
              use.Get_solution_ptr()->Get_total_h() - tot1_h);
          use.Get_solution_ptr()->Set_total_o(
              use.Get_solution_ptr()->Get_total_o() - tot1_o);
          ...
          for (l = 0; l < count_m_s; l++) {
            ...
            for (it = use.Get_solution_ptr()->Get_totals().begin();
              ...
              if (strncmp(m_s[l].name, it->first.c_str(), length) == 0 &&
                  length == length2) {
      ...
      if (moles < 0 && ct[i].dl_s) {
        ...
        for (size_t j = 0; j < s_ptr->Get_surface_charges().size(); j++) {
          if (s_ptr->Get_dl_type() == cxxSurface::DONNAN_DL) {
            ...
            for (jit = charge_ptr->Get_diffuse_layer_totals().begin();
                 jit != charge_ptr->Get_diffuse_layer_totals().end(); jit++) {
              if (strcmp(jit->first.c_str(), "H") == 0 ||
                  strcmp(jit->first.c_str(), "O") == 0)
                continue;
      ...
      if (moles < 0) {
        ...
        if (temp < -1e-12) {
          ...
          for (i1 = 0; i1 < count_moles_added; i1++) {
            if (moles_added[i1].name &&
                !strcmp(moles_added[i1].name, it->first.c_str())) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::fill_m_s(class J_ij *l_J_ij, int l_J_ij_count_spec, int icell,
                      int stagnant)
/* ---------------------------------------------------------------------- */
{
      ...
      for (k = 0; k < count_elts; k++) {
        for (l = 0; l < count_m_s; l++) {
          if (strcmp(ct[icell].m_s[l].name, elt_list[k].elt->name) == 0) {
            fraction = fabs((double)elt_list[k].coef * l_J_ij[j].tot1) +
                       fabs(ct[icell].m_s[l].tot1);
            if (fraction)
              fraction =
                  fabs((double)elt_list[k].coef * l_J_ij[j].tot1) / fraction;
            else
              fraction = 1;
            ...
            if (stagnant)
              ct[icell].m_s[l].tot_stag +=
                  elt_list[k].coef * l_J_ij[j].tot_stag;
            break;
          }
        }
      }
...
/* ---------------------------------------------------------------------- */
void Phreeqc::calc_b_ij(int icell, int jcell, int k, LDBLE b_i, LDBLE b_j,
                        LDBLE g_i, LDBLE g_j, LDBLE free_i, LDBLE free_j,
                        int stagnant)
/* ---------------------------------------------------------------------- */
//{
//	ct[icell].v_m[k].b_ij = b_i * (free_i + g_i) * b_j * (free_j + g_j) /
//(b_i * (free_i + g_i) + b_j * (free_j + g_j));
...
//}
{
  ...
  LDBLE fg_i = (1 - free_i) * g_i, fg_j = (1 - free_j) * g_j;
  ct[icell].v_m[k].b_ij = b_i * (free_i + fg_i) * b_j * (free_j + fg_j) /
                          (b_i * (free_i + fg_i) + b_j * (free_j + fg_j));
  // At filterends and boundary cells, concentrations of ions change step-wise
  // to the DL. filter cells, harmonic mean for f_free, the average for the DL.
  ...
  if (stagnant > 1) { /* for a diffusion experiment with well-mixed reservoir in
                         cell 3 and the last stagnant cell, and with the mixf *
                         2 for the boundary cells in the input... */
    if (icell == 3 && !g_i && g_j)
  ...
  // regular column...
    ...
    if (icell == 0 ||
        (icell == count_cells + 1 && jcell == count_cells + count_cells + 1))
  ...
  if (ct[icell].v_m[k].z)
    ct[icell].Dz2c +=
        ct[icell].v_m[k].b_ij * ct[icell].v_m[k].zc * ct[icell].v_m[k].z;
  return;
}

/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::find_J(int icell, int jcell, LDBLE mixf, LDBLE DDt, int stagnant)
/* ---------------------------------------------------------------------- */
{
  /* mole transfer of the individual master_species:
  * Eqn 1:
  * J_ij = DDt * (A_ij / lav) * (-D_i*grad(c) + D_i*z_i*c_i *
  SUM(D_i*z_i*grad(c)) / SUM(D_i*(z_i)^2*c_i))
  ...
  ** dec. 28, 2015**
  ...
  J_ij = - b_i * b_j / (b_i + b_j) * (c_j - c_i) in mol/s (see ex 21 in the
  manual 3). b_i = A1 / (G_i * h_i / 2) * Dw for a pore without EDL. A1 = aq1 /
  h_i (m^2). with EDL (no aq_il_i in A1, for now): t_aq1 = aq1 + aq_dl_i. A1 =
  t_aq1 / h_i. f_free_i = aq1 / t_aq1. b_i_cat = A1 / (G_i * h_i / 2) * Dw *
  {f_free + (1 - f_free) * Bm}. Bm  = Boltzmann enrichment in EDL = g_dl.
  b_i_ani = A1 / (G_i * h_i / 2) * Dw * {f_free + (1 - f_free) / Bm)}.
  22/2/18: now calculates diffusion through EDL's of multiple, differently
  charged surfaces
  *  stagnant TRUE:
  *    same eqn for J_ij, but multiplies with 2 * mixf. (times 2, because mixf =
  A / (G_i * h_i))
  *    mixf_ij = mixf / (Dw / init_tort_f) / new_tort_f * new_por / init_por
  *    mixf is defined in MIX; Dw is default multicomponent diffusion
  coefficient;
  *    init_tort_f equals multi_Dpor^(-multi_Dn); new_pf = new tortuosity
  factor.
  * Interlayer diffusion (IL) takes the gradient in the equivalent
  concentrations on X-. surface area A for IL: stagnant: ct[icell].mixf_il is
  mixf * por_il / por. por_il = interlayer porosity, from -interlayer_D true
  'por_il'. por = free + DL porewater porosity, from -multi_D true 'multi_Dpor'.
  ...
  IL water is related to X-, thus the cec (eq/L IL water) is the same for all
  cells if X is defined. IL-water = (free + DL porewater) * por_il / por. for
  IL: A * aq_il / t_aq.
  */
  ...
  if (!il_calcs) {
    if (stagnant) {
      ...
      if ((icell == 0 && cell_data[1].por < multi_Dpor_lim) ||
          (icell == count_cells &&
           cell_data[count_cells].por < multi_Dpor_lim) ||
          (icell != 0 && icell != count_cells &&
           (cell_data[icell].por < multi_Dpor_lim ||
            cell_data[jcell].por < multi_Dpor_lim))) {
        if (dV_dcell) {
  ...
  /* do the calcs */
  aq1 = ct[icell].kgw =
      Utilities::Rxn_find(Rxn_solution_map, icell)->Get_mass_water();
  aq2 = ct[jcell].kgw =
      Utilities::Rxn_find(Rxn_solution_map, jcell)->Get_mass_water();
  /*
    ...
    if (s_ptr1->Get_dl_type() != cxxSurface::NO_DL) {
      ...
      if (print_viscosity) {
        if (s_ptr1->Get_calc_viscosity()) {
          ...
          ct[icell].visc1 =
              viscos_0 / s_ptr1->Get_DDL_viscosity(); // ratio for visc1^a_v_dif
        } else {
          viscos_0 =
              Utilities::Rxn_find(Rxn_solution_map, icell)->Get_viscos_0();
          ct[icell].visc1 =
              viscos_0 /
              (s_ptr1->Get_DDL_viscosity() *
               Utilities::Rxn_find(Rxn_solution_map, icell)->Get_viscosity());
        }
      }
      /* find the immobile surface charges with DL... */
      s_charge_p.assign(s_ptr1->Get_surface_charges().begin(),
                        s_ptr1->Get_surface_charges().end());
      s_com_p.assign(s_ptr1->Get_surface_comps().begin(),
                     s_ptr1->Get_surface_comps().end());

      for (i = 0; i < (int)s_charge_p.size(); i++) {
        for (i1 = 0; i1 < (int)s_com_p.size(); i1++) {
          if (!(s_charge_p[i].Get_name().compare(
                  s_com_p[i1].Get_charge_name())) &&
              !s_com_p[i1].Get_Dw()) {
    ...
    if (s_ptr2->Get_dl_type() != cxxSurface::NO_DL) {
      ...
      if (print_viscosity) {
        if (s_ptr2->Get_calc_viscosity()) {
          ...
          viscos_0 =
              Utilities::Rxn_find(Rxn_solution_map, jcell)->Get_viscos_0();
          ct[icell].visc2 =
              viscos_0 /
      ...
      s_charge_p.assign(s_ptr2->Get_surface_charges().begin(),
                        s_ptr2->Get_surface_charges().end());
      s_com_p.assign(s_ptr2->Get_surface_comps().begin(),
                     s_ptr2->Get_surface_comps().end());
      for (i = 0; i < (int)s_charge_p.size(); i++) {
        for (i1 = 0; i1 < (int)s_com_p.size(); i1++) {
          if (!(s_charge_p[i].Get_name().compare(
                  s_com_p[i1].Get_charge_name())) &&
  ...
  if (il_calcs) {
    ...
    make it relative to exchange capacity (mol X/L), highest X in sol_D[1].x_max
    (mol X / L). Find amounts of IL water and cec. */
    por_il1 = por_il2 = por_il12 = 0.0;
    cec1 = cec2 = cec12 = rc1 = rc2 = 0.0;
    if (icell == 0) {
      por_il1 =
          sol_D[0].exch_total / aq1 / sol_D[1].x_max * cell_data[1].por_il;
      por_il2 =
          sol_D[1].exch_total / aq2 / sol_D[1].x_max * cell_data[1].por_il;
      ...
      por_il2 = sol_D[count_cells + 1].exch_total / aq2 / sol_D[1].x_max *
                cell_data[count_cells].por_il;
      if (sol_D[count_cells].exch_total > 3e-10 &&
          sol_D[count_cells + 1].exch_total > 3e-10)
        por_il12 = 2 * por_il1 * por_il2 / (por_il1 + por_il2);
      else
        por_il12 = (por_il1 >= por_il2 ? por_il1 : por_il2);
    ...
    if (por_il12 < interlayer_Dpor_lim)
      ...
      // the rc's are for distributing the mole transfer (later on) over X and
      // solution.
      ...
      if (sol_D[jcell].exch_total > 3e-10)
        ...
        cec2 = 2e-10;
      /* and the largest for calculating the mass transfer since it is usually
       * at the boundary... */
  ...
  if (icell == 0) {
    ...
    if (stagnant)
      ...
      if (il_calcs && !stagnant)
        ct[icell].A_ij_il =
            A2 * por_il12 / (cell_data[1].por * interlayer_tortf);
      A2 /= tort2;
    }
    A1 = A2;
  } else if (icell == count_cells) {
    ...
    if (stagnant)
      ...
      A1 = t_aq1 / (cell_data[count_cells].length * 0.5 *
                    cell_data[count_cells].length);
      if (il_calcs && !stagnant)
        ct[icell].A_ij_il =
            A1 * por_il12 / (cell_data[count_cells].por * interlayer_tortf);
      A1 /= tort1;
    }
    A2 = A1;
  } else {
  ...
  if (ct[icell].J_ij == NULL) {
    ct[icell].J_ij =
        (class J_ij *)PHRQ_malloc((size_t)(k) * sizeof(class J_ij));
    ct[icell].J_ij_size = k;
  } else if (k > ct[icell].J_ij_size) {
    ct[icell].J_ij = (class J_ij *)PHRQ_realloc(
        ct[icell].J_ij, (size_t)(k) * sizeof(class J_ij));
    ct[icell].J_ij_size = k;
  }
  if (ct[icell].J_ij == NULL)
    malloc_error();
  ...
  if (ct[icell].v_m == NULL) {
    ...
    ct[icell].v_m = (struct V_M *)PHRQ_realloc(
        ct[icell].v_m, (size_t)(k) * sizeof(struct V_M));
    ct[icell].v_m_size = k;
  }
  if (ct[icell].v_m == NULL)
    malloc_error();
  ...
  if (il_calcs) {
    ...
    ct[icell].v_m_il =
        (struct V_M *)PHRQ_malloc((size_t)k * sizeof(struct V_M));
  ...
  while (i < i_max || j < j_max) {
    if (j == j_max || (i < i_max && strcmp(sol_D[icell].spec[i].name,
      ...
      if (il_calcs && sol_D[icell].spec[i].type == EX) {
        ...
        ct[icell].v_m_il[k_il].D =
            sol_D[icell]
                .spec[i]
                .Dwt; // .Dwt = dw * tk_corr * (visc_0_25/visc_0)
        ct[icell].v_m_il[k_il].z = sol_D[icell].spec[i].z;
        ct[icell].v_m_il[k_il].Dz =
            ct[icell].v_m_il[k_il].D * ct[icell].v_m_il[k_il].z;
        dum = sol_D[icell].spec[i].c * cec12 / (2 * ct[icell].v_m_il[k_il].z);
        ct[icell].v_m_il[k_il].Dzc = ct[icell].v_m_il[k_il].Dz * dum;
        ct[icell].Dz2c_il +=
            ct[icell].v_m_il[k_il].Dzc * ct[icell].v_m_il[k_il].z;
        ct[icell].v_m_il[k_il].grad =
            -sol_D[icell].spec[i].c * cec12 /
            ct[icell].v_m_il[k_il].z; /* use equivalent fraction */
        k_il++;
      } else {
        ct[icell].J_ij[k].name = sol_D[icell].spec[i].name;
        ct[icell].v_m[k].z = sol_D[icell].spec[i].z;
        ct[icell].v_m[k].grad =
            -sol_D[icell].spec[i].c; /* assume d log(gamma) / d log(c) = 0 */
        ...
        if (dV_dcell && ct[icell].v_m[k].z && !fix_current) {
          ...
          if (icell == 0)
            dum2 = (cell_data[1].potV - cell_data[0].potV) /
                   (cell_data[1].length / 2);
          else if (icell == count_cells)
            dum2 = (cell_data[count_cells + 1].potV -
                    cell_data[count_cells].potV) /
                   (cell_data[count_cells].length / 2);
          else
            dum2 = (cell_data[jcell].potV - cell_data[icell].potV) /
                   ((cell_data[jcell].length + cell_data[icell].length) / 2);
          dum2 *= F_Re3 / tk_x2 * ct[icell].v_m[k].z * c1;
          if (dum + dum2 > 0) {
            // step out: no transport against the dV_dcell gradient if c = 0 in
            // jcell...
            if (i < i_max)
              i++;
            continue;
        ...
        if (ct[icell].dl_s > 0) {
          if (dl_aq1) {
            for (it_sc = s_charge_p1.begin(); it_sc != s_charge_p1.end();
                 it_sc++) {
              g_i += it_sc->Get_z_gMCD_map()[ct[icell].v_m[k].z];
            }
            g_i *= sol_D[icell].spec[i].erm_ddl;
            if (sol_D[icell].spec[i].dw_a_v_dif)
              g_i *= pow(ct[icell].visc1,
                         sol_D[icell]
                             .spec[i]
                             .dw_a_v_dif); // .visc1 = viscos_0 / DDL_visc
          }
          if (dl_aq2) {
            for (it_sc = s_charge_p2.begin(); it_sc != s_charge_p2.end();
                 it_sc++) {
        ...
        if (sol_D[icell].tk_x == sol_D[jcell].tk_x)
          ...
          if (sol_D[icell].spec[i].dw_t)
            dum2 *= exp(sol_D[icell].spec[i].dw_t / sol_D[jcell].tk_x -
                        sol_D[icell].spec[i].dw_t / 298.15);
          if (sol_D[icell].spec[i].dw_a_v_dif)
            dum2 *= pow(sol_D[jcell].viscos_0 / sol_D[jcell].viscos,
      ...
      if (i < i_max)
        i++;
             ...
             (j < j_max && strcmp(sol_D[icell].spec[i].name,
                                  sol_D[jcell].spec[j].name) > 0)) {
      ...
      if (il_calcs && sol_D[jcell].spec[j].type == EX) {
        ...
        ct[icell].v_m_il[k_il].Dz =
            ct[icell].v_m_il[k_il].D * ct[icell].v_m_il[k_il].z;
        ct[icell].v_m_il[k_il].Dzc = ct[icell].v_m_il[k_il].Dz *
                                     sol_D[jcell].spec[j].c * cec12 /
                                     (2 * ct[icell].v_m_il[k_il].z);
        ct[icell].Dz2c_il +=
            ct[icell].v_m_il[k_il].Dzc * ct[icell].v_m_il[k_il].z;
        ct[icell].v_m_il[k_il].grad =
            sol_D[jcell].spec[j].c * cec12 /
        ...
        ct[icell].v_m[k].grad =
            sol_D[jcell].spec[j].c; /* assume d log(gamma) / d log(c) = 0 */
        ...
        if (sol_D[icell].tk_x == sol_D[jcell].tk_x)
          ...
          if (sol_D[jcell].spec[j].dw_t)
            dum2 *= exp(sol_D[jcell].spec[j].dw_t / sol_D[icell].tk_x -
                        sol_D[jcell].spec[j].dw_t / 298.15);
          if (sol_D[jcell].spec[j].dw_a_v_dif)
            dum2 *= pow(sol_D[icell].viscos_0 / sol_D[icell].viscos,
                        sol_D[jcell].spec[j].dw_a_v_dif);
      ...
      if (il_calcs && sol_D[icell].spec[i].type == EX) {
        ...
        ct[icell].v_m_il[k_il].Dzc =
            ct[icell].v_m_il[k_il].Dz *
            (sol_D[icell].spec[i].c + sol_D[jcell].spec[j].c) * cec12 /
            ct[icell].v_m_il[k_il].z;
        ct[icell].Dz2c_il +=
            ct[icell].v_m_il[k_il].Dzc * ct[icell].v_m_il[k_il].z;
        ct[icell].v_m_il[k_il].grad =
            (sol_D[jcell].spec[j].c - sol_D[icell].spec[i].c) * cec12 /
            ct[icell].v_m_il[k_il].z; /* use equivalent fraction */
        k_il++;
      } else {
        ct[icell].J_ij[k].name = sol_D[icell].spec[i].name;
  ...
  if (dV_dcell) {
    ...
    for (i = 0; i < ct[icell].J_ij_count_spec; i++) {
      if (!ct[icell].v_m[i].z)
        continue;
      current_cells[icell].ele -=
          ct[icell].v_m[i].b_ij * ct[icell].v_m[i].z * ct[icell].v_m[i].zc;
      current_cells[icell].dif -=
          ct[icell].v_m[i].b_ij * ct[icell].v_m[i].z * ct[icell].v_m[i].grad;
    }
    current_cells[icell].R = tk_x2 / (current_cells[icell].ele * F_Re3);
    sum_R += current_cells[icell].R;
    sum_Rd += (current_cells[0].dif - current_cells[icell].dif) *
              current_cells[icell].R;
    return (il_calcs);
  }
...
dV_dcell2:
  ...
  // assure that icell and jcell have dl water when checking negative conc's in
  // MCD
  ...
  if (dV_dcell) {
    ...
    for (i = 0; i < ct[icell].J_ij_count_spec; i++) {
      if (!ct[icell].v_m[i].z)
        continue;
      ct[icell].J_ij[i].tot1 -=
          ct[icell].v_m[i].b_ij * ct[icell].v_m[i].zc * dum * DDt;
      ct[icell].J_ij[i].tot2 = ct[icell].J_ij[i].tot1;
      // ct[icell].J_ij_sum += ct[icell].v_m[i].z * ct[icell].J_ij[i].tot1;
    }
  ...
  if (il_calcs && ct[icell].Dz2c_il != 0 && ct[icell].J_ij_il_count_spec > 0) {
    ...
    for (i = 0; i < i_max; i++) {
      ct[icell].J_ij_il[i].tot1 =
          -ct[icell].v_m_il[i].D * ct[icell].v_m_il[i].grad +
      ...
      // ct[icell].J_ij_sum += ct[icell].v_m_il[i].z *
      // ct[icell].J_ij_il[i].tot1;
      ct[icell].J_ij_il[i].tot2 = ct[icell].J_ij_il[i].tot1;
      // ct[icell].J_ij_il[i].charge = ct[icell].v_m_il[i].z; // appt not used
      // now
    }
    ...
    m_s = (class M_S *)free_check_null(m_s);
    m_s =
        (class M_S *)PHRQ_malloc((size_t)count_moles_added * sizeof(class M_S));
    if (m_s == NULL)
      malloc_error();
    for (i1 = 0; i1 < count_moles_added; i1++) {
      m_s[i1].charge = 0;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::disp_surf(LDBLE DDt)
/* ---------------------------------------------------------------------- */
/*
...
*  Implementation:
*  Mobile surface comps and charges are mixed face by face and 1 by 1 in
surface[n1]: Step (from cell 1 to count_cells + 1):
*  1. Surface n1 is made a copy of cell[i1]'s surface, if it exists, or else
*       b. a copy of the first encountered mobile surface[n2] from cell i2 = i1
- 1.
*  2  a. Column surfaces are mixed by dispersion/diffusion
*       b. Column surfaces are mixed by MCD (if multi_Dflag is true)
*  3. Surfaces n1 and n2 are stored in a temporary surface, with n_user = i1 or
i2.
*  4. When done for all cells, new surfaces are copied into the cells.
*       NOTE... The surfaces' diffuse_layer, edl and phases/kinetics relations
must be identical, *		       but only mobile surface_comp's (Dw > 0)
and their surface_charge's are transported.
*/
{
  ...
  LDBLE viscos_f0;
  /*
   * temperature and viscosity correction for MCD coefficient, D_T = D_298 *
   * viscos_298 / viscos
   */
  ...
  for (i1 = 1; i1 <= count_cells + 1; i1++) {
    ...
    if (surf1 || surf2) {
      /*
       * step 2a. Dispersive mixing of mobile surfaces from column cells i2 <
       * i1...
       */
      if (i1 <= count_cells)
      ...
      /* step 1b. If cell i1 has no surface, get the mobile comps from surface
       * i2... */
      if (surface_n1.Get_n_user() == -99) {
        ...
        if (surface_n1.Get_surface_charges().size() > 1) {
          std::string charge_name =
              surface_n1.Get_surface_charges()[0].Get_name();
          surface_n1 =
              sum_surface_comp(&surface_n1, 0, &surface_n1, charge_name, 1,
        ...
        /* if not found... */
          ...
          if (surface_ptr2 != NULL) {
            ...
            if (surface_n2.Get_surface_charges().size() > 1) {
              std::string charge_name =
                  surface_n2.Get_surface_charges()[0].Get_name();
              surface_n2 =
                  sum_surface_comp(&surface_n2, 0, &surface_n2, charge_name, 1,
      ...
      if (i1 == 1) {
        ...
        A_ij = Utilities::Rxn_find(Rxn_solution_map, 1)->Get_mass_water() /
               cell_data[1].length;
      } else if (i1 == count_cells + 1) {
        por = cell_data[count_cells].por;
        lav = cell_data[count_cells].length / 2;
        A_ij = Utilities::Rxn_find(Rxn_solution_map, count_cells)
                   ->Get_mass_water() /
               cell_data[count_cells].length;
      } else {
        por = cell_data[i2].por;
        lav = (cell_data[i1].length + cell_data[i2].length) / 2;
        A_ij = Utilities::Rxn_find(Rxn_solution_map, i1)->Get_mass_water() /
               (cell_data[i1].length * cell_data[i1].por);
        A_ij += Utilities::Rxn_find(Rxn_solution_map, i2)->Get_mass_water() /
                (cell_data[i2].length * cell_data[i2].por);
        A_ij /= 2;
        A_ij *= (cell_data[i1].por < cell_data[i2].por ? cell_data[i1].por
                                                       : cell_data[i2].por);
      }
      ...
      if (surf2) {
        for (k = 0; k < (int)surface_ptr2->Get_surface_comps().size(); k++) {
          ...
          charge_done = FALSE;
          for (k1 = 0; k1 < k; k1++) {
            cxxSurfaceComp *comp_k1_ptr =
                &(surface_ptr2->Get_surface_comps()[k1]);
          ...
          if (multi_Dflag) {
            ...
            if (surface_ptr1 != NULL && surface_ptr1->Get_transport()) {
              for (k1 = 0; k1 < (int)surface_ptr1->Get_surface_comps().size();
                   k1++) {
                cxxSurfaceComp *comp_k1_ptr =
                    &(surface_ptr1->Get_surface_comps()[k1]);
                if (strcmp(comp_k1_ptr->Get_formula().c_str(),
                           comp_k_ptr->Get_formula().c_str()) != 0)
                  continue;
                Dp1 = comp_k1_ptr->Get_Dw() * pow(cell_data[i1].por, multi_Dn);
                break;
              }
            }
            if (Dp1 > 0)
          ...
          if (i1 <= count_cells) {
            surface_n1 = sum_surface_comp(
                &surface_n1, f1, surface_ptr2, charge_name, mixf,
                surface_ptr2->Get_surface_comps()[k].Get_Dw());
            f1 = 1;
          }
          if (i2 > 0) {
            surface_n2 = sum_surface_comp(
                &surface_n2, f2, surface_ptr2, charge_name, -mixf,
                surface_ptr2->Get_surface_comps()[k].Get_Dw());
            f2 = 1;
          }
      ...
      if (surf1) {
        for (k = 0; k < (int)surface_ptr1->Get_surface_comps().size(); k++) {
          ...
          if (multi_Dflag) {
            if (i1 <= count_cells) {
              Dp1 = comp_k_ptr->Get_Dw() * pow(cell_data[i1].por, multi_Dn);
            } else {
              Dp1 = comp_k_ptr->Get_Dw() * pow(por, multi_Dn);
            }
            Dp2 = 0;
            if (surface_ptr2 != NULL && surface_ptr2->Get_transport()) {
              for (k1 = 0; k1 < (int)surface_ptr2->Get_surface_comps().size();
                   k1++) {
                cxxSurfaceComp *comp_k1_ptr =
                    &(surface_ptr2->Get_surface_comps()[k1]);
                if (strcmp(comp_k1_ptr->Get_formula().c_str(),
  ...
  /*
   * Step 4. Dispersion/diffusion is done. New surfaces can be copied in the
   * cell's surface...
   */
  // n2 = 0;
...
/* ---------------------------------------------------------------------- */
cxxSurface
Phreeqc::sum_surface_comp(cxxSurface *source1, LDBLE f1, cxxSurface *source2,
                          std::string charge_name, LDBLE f2, LDBLE new_Dw)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Takes fraction f1 of the 1st surface, adds fraction f2 of the 2nd
   * surface's comps[k] and its charge. The result goes in target
   */
  int new_n_user;
  cxxSurface *surface_ptr1, *surface_ptr2;
  std::string token;
  ...
  for (std::vector<cxxSurfaceComp>::iterator it =
           surface_ptr2->Get_surface_comps().begin();
       it != surface_ptr2->Get_surface_comps().end(); it++) {
    if (it->Get_charge_name() == charge_name) {
      addee.Get_surface_comps().push_back(*it);
    }
  }
  for (std::vector<cxxSurfaceCharge>::iterator it =
           surface_ptr2->Get_surface_charges().begin();
...
/* ---------------------------------------------------------------------- */
int Phreeqc::check_surfaces(cxxSurface *surface_ptr1, cxxSurface *surface_ptr2)
/* ---------------------------------------------------------------------- */
{
  ...
  if (surface_ptr1->Get_type() != surface_ptr2->Get_type()) {
    error_string = sformatf("Surfaces %d and %d differ in use of electrical "
                            "double layer. Cannot mix.",
                            n_user1, n_user2);
    error_msg(error_string, STOP);
    return_code = ERROR;
    input_error++;
  }
  if (surface_ptr1->Get_only_counter_ions() !=
      surface_ptr2->Get_only_counter_ions()) {
    error_string = sformatf("Surfaces %d and %d differ in use of only counter "
                            "ions in the diffuse layer. Cannot mix.",
                            n_user1, n_user2);
    error_msg(error_string, STOP);
    return_code = ERROR;
    input_error++;
  ...
  if (surface_ptr1->Get_related_phases() !=
      surface_ptr2->Get_related_phases()) {
    error_string =
        sformatf("Surfaces %d and %d differ in use of related phases (sites "
                 "proportional to moles of an equilibrium phase). Cannot mix.",
                 n_user1, n_user2);
    error_msg(error_string, STOP);
    return_code = ERROR;
    input_error++;
  ...
  if (surface_ptr1->Get_related_rate() != surface_ptr2->Get_related_rate()) {
    error_string =
        sformatf("Surfaces %d and %d differ in use of related rate (sites "
                 "proportional to moles of a kinetic reactant). Cannot mix.",
                 n_user1, n_user2);
    error_msg(error_string, STOP);
    return_code = ERROR;
    input_error++;
...
/* ---------------------------------------------------------------------- */
cxxSurface Phreeqc::mobile_surface_copy(cxxSurface *surface_old_ptr,
...
/* ---------------------------------------------------------------------- */
{
   ...
   *   NOTE... when all comps are moved, the old surface is deleted and surfaces
   *are sorted again, which will modify pointers and surface numbers. User
   *number of new surface structure is n_user_new, structure is freed when
   *n_user_new is already defined
   */
  ...
  for (size_t i = 0; i < count_comps; i++) {
    ...
    if (comp_ref.Get_Dw() > 0) {
      ...
      cxxSurfaceCharge *charge_ptr =
          temp_surface.Find_charge(comp_ref.Get_charge_name());
      if (charge_ptr == NULL) {
        i2++;
        cxxSurfaceCharge *old_charge_ptr =
            surface_old_ptr->Find_charge(comp_ref.Get_charge_name());
  ...
  if (i1 > 0) {
    ...
    if (surf_ptr != NULL) {
      for (size_t k = 0; k < surf_ptr->Get_surface_comps().size(); k++) {
        ...
        if (charge_done)
          continue;
        temp_surface = sum_surface_comp(&temp_surface, 1, surf_ptr,
                                        comp_ref.Get_charge_name(), 1, 0);
      }
    }
    ...
    if ((size_t)i1 != count_comps) {
      ...
      for (size_t i = 0; i < count_comps; i++) {
        if (surface_old_ptr->Get_surface_comps()[i].Get_Dw() == 0) {
          ...
          cxxSurfaceCharge *charge_ptr =
              replace_old.Find_charge(comp_ref.Get_charge_name());
 ...
 *  for any cell in MCD, need only include the mixing factors for higher
 * numbered cells.
 */
{
  ...
  LDBLE viscos_f0;
  /*
   * temperature and viscosity correction for MCD coefficient, D_T = D_298 * Tk
   * * viscos_298 / (298 * viscos)
   */
  viscos_f0 = viscos_0_25 / viscos_0;
  ...
  for (ns = 0; ns < stag_data.count_stag; ns++) {
    ...
    for (size_t i3 = 0; i3 < num.size(); i3++) {
      ...
      if (multi_Dflag) {
        mixf_store *=
            (cell_data[i1].por <= cell_data[i2].por ? cell_data[i1].por
                                                    : cell_data[i2].por);
      ...
      if (surf2) {
        for (k = 0; k < (int)surface_ptr2->Get_surface_comps().size(); k++) {
          ...
          if (multi_Dflag) {
            Dp2 = comp_k_ptr->Get_Dw() * pow(cell_data[i2].por, multi_Dn) *
                  viscos_f0;
            Dp1 = 0;
            if (surf1) {
              for (k1 = 0; k1 < (int)surface_ptr1->Get_surface_comps().size();
          ...
          if (mixf > 0.99999999)
            mixf = 0.99999999;
          surface_n1 =
              sum_surface_comp(&surface_n1, f1, surface_ptr2, charge_name, mixf,
                               surface_ptr2->Get_surface_comps()[k].Get_Dw());
          f1 = 1;
      ...
      if (surf1) {
        for (k = 0; k < (int)surface_ptr1->Get_surface_comps().size(); k++) {
            ...
            if (surf2) {
              for (k1 = 0; k1 < (int)surface_ptr2->Get_surface_comps().size();
                ...
                if (strcmp(comp_k1_ptr->Get_formula().c_str(),
                           comp_k_ptr->Get_formula().c_str()) != 0)
                  continue;
                Dp2 = comp_k1_ptr->Get_Dw() * pow(cell_data[i2].por, multi_Dn) *
                      viscos_f0;
                break;
              }
            }
          ...
          if (mixf > 0.99999999)
            mixf = 0.99999999;
          surface_n2 =
              sum_surface_comp(&surface_n2, f2, surface_ptr1, charge_name, mixf,
                               surface_ptr1->Get_surface_comps()[k].Get_Dw());
          f2 = 1;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::reformat_surf(const char *comp_name, LDBLE fraction,
                           const char *new_comp_name, LDBLE new_Dw, int l_cell)
/* ---------------------------------------------------------------------- */
{
  cxxSurface *surface_ptr;
  ...
  for (size_t i = 0; i < temp_surface.Get_surface_comps().size(); i++) {
    ...
    if (charge_name == old_charge_name) {
      ...
      cxxNameDouble nd;
      cxxNameDouble::iterator it;
      for (it = comp.Get_totals().begin(); it != comp.Get_totals().end();
           it++) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::viscosity(cxxSurface *surf_ptr)
/* ---------------------------------------------------------------------- */
{
  if (surf_ptr && !surf_ptr->Get_calc_viscosity()) {
    for (int i = 0; i < (int)surf_ptr->Get_surface_charges().size(); i++)
      surf_ptr->Get_surface_charges()[i].Set_DDL_viscosity(
          surf_ptr->Get_DDL_viscosity());
  ...
  LDBLE H2[42] = {
      5.20094e-1,  2.22531e-1,  -2.81378e-1, 1.61913e-1,  -3.25372e-2,
      0,           0,           8.50895e-2,  9.99115e-1,  -9.06851e-1,
      2.57399e-1,  0,           0,           0,           -1.08374,
      1.88797,     -7.72479e-1, 0,           0,           0,
      0,           -2.89555e-1, 1.26613,     -4.89837e-1, 0,
      6.98452e-2,  0,           -4.35673e-3, 0,           0,
      -2.57040e-1, 0,           0,           8.72102e-3,  0,
      0,           1.20573e-1,  0,           0,           0,
      0,           -5.93264e-4};
  ...
  viscos = pow((LDBLE)10.,
               -(1.37023 * (tc_x - 20) + 0.000836 * (tc_x - 20) * (tc_x - 20)) /
                   (109 + tc_x));
  viscos_0_25 = 0.88862;
#endif
  // return viscos;
  ...
  B_i = b0 + b1*exp(b2 * tc), b0..2 in Jones_Dole[0..2], read in
  SOLUTION_SPECIES D_i = d1 * exp(d2 * tc), d1, 2 in Jones_Dole[3, 4] d3_i in
  Jones_Dole[5] Jones_Dole[6] contains the anion factor, 1 for Cl-, variable for
  other anions f_z = (z * z + |z|) / 2, the contribution of the ion to mu_x, if
  z = 0: f_z = mu_x / m_i f_I = variable, depends on d3_i > 1, or d3_i < 1. tc
  is limited to 200 C.


  A from Falkenhagen-Dole for a salt:
  A = 4.3787e-14 * TK**1.5 / (eps_r**0.5)* (z1 + z2)**-0.5 / (D1 * D2) * psi
  psi = (D1*z2 + D2*z1)/4 - z1*z2 * (D1-D2)**2 / ((D1*z1 + D2*z2)**0.5 + ((D1 +
  D2) * (z1 + z2))**0.5)**2 D1, z1 for the cation, D2, |z2| for the anion of the
  salt. We use the harmonic mean of the Dw's, and the arithmetic mean of the
  z's, both weighted by the equivalent concentration.
  */
  LDBLE D1, D2, z1, z2, m_plus, m_min, eq_plus, eq_min, eq_dw_plus, eq_dw_min,
      t1, t2, t3, fan = 1;
  LDBLE A, psi, Bc = 0, Dc = 0, Dw = 0.0, l_z, f_z, lm, V_an, m_an, V_Cl, tc,
                l_moles, l_water, l_mu_x, dw_t_visc;

  m_plus = m_min = eq_plus = eq_min = eq_dw_plus = eq_dw_min = V_an = m_an =
      V_Cl = 0;
  ...
  for (i1 = 0; i1 < i1_last; i1++) {
    Bc = Dc = Dw = m_plus = m_min = eq_plus = eq_min = eq_dw_plus = eq_dw_min =
        V_an = m_an = 0;
    if (surf_ptr) {
      z_g_map.clear();
      if (i1_last == 1 || i1 < i1_last - 1) {
        ...
        z_g_map.insert(s_charge_p.Get_z_gMCD_map().begin(),
                       s_charge_p.Get_z_gMCD_map().end());
        for (std::map<double, double>::iterator x = z_g_map.begin();
             x != z_g_map.end(); ++x)
    ...
    for (i = 0; i < (int)this->s_x.size(); i++) {
      ...
      if (s_x[i]->Jones_Dole[0] || s_x[i]->Jones_Dole[1] ||
          s_x[i]->Jones_Dole[3]) {
        dw_t_visc = 0;
        t1 = l_moles / l_water;
        if (t1 < 1e-9)
        ...
        if (l_z)
          ...
          f_z = l_mu_x / t1;
        // if data at tc's other than 25 are scarce, put the values found for 25
        // C in [7] and [8], optimize [1], [2], and [4]...
        ...
        if (s_x[i]->Jones_Dole[5] >= 1)
          ...
          t2 = -1;
        t3 = (s_x[i]->Jones_Dole[3] * exp(-s_x[i]->Jones_Dole[4] * tc)) * t1 *
             (pow(l_mu_x, s_x[i]->Jones_Dole[5]) * (1 + t2) +
              pow(t1 * f_z, s_x[i]->Jones_Dole[5])) /
             (2 + t2);
        if (t3 < -1e-5)
          t3 = 0;
        Dc += t3;
        if (!surf_ptr)
          s_x[i]->dw_t_visc = dw_t_visc + t3;
    ...
    if (m_plus && m_min && eq_dw_plus && eq_dw_min) {
      ...
      A = 4.3787e-14 * pow(tk_x, 1.5) /
          (sqrt(eps_r * (z1 + z2) / ((z1 > z2) ? z1 : z2)) * (D1 * D2)) * psi;
    } else
      A = 0;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_vm_Cl(void)
/* ---------------------------------------------------------------------- */
{
  /*
  *  Calculate molar volume of Cl- with a Redlich type eqn:
  Vm = Vm0(tc) + (Av / 2) * z^2 * I^0.5 + coef(tc) * I^(b4).
  *    Vm0(tc) is calc'd using supcrt parms, or from millero[0] + millero[1] *
  tc + millero[2] * tc^2
  *    for Av * z^2 * I^0.5, see Redlich and Meyer, Chem. Rev. 64, 221.
  Av is in (cm3/mol)(mol/kg)^-0.5, = DH_Av.
  If b_Av != 0, the extended DH formula is used: I^0.5 /(1 + b_Av * DH_B *
  I^0.5). DH_Av and DH_B are from calc_dielectrics(tc, pa). *	  coef(tc) =
  logk[vmi1] + logk[vmi2] / (TK - 228) + logk[vmi3] * (TK - 228).
  *    b4 = logk[vmi4], or
  *	  coef(tc) = millero[3] + millero[4] * tc + millero[5] * tc^2
  */
  LDBLE V_Cl = 0;
  LDBLE pb_s = 2600. + patm_x * 1.01325, TK_s = tc_x + 45.15,
        sqrt_mu = sqrt(mu_x);
  class species *s_ptr;
  ...
  if (s_ptr->logk[vma1]) {
    ...
    V_Cl = s_ptr->millero[0] +
           tc_x * (s_ptr->millero[1] + tc_x * s_ptr->millero[2]);
    if (s_ptr->z) {
      /* the ionic strength terms... */
      V_Cl += s_ptr->z * s_ptr->z * 0.5 * DH_Av * sqrt_mu +
              (s_ptr->millero[3] +
               tc_x * (s_ptr->millero[4] + tc_x * s_ptr->millero[5])) *
                  mu_x;
    }
  }
  return V_Cl;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::flux_mcd(const char *species_name, int option)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  double f = 0.0;
  if (state == TRANSPORT && multi_Dflag) {
    ...
    if (s_ptr != NULL && s_ptr->in != FALSE && s_ptr->type < EMINUS) {
      ...
      if (j_map_iter != cell_J_ij.end()) {
        std::map<std::string, class J_ij_save>::iterator j_save_iter =
            j_map_iter->second.find(species_name);
