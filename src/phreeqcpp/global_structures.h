...
#define MAX_PP_ASSEMBLAGE                                                      \
  10 /* default estimate of the number of phase assemblages */
#define MAX_ADD_EQUATIONS                                                      \
  20 /* maximum number of equations added together to reduce eqn to            \
                                        master species */
...
#define MAX_MASS_BALANCE                                                       \
  10 /* initial guess of number mass balance equations for a solution */
#define MAX_MASTER 50 /* default estimate of the number of master species */
#define MAX_ELTS                                                               \
  15 /* default estimate for maximum number of times elements occur in         \
                        an equation */
#define MAX_PHASES 500 /* initial guess of number of phases defined */
#define MAX_S                                                                  \
  500 /* default estimate for maximum number of species in aqueous model */
...
#define MAX_TRXN                                                               \
  16 /* default estimate for maximum number of components in an eqn */
#define MAX_UNKNOWNS                                                           \
  15             /* default estimate for maximum number of unknowns in model */
...
// #define MIN_LM -80.0			/* minimum log molality allowed before
// molality set to zero */ #define LOG_ZERO_MOLALITY -80	/* molalities <=
// LOG_ZERO_MOLALITY are considered equal to zero */ #define MIN_TOTAL 1e-60
// #define MIN_TOTAL_SS MIN_TOTAL/100
// #define MIN_RELATED_SURFACE MIN_TOTAL*100
// #define MIN_RELATED_LOG_ACTIVITY -60
#else
// #define MIN_LM -30.0			/* minimum log molality allowed before
// molality set to zero */ #define LOG_ZERO_MOLALITY -30	/* molalities <=
// LOG_ZERO_MOLALITY are considered equal to zero */ #define MIN_TOTAL 1e-25
// #define MIN_TOTAL_SS MIN_TOTAL/100
// #define MIN_RELATED_SURFACE MIN_TOTAL*100
...
typedef enum {
  ...
  T_A5,
  T_A6,
  delta_v, /* set in calc_delta_v: calculated molar volume-change of the
              reaction */
  ...
  vma2,
  vma3,
  vma4, /* read: a1..a4 from supcrt, see calc_vm */
  wref, /* from supcrt */
  b_Av, /* b in z^2 * A_v * log(1 + b * I^0.5) / (2 * b) */
  vmi1,
  vmi2,
  vmi3,
  vmi4, /* ionic strength terms: (i1 + i2/(TK - 228) + i3 * (TK - 228) ) * I^i4
         */
...
public:
  ~elt_list(){};
  elt_list() {  /* list of name and number of elements in an equation */
...
public:
  ~species(){};
  species() {            /* all data pertinent to an aqueous species */
    ...
    dw_t = 0; // correct Dw for temperature: Dw(TK) = Dw(298.15) * exp(dw_t / TK
              // - dw_t / 298.15)
    // parms for calc'ng SC = SC0 * exp(-dw_a * z * mu^0.5 / (1 + DH_B * dw_a2 *
    // mu^0.5) / (1 + mu^dw_a3)) with DHO: ka = DH_B * dw_a * (1 +
    // DD(V_apparent)^dw_a2 * sqrt_mu, dw_a3 is a switch, see calc_SC in PBasic
    dw_a = 0;
    dw_a2 = 0;
    dw_a3 = 0;
    dw_a_visc = 0;  // exponent in viscosity correction of SC
    dw_a_v_dif = 0; // exponent in viscosity correction of D, the diffusion
    ...
    for (size_t i = 0; i < MAX_LOG_K_INDICES; i++)
      logk[i] = 0;
    // 7 coefficients analytical expression for B, D, anion terms and pressure
    // in Jones_Dole viscosity eqn
    for (size_t i = 0; i < 10; i++)
      Jones_Dole[i] = 0;
    // regression coefficients to calculate temperature dependent phi_0and b_v
    // of Millero density model
    ...
    // CReaction rxn_s; // reaction converted to secondary and primary master
    // species CReaction rxn_x; // reaction to be used in model
    //  (1 + sum(g)) * moles
    tot_g_moles = 0;
    // sum(moles*g*Ws/Waq)
    tot_dh2o_moles = 0;
...
public:
  ~phase(){};
  phase() {         /* all data pertinent to a pure solid phase */
    name = NULL;    // name of species
    ...
    pr_aa_sum2 = 0;
    // delta_v[0] = [1] + [2]*T + [3]/T + [4]*log10(T) + [5]/T^2 + [6]*T^2 +
    // [7]*P
...
public:
  ~rxn_token_temp(){};
  rxn_token_temp() { // data for equations, aq. species or minerals
...
public:
  ...
  spec() {
    ...
    Dw = 0;
    // temperature and viscosity corrected free water diffusion coefficient,
    // m2/s
    Dwt = 0;
