...
#define IJth(A, i, j)                                                          \
  DENSE_ELEM(A, i - 1, j - 1) /* IJth numbers rows,cols 1..NEQ */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::calc_kinetic_reaction(cxxKinetics *kinetics_ptr, LDBLE time_step)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (size_t i = 0; i < kinetics_ptr->Get_kinetics_comps().size(); i++) {
    cxxKineticsComp *kinetics_comp_ptr =
        &(kinetics_ptr->Get_kinetics_comps()[i]);
    coef = 0.0;
    /*
    ...
    if (rate_ptr == NULL) {
      ...
      if (rate_ptr->new_def == TRUE) {
        if (basic_compile(rates[j].commands.c_str(), &rates[j].linebase,
                          &rates[j].varbase, &rates[j].loopbase) != 0) {
      ...
      if (basic_run(l_command, rates[j].linebase, rates[j].varbase,
                    rates[j].loopbase) != 0) {
        error_string = sformatf("Fatal Basic error in rate %s.",
                                kinetics_comp_ptr->Get_rate_name().c_str());
        error_msg(error_string, STOP);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::calc_final_kinetic_reaction(cxxKinetics *kinetics_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
   *	Go through kinetic components to
...
RESTART: // if limiting rates, jump to here
  ...
  for (size_t i = 0; i < kinetics_ptr->Get_kinetics_comps().size(); i++) {
    ...
    for (; it != kinetics_comp_ptr->Get_namecoef().end(); it++) {
      ...
      if (phase_ptr != NULL) {
        add_elt_list(phase_ptr->next_elt, coef * coef1);
      } else {
      ...
      for (size_t j = 0; j < exchange_ptr->Get_exchange_comps().size(); j++) {
        ...
        if (name.size() > 0) {
          if (strcmp_nocase(kinetics_comp_ptr->Get_rate_name().c_str(),
            ...
            std::string formula =
                exchange_ptr->Get_exchange_comps()[j].Get_formula().c_str();
            const char *ptr = formula.c_str();
            if (get_elts_in_species(
                    &ptr, -coef * exchange_ptr->Get_exchange_comps()[j]
                                      .Get_phase_proportion()) == ERROR) {
    ...
    if (use.Get_surface_ptr() != NULL &&
      ...
      for (size_t j = 0; j < use.Get_surface_ptr()->Get_surface_comps().size();
           j++) {
        cxxSurfaceComp *surface_comp_ptr =
            &(use.Get_surface_ptr()->Get_surface_comps()[j]);
        if (surface_comp_ptr->Get_rate_name().size() > 0) {
          if (strcmp_nocase(kinetics_comp_ptr->Get_rate_name().c_str(),
            ...
            if (0.9 * surface_comp_ptr->Get_phase_proportion() *
                ...
                MIN_RELATED_SURFACE) {
              master_ptr = master_bsearch(
                  surface_comp_ptr->Get_master_element().c_str());
              if (master_ptr != NULL) {
              ...
              if (get_elts_in_species(
                      &cptr,
                      -coef * surface_comp_ptr->Get_phase_proportion()) ==
                  ERROR) {
                error_string =
                    sformatf("Error in -formula: %s", temp_formula.c_str());
                error_msg(error_string, CONTINUE);
              }
            }
          }
    ...
    kinetics_ptr->Get_totals().add_extensive(
        kinetics_comp_ptr->Get_moles_of_reaction(), 1.0);
  }
  if (count > 2) {
#if !defined(R_SO)
...
/* ---------------------------------------------------------------------- */
int Phreeqc::rk_kinetics(int i, LDBLE kin_time, int use_mix, int nsaver,
                         LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
  /*
   ...
   *	calc_kinetic_reaction(.., ..) calculates moles of intermediate
   *derivatives; these are calc'd for the whole step h. calc_final_kinetic
   *reaction(..) translates moles to PHREEQC reaction.
   */
  int k, save_old;
  ...
  LDBLE b31 = 3. / 40., b32 = 9. / 40., b51 = -11. / 54., b53 = -70. / 27.,
  ...
  while (h_sum < kin_time) {
    ...
    if (step_bad > kinetics_ptr->Get_bad_step_max()) {
      error_string = sformatf("Bad RK steps > %d in cell %d. Please decrease "
                              "(time)step or increase -bad_step_max.",
  ...
  MOLES_TOO_LARGE:
    ...
    if (l_bad == TRUE) {
      ...
      if (use.Get_pp_assemblage_ptr() != NULL) {
        cxxPPassemblage *pp_assemblage_ptr = Utilities::Rxn_find(
            Rxn_pp_assemblage_map, use.Get_pp_assemblage_ptr()->Get_n_user());
        assert(pp_assemblage_ptr);
        pp_assemblage_save = new cxxPPassemblage(*pp_assemblage_ptr);
      }
      if (use.Get_ss_assemblage_ptr() != NULL) {
        cxxSSassemblage *ss_assemblage_ptr = Utilities::Rxn_find(
            Rxn_ss_assemblage_map, use.Get_ss_assemblage_ptr()->Get_n_user());
    ...
    if (pp_assemblage_save != NULL) {
      Rxn_pp_assemblage_map[pp_assemblage_save->Get_n_user()] =
          *pp_assemblage_save;
      use.Set_pp_assemblage_ptr(Utilities::Rxn_find(
          Rxn_pp_assemblage_map, pp_assemblage_save->Get_n_user()));
    }
    if (ss_assemblage_save != NULL) {
      Rxn_ss_assemblage_map[ss_assemblage_save->Get_n_user()] =
          *ss_assemblage_save;
    ...
    for (size_t j = 0; j < kinetics_ptr->Get_kinetics_comps().size(); j++) {
      ...
      kinetics_comp_ptr->Set_moles(b31 * rk_moles[j] + b32 * rk_moles[k + j]);
      /*
       * check for equal_rate ...
       */
      if (equal_rate &&
          fabs(rk_moles[j] - rk_moles[k + j]) > kinetics_comp_ptr->Get_tol()) {
        equal_rate = FALSE;
      }
    }
    if (moles_reduction > 1.0)
    ...
    if (kinetics_ptr->Get_rk() == 2 && equal_rate) {
      for (size_t j = 0; j < kinetics_ptr->Get_kinetics_comps().size(); j++) {
        cxxKineticsComp *kinetics_comp_ptr =
            &(kinetics_ptr->Get_kinetics_comps()[j]);
        kinetics_comp_ptr->Set_moles(0.3 * rk_moles[j] + 0.7 * rk_moles[k + j]);
      }
      calc_final_kinetic_reaction(kinetics_ptr);
      for (size_t j = 0; j < kinetics_ptr->Get_kinetics_comps().size(); j++) {
        cxxKineticsComp *kinetics_comp_ptr =
    ...
    {
      char str[MAX_LENGTH];
      snprintf(str, sizeof(str), "RK-steps: Bad%4d. OK%5d. Time %3d%%",
               step_bad, step_ok, (int)(100 * h_sum / kin_time));
      status(0, str, true);
    }
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::set_and_run_wrapper(int i, int use_mix, int use_kinetics,
                                 int nsaver, LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
...
restart:
  for (j = 0; j < max_try; j++) {
    if (j == 1) {
      ...
      error_string =
          sformatf("Trying smaller step size, pe step size %g, %g ... \n",
      ...
      ineq_tol /= 10.;
      error_string =
          sformatf("Trying reduced tolerance %g ...\n", (double)ineq_tol);
      warning_msg(error_string);
    } else if (j == 3) {
      itmax *= 2;
      ineq_tol *= 10.;
      error_string =
          sformatf("Trying increased tolerance %g ...\n", (double)ineq_tol);
      warning_msg(error_string);
    } else if (j == 4) {
      always_full_pitzer = TRUE;
      itmax *= 2;
      ...
      ineq_tol /= 10.;
      error_string =
          sformatf("Trying diagonal scaling and reduced tolerance %g ...\n",
                   (double)ineq_tol);
      warning_msg(error_string);
    } else if (j == 6) {
      if (pitzer_model == TRUE || sit_model == TRUE)
      ...
      error_string = sformatf("Trying scaling pure_phase columns %g ...\n",
      ...
      error_string =
          sformatf("Trying delay removal of equilibrium phases %g ...\n",
                   (double)equi_delay);
      warning_msg(error_string);
    }
      ...
      min_value *= 10;
      error_string =
          sformatf("Trying increased scaling %g ...\n", (double)min_value);
      warning_msg(error_string);
    } else if (j == 10) {
      if (pitzer_model == TRUE || sit_model == TRUE)
        continue;
      aqueous_only = 5;
      error_string =
          sformatf("Skipping optimize equations for first %d iterations ...\n",
                   aqueous_only);
      warning_msg(error_string);
    } else if (j == 11) {
      if (pitzer_model == TRUE || sit_model == TRUE)
    ...
    if (j > 0) {
      ...
      if (kinetics_save.get() != NULL) {
        ...
        use.Set_kinetics_ptr(
            Utilities::Rxn_find(Rxn_kinetics_map, kinetics_save->Get_n_user()));
      }
    }
    if (j == 14) {
  ...
  if (converge == FALSE && use.Get_kinetics_ptr() != NULL &&
    ...
    error_string = sformatf("Numerical method failed on all parameter "
                            "combinations, retrying integration, cell/soln %d",
  ...
  if (converge == FALSE) {
    ...
    sum_species();
    print_all();
    error_string = sformatf("Numerical method failed on all combinations of "
                            "convergence parameters, cell/soln/mix %d",
                            this->solution_number());
    error_msg(error_string, STOP);
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::set_and_run(int i, int use_mix, int use_kinetics, int nsaver,
                         LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (use.Get_surface_ptr() != NULL && dl_type_x != cxxSurface::NO_DL) {
    ...
    prep();
    k_temp(use.Get_solution_ptr()->Get_tc(),
           use.Get_solution_ptr()->Get_patm());
    set(FALSE);
    converge = model();
  }
  sum_species();
  ...
  if (use.Get_surface_ptr() != NULL && dl_type_x != cxxSurface::NO_DL &&
      use.Get_surface_ptr()->Get_calc_viscosity())
    use.Get_surface_ptr()->Set_DDL_viscosity(viscosity(use.Get_surface_ptr()));
  return (converge);
}

/* ---------------------------------------------------------------------- */
int Phreeqc::set_transport(int i, int use_mix, int use_kinetics, int nsaver)
/* ---------------------------------------------------------------------- */
{
  /*
   *   i			--user number for soln, reaction, etc.
   *   use_mix	  --integer flag
                                          state == TRANSPORT: DISP, STAG, NOMIX,
   MIX_BS state == REACTION: TRUE, FALSE
   *   use_kinetics --true or false flag to calculate kinetic reactions
   *   nsaver	   --user number to store solution
   */
  cell = i;
  ...
  if (use_mix == DISP) {
    ...
    if (use.Get_mix_ptr() != NULL) {
      ...
      if (use.Get_solution_ptr() == NULL) {
        error_string = sformatf("Solution %d not found, while searching mix "
                                "structure for solution %d.",
...
/* ---------------------------------------------------------------------- */
int Phreeqc::set_reaction(int i, int use_mix, int use_kinetics)
/* ---------------------------------------------------------------------- */
{
  /*
   *   i			--user number for soln, reaction, etc.
   ...
   *   Find surface
   */
  // if (use.Get_surface_in() && use.Get_kinetics_in() && use.Get_kinetics_ptr()
  // && !use.Get_kinetics_ptr()->Get_use_cvode() && reaction_step > 1)
  //{
  //	// use.Set_surface_ptr(Utilities::Rxn_find(Rxn_surface_map, i));
  //	// appt: we may come here with zero kinetic reaction, but surface may
  //have to keep DONNAN_DL
  // }
  // else
  //	dl_type_x = cxxSurface::NO_DL;
  ...
  if (use.Get_ss_assemblage_in() == TRUE) {
    use.Set_ss_assemblage_ptr(Utilities::Rxn_find(Rxn_ss_assemblage_map, i));
    if (use.Get_ss_assemblage_ptr() == NULL) {
      error_string = sformatf("Solid-solution Assemblage %d not found.", i);
      error_msg(error_string, STOP);
    }
  }
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::run_reactions(int i, LDBLE kin_time, int use_mix,
                           LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (kin_time <= 0 ||
    ...
    if (converge == MASS_BALANCE) {
      error_string = sformatf(
          "Negative concentration in solution %d. Stopping calculation.",
          cell_no);
      error_msg(error_string, STOP);
    }
    run_reactions_iterations += iterations;
    ...
    pr.all = FALSE;
    /*
     *   This condition makes output equal for incremental_reactions
     *TRUE/FALSE... (if (incremental_reactions == FALSE || reaction_step == 1)
     */
    store_get_equi_reactants(i, FALSE);
    if (!kinetics_ptr->Get_use_cvode()) {
      /* in case dispersivity is not wanted..
                              if (multi_Dflag)
                                      rk_kinetics(i, kin_time, NOMIX, nsaver,
         step_fraction); else
       */
      ...
      kinetics_y = N_VNew(n_reactions,
                          kinetics_machEnv); /* Allocate y, abstol vectors */
      if (kinetics_y == NULL)
        malloc_error();
      cvode_last_good_y = N_VNew(
          n_reactions, kinetics_machEnv); /* Allocate y, abstol vectors */
      if (cvode_last_good_y == NULL)
        malloc_error();
      cvode_prev_good_y = N_VNew(
          n_reactions, kinetics_machEnv); /* Allocate y, abstol vectors */
      ...
      /*
         iopt[SLDET] = TRUE;
         cvode_mem = CVodeMalloc(n_reactions, f, 0.0, y, BDF, NEWTON, SV,
         &reltol, abstol, NULL, NULL, TRUE, iopt, ropt, machEnv); cvode_mem =
         CVodeMalloc(n_reactions, f, 0.0, y, ADAMS, FUNCTIONAL, SV, &reltol,
         abstol, NULL, NULL, FALSE, iopt, ropt, machEnv); iopt[MXSTEP] is
         maximum number of steps that CVODE tries.
       */
      // iopt[SLDET] = TRUE; // appt
      iopt[MXSTEP] = kinetics_ptr->Get_cvode_steps();
      iopt[MAXORD] = kinetics_ptr->Get_cvode_order();
      kinetics_cvode_mem = CVodeMalloc(
          n_reactions, f, 0.0, kinetics_y, BDF, NEWTON, SV, &reltol,
          kinetics_abstol, this, NULL, TRUE, iopt, ropt, kinetics_machEnv);
    ...
    RESTART:
      while (flag != SUCCESS) {
        ...
        {
          error_string =
              sformatf("CV_ODE: Time: %8.2e s. Delta t: %8.2e s. Calls: %d.",
                       (double)(sum_t), (double)cvode_last_good_time, m_iter);
          status(0, error_string, true);
        }
        // if (state != TRANSPORT)
        //{
        //	error_string = sformatf(
        //		"CVode incomplete at cvode_steps %d. Cell: %d. Time:
        //%8.2e s. Cvode calls: %d, continuing...\n", 		(int)iopt[NST], cell_no,
        //(double)sum_t, m_iter + 1); 	warning_msg(error_string);
        // }
        ...
        //	(cvode_last_good_time < 1e-6 || cvode_last_good_time < 1e-6 *
        //tout)) // appt
        //{
        //	if (increase_tol < 3)
        //	{
        //		increase_tol += 1;
        //		for (size_t j = 0; j <
        //kinetics_ptr->Get_kinetics_comps().size(); j++)
        //		{
        //			cxxKineticsComp * kinetics_comp_ptr =
        //&(kinetics_ptr->Get_kinetics_comps()[j]); 			LDBLE tr =
        //kinetics_comp_ptr->Get_tol() * 10.0; 			kinetics_comp_ptr->Set_tol(tr);
        //			tr += 0;
        //		}
        //	}
        // }
        ...
        if (++m_iter >= kinetics_ptr->Get_bad_step_max()) {
          ...
          error_string = sformatf(
              ...
              "Please increase the maximum calls with -bad_step_max.",
      ...
      /*
         odeint(&ystart[-1], n_reactions, 0, kin_time,
         kinetics_ptr->comps[0].tol, kin_time/kinetics_ptr->step_divide, 0.0,
         &nok, &nbad, i, nsaver );
       */
      for (size_t j = 0; j < kinetics_ptr->Get_kinetics_comps().size(); j++) {
        cxxKineticsComp *kinetics_comp_ptr =
        ...
        kinetics_comp_ptr->Set_m(m_original[j] -
                                 kinetics_comp_ptr->Get_moles());
      ...
      if (use.Get_pp_assemblage_ptr() != NULL) {
        Rxn_pp_assemblage_map[cvode_pp_assemblage_save->Get_n_user()] =
            *cvode_pp_assemblage_save;
        use.Set_pp_assemblage_ptr(Utilities::Rxn_find(
            Rxn_pp_assemblage_map, cvode_pp_assemblage_save->Get_n_user()));
      }
      if (use.Get_ss_assemblage_ptr() != NULL) {
        Rxn_ss_assemblage_map[cvode_ss_assemblage_save->Get_n_user()] =
            *cvode_ss_assemblage_save;
        use.Set_ss_assemblage_ptr(Utilities::Rxn_find(
            Rxn_ss_assemblage_map, cvode_ss_assemblage_save->Get_n_user()));
      }
      calc_final_kinetic_reaction(kinetics_ptr);
      if (set_and_run_wrapper(i, NOMIX, TRUE, nsaver, 0) == MASS_BALANCE) {
      ...
      error_string =
          sformatf("CV_ODE: Final Delta t: %8.2e s. Calls: %d.             ",
                   (double)cvode_last_good_time, m_iter);
      status(0, error_string, true);
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (kinetics_machEnv != NULL)
    M_EnvFree_Serial(
        kinetics_machEnv); /* Free the machine environment memory */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::set_advection(int i, int use_mix, int use_kinetics, int nsaver)
/* ---------------------------------------------------------------------- */
{
  /*
   *   i			--user number for soln, reaction, etc.
...
/* ---------------------------------------------------------------------- */
int Phreeqc::store_get_equi_reactants(int l, int kin_end)
/* ---------------------------------------------------------------------- */
{
  int i, k;
  ...
  if (kin_end == FALSE) {
    ...
    /*
     *   This condition makes output equal for incremental_reactions
     *TRUE/FALSE... if (incremental_reactions == FALSE || reaction_step ==
     *count_total_steps)
     */
    x0_moles.clear();
  }
...
void Phreeqc::f(integertype N, realtype t, N_Vector y, N_Vector ydot,
                void *f_data) {
  int n_user;
  // LDBLE step_fraction;
  cxxKinetics *kinetics_ptr;
  ...
  /*      if (set_and_run(n_user, FALSE, TRUE, n_user, step_fraction) ==
   * MASS_BALANCE) { */
  if (pThis->use.Get_pp_assemblage_ptr() != NULL) {
    pThis
        ->Rxn_pp_assemblage_map[pThis->cvode_pp_assemblage_save->Get_n_user()] =
        *pThis->cvode_pp_assemblage_save;
    pThis->use.Set_pp_assemblage_ptr(
        Utilities::Rxn_find(pThis->Rxn_pp_assemblage_map,
                            pThis->cvode_pp_assemblage_save->Get_n_user()));
  }
  if (pThis->use.Get_ss_assemblage_ptr() != NULL) {
    pThis
        ->Rxn_ss_assemblage_map[pThis->cvode_ss_assemblage_save->Get_n_user()] =
        *pThis->cvode_ss_assemblage_save;
    pThis->use.Set_ss_assemblage_ptr(
        Utilities::Rxn_find(pThis->Rxn_ss_assemblage_map,
                            pThis->cvode_ss_assemblage_save->Get_n_user()));
  }

  if (pThis->set_and_run_wrapper(n_user, FALSE, TRUE, n_user, 0.0) ==
      MASS_BALANCE) {
...
static void Jac(integertype N, DenseMat J, RhsFn f, void *f_data, realtype t,
                                N_Vector y, N_Vector fy, N_Vector ewt, realtype
h, realtype uround, void *jac_data, long int *nfePtr, N_Vector vtemp1, N_Vector
vtemp2, N_Vector vtemp3);
*/
void Phreeqc::Jac(integertype N, DenseMat J, RhsFn f, void *f_data, realtype t,
  ...
  for (size_t i = 0; i < kinetics_ptr->Get_kinetics_comps().size(); i++) {
    cxxKineticsComp *kinetics_comp_i_ptr =
        &(kinetics_ptr->Get_kinetics_comps()[i]);
    ...
    while (pThis->cvode_error == TRUE) {
      del /= 10.;
      for (size_t j = 0; j < kinetics_ptr->Get_kinetics_comps().size(); j++) {
        cxxKineticsComp *kinetics_comp_j_ptr =
            &(kinetics_ptr->Get_kinetics_comps()[j]);
        /*
      ...
      pThis->calc_final_kinetic_reaction(kinetics_ptr);
      if (pThis->use.Get_pp_assemblage_ptr() != NULL) {
        pThis->Rxn_pp_assemblage_map[pThis->cvode_pp_assemblage_save
                                         ->Get_n_user()] =
            *pThis->cvode_pp_assemblage_save;
        pThis->use.Set_pp_assemblage_ptr(
            Utilities::Rxn_find(pThis->Rxn_pp_assemblage_map,
      ...
      if (pThis->set_and_run_wrapper(n_user, FALSE, TRUE, n_user,
                                     step_fraction) == MASS_BALANCE) {
        count_cvode_errors++;
        pThis->cvode_error = TRUE;
...
bool Phreeqc::cvode_update_reactants(int i, int nsaver, bool save_it) {
  ...
  if (save_it) {
    ...
    cxxPPassemblage *pp_assemblage_ptr =
        Utilities::Rxn_find(Rxn_pp_assemblage_map, nsaver);
    cxxSSassemblage *ss_assemblage_ptr =
        Utilities::Rxn_find(Rxn_ss_assemblage_map, nsaver);
    if (cvode_pp_assemblage_save != NULL) {
      delete cvode_pp_assemblage_save;
      cvode_pp_assemblage_save = new cxxPPassemblage(*pp_assemblage_ptr);
    }
  ...
  for (size_t j = 0; j < negative_rate.size(); j++) {
    ...
    for (size_t i = 0; i < kinetics_ptr->Get_kinetics_comps().size(); i++) {
      ...
      cxxNameDouble::iterator it =
          kinetics_comp_ptr->Get_moles_of_reaction().find(elt);
    ...
    if (negative_rates < 0.0) {
      ...
      // limiter_fraction = fabs((0.9*total(elt.c_str()) + positive_rates) /
      // negative_rates);
    } else {
      assert(false);
    }
    ...
    for (size_t i = 0; i < kinetics_ptr->Get_kinetics_comps().size(); i++) {
      ...
      if (it != kinetics_comp_ptr->Get_moles_of_reaction().end()) {
        if (it->second < 0.0) {
          kinetics_comp_ptr->Set_moles(kinetics_comp_ptr->Get_moles() *
                                       limiter_fraction);
        }
      }
    }
