...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::activity(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE a;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::activity_coefficient(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE g, dum = 0.0;
  ...
  s_ptr = s_search(species_name);
  if (s_ptr != NULL && s_ptr->in != FALSE &&
      ((s_ptr->type < EMINUS) || (s_ptr->type == EX) ||
       (s_ptr->type == SURF))) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::log_activity_coefficient(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE g, dum = 0.0;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::aqueous_vm(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE g;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::phase_vm(const char *phase_name)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::sa_declercq(double sa_type, double Sa, double d, double m,
                           double m0, double gfw)
/* ---------------------------------------------------------------------- */
{
  if (sa_type == 0) {
  ...
  // surface-area-calculation-Square
  {
    ...
    double St0 = mass0 * Sa; // total surface
    double a0 = pow(3.0 * V0 / (4.0 * piConstant),
                    1.0 / 3.0); // ((3*V0)/(4 * 3.14159265359))^(1/3)
    ...
    double mass = m * gfw;
    double V = mass / d;
    double a = pow(3.0 * V / (4.0 * piConstant),
                   1.0 / 3.0); //((3*V)/(4 * 3.14159265359))^(1/3)
    double St = 4.0 * piConstant * a * a * np;
    return St * RATS; // total current surface
  }
  error_string =
      sformatf("Unknown surface area type in SA_DECLERCQ %d.", (int)sa_type);
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::diff_c(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE Dw;
  ...
  if (!Falk) {
    for (i = 0; i < (int)this->s_x.size(); i++) {
      ...
      if ((l_z = fabs(s_x[i]->z)) == 0) {
        // l_z = 1; // only a 1st approximation for correct_Dw in electrical
        // field
        continue;
      } else {
        s_ptr = s_x[i];
        if (print_viscosity) {
          ...
          av = (s_ptr->dw_a_visc ? pow((viscos_0 / viscos), s_ptr->dw_a_visc)
                                 : 1);
          a3 = (s_ptr->dw_a3       ? pow(mu_x, s_ptr->dw_a3)
                : s_ptr->dw_a_visc ? 1
                                   : pow(mu_x, 0.75));
        } else {
          a = (s_ptr->dw_a ? s_ptr->dw_a : 1.6);
    ...
    // average z and Dw for transport numbers t1_0 and t2_0 at zero conc's, and
    // q of the solution...
    for (i = 0; i < (int)this->s_x.size(); i++) {
      if (s_x[i]->type != AQ && s_x[i]->type != HPLUS)
        continue;
      ...
      if ((Dw = s_x[i]->dw) == 0) {
        if (correct_Dw)
          Dw = default_Dw; // or charge based...Dw = l_z > 0 ? 1.6e-9 / l_z :
                           // 2e-9 / -l_z;
        else {
          s_x[i]->dw_corr = 0;
          continue;
    ...
    //     z1 = z_plus, z2 = z_min, t1_0 = (eq_dw_plus / m_plus) / (eq_dw_plus /
    //     m_plus + eq_dw_min / m_min)
    // a = (eq_plus - eq_min) / 2;
    // eq_min += a;
    // eq_plus -= a;
    ...
    // B1 = relaxtion, B2 = electrophoresis in ll = (ll0 - B2 * sqrt(mu) / f2(1
    ...
    B2 = a * AVOGADRO / viscos_0 * DH_B *
         1e17; // DH_B per Angstrom (*1e10), viscos in mPa.s (*1e3), B2 in cm2
               // (*1e4)
    // B1 = a / (2 * 8.8542e-12 * eps_c * 1.38066e-23 * tk_x) * q / (1 + sqrt_q)
    // * DH_B * 1e10 * z_plus * z_min;  // DH_B is per Angstrom (*1e10) B2 = a *
    // AVOGADRO / viscos * DH_B * 1e17;  // DH_B per Angstrom (*1e10), viscos in
    // mPa.s (*1e3), B2 in cm2 (*1e4)
...
/* ---------------------------------------------------------------------- */
{
  ...
  /* 2 options: original VP, assign the volumes of species with zero molar
     volume to their master species, but this doubles counts of complexes with
     -Vm defined. And, cation-OH and H-anion complexes are counted once. Also,
     must add H+ and OH-... */
  // class species *s_ptr;
  ...
  density_x = rho_0;
  if (M_T > 0.0) {
    density_x = rho_0 * (1e3 + M_T / mass_water_aq_x) /
                (rho_0 * V_solutes / mass_water_aq_x + 1e3);
  }
...
#ifdef NOT_USED
LDBLE Phreeqc::f_rho(LDBLE rho_old, void *cookie)
/* ---------------------------------------------------------------------- */
{
  LDBLE rho = 1.0;
  Phreeqc *pThis;
...
#ifdef original
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_solution_volume(void)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (int i = 0; i < (int)master.size(); i++) {
    if (master[i]->s->type != AQ)
      continue;
    class master *master_ptr = master[i];
    if (master_ptr->primary == TRUE &&
        strcmp(master_ptr->elt->name, "Alkalinity")) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_logk_n(const char *name)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_logk_p(const char *name)
/* ---------------------------------------------------------------------- */
{
  int i, j;
  char token[MAX_LENGTH];
  ...
  if (phase_ptr != NULL) {
     ...
     */
    reaction_ptr->logk[delta_v] =
        calc_delta_v(*reaction_ptr, true) - phase_ptr->logk[vm0];
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_logk_s(const char *name)
/* ---------------------------------------------------------------------- */
{
  int i;
  char token[MAX_LENGTH];
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::dh_a0(const char *name)
/* ---------------------------------------------------------------------- */
{
  char token[MAX_LENGTH];
  class species *s_ptr;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::dh_bdot(const char *name)
/* ---------------------------------------------------------------------- */
{
  char token[MAX_LENGTH];
  class species *s_ptr;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_deltah_p(const char *name)
/* ---------------------------------------------------------------------- */
{
  int i, j;
  char token[MAX_LENGTH];
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_deltah_s(const char *name)
/* ---------------------------------------------------------------------- */
{
  int i;
  char token[MAX_LENGTH];
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_surface_charge(const char *surface_name)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::diff_layer_total(const char *total_name,
...
/* ---------------------------------------------------------------------- */
{
  ...
  for (j = 0; j < count_unknowns; j++) {
    if (use.Get_surface_ptr()->Get_type() == cxxSurface::DDL ||
        use.Get_surface_ptr()->Get_type() == cxxSurface::CCM) {
  ...
  if (strcmp_nocase("psi", total_name) == 0) {
    if (use.Get_surface_ptr()->Get_type() == cxxSurface::DDL ||
        use.Get_surface_ptr()->Get_type() == cxxSurface::CCM) {
      return ((LDBLE)(x[j]->master[0]->s->la * 2 * R_KJ_DEG_MOL * tk_x *
                      LOG_10 / F_KJ_V_EQ));
    } else if (use.Get_surface_ptr()->Get_type() == cxxSurface::CD_MUSIC) {
      master_ptr = surface_get_psi_master(surface_name, SURF_PSI);
      if (master_ptr != NULL) {
        return ((LDBLE)(-master_ptr->s->la * R_KJ_DEG_MOL * tk_x * LOG_10 /
                        F_KJ_V_EQ));
      } else {
        return (0.0);
      }
    ...
    if ((use.Get_surface_ptr()->Get_type() == cxxSurface::DDL ||
      ...
      cxxSurfaceCharge *charge_ptr =
          use.Get_surface_ptr()->Find_charge(x[j]->surface_charge);
    ...
    if (use.Get_surface_ptr()->Get_type() == cxxSurface::DDL ||
      ...
      if ((charge_ptr->Get_specific_area() * charge_ptr->Get_grams()) > 0) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_t_sc(const char *name)
/* ---------------------------------------------------------------------- */
{
  char token[MAX_LENGTH];
  class species *s_ptr;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::calc_f_visc(const char *name)
/* ---------------------------------------------------------------------- */
{
  char token[MAX_LENGTH];
  class species *s_ptr;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::equi_phase(const char *phase_name)
/* ---------------------------------------------------------------------- */
{
  int j;
  ...
  if (j == count_unknowns) {
    ...
    for (; it != pp_assemblage_ptr->Get_pp_assemblage_comps().end(); it++) {
      if (strcmp_nocase(it->second.Get_name().c_str(), phase_name) == 0) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::equi_phase_delta(const char *phase_name)
/* ---------------------------------------------------------------------- */
{
  int j;
  ...
  if (j == count_unknowns) {
    ...
    // pp_assemblage_ptr->Find(x[j]->pp_assemblage_comp_name);
    cxxPPassemblageComp *comp_ptr =
        (cxxPPassemblageComp *)x[j]->pp_assemblage_comp_ptr;
    if (state != TRANSPORT && state != PHAST) {
      LDBLE delta_moles =
          x[j]->moles - comp_ptr->Get_moles() - comp_ptr->Get_delta();
      return (delta_moles);
    } else {
      LDBLE delta_moles = x[j]->moles - comp_ptr->Get_initial_moles();
      return (delta_moles);
    }
  }
  return (0);
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::equivalent_fraction(const char *name, LDBLE *eq,
                                   std::string &elt_name)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::find_gas_comp(const char *gas_comp_name)
/* ---------------------------------------------------------------------- */
{
  int i;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::find_gas_vm(void)
/* ---------------------------------------------------------------------- */
{
  if (use.Get_gas_phase_in() == FALSE || use.Get_gas_phase_ptr() == NULL)
    return (0);
  ...
  if (gas_phase_ptr->Get_type() == cxxGasPhase::GP_PRESSURE) {
    ...
    gas_phase_ptr->Set_volume(gas_phase_ptr->Get_total_moles() * R_LITER_ATM *
                              tk_x / gas_phase_ptr->Get_total_p());
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::find_misc1(const char *ss_name)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::find_misc2(const char *ss_name)
/* ---------------------------------------------------------------------- */
{
  if (use.Get_ss_assemblage_in() == FALSE ||
      use.Get_ss_assemblage_ptr() == NULL)
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::find_ss_comp(const char *ss_comp_name)
/* ---------------------------------------------------------------------- */
{
  if (use.Get_ss_assemblage_in() == FALSE ||
      use.Get_ss_assemblage_ptr() == NULL)
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::get_calculate_value(const char *name)
/* ---------------------------------------------------------------------- */
/*
 ...
 */
{
  ...
  if (calculate_value_ptr == NULL) {
    error_string = sformatf("CALC_VALUE Basic function, %s not found.", name);
    // error_msg(error_string, CONTINUE);
    // input_error++;
  ...
  if (name == NULL) {
    error_string =
        sformatf("Definition for calculated value not found, %s", name);
    input_error++;
    error_msg(error_string, CONTINUE);
    return (MISSING);
  }
  ...
  if (calculate_value_ptr->new_def == TRUE) {
    if (interp.basic_compile(calculate_value_ptr->commands.c_str(),
                             &calculate_value_ptr->linebase,
                             &calculate_value_ptr->varbase,
                             &calculate_value_ptr->loopbase) != 0) {
      error_string = sformatf("Fatal Basic error in CALCULATE_VALUES %s.",
  ...
  if (interp.basic_run(l_command, calculate_value_ptr->linebase,
                       calculate_value_ptr->varbase,
                       calculate_value_ptr->loopbase) != 0) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::kinetics_moles(const char *kinetics_name)
/* ---------------------------------------------------------------------- */
{

  if (use.Get_kinetics_in() == FALSE || use.Get_kinetics_ptr() == NULL)
    return (0);
  for (size_t i = 0; i < use.Get_kinetics_ptr()->Get_kinetics_comps().size();
       i++) {
    cxxKineticsComp *kinetics_comp_ptr =
        &(use.Get_kinetics_ptr()->Get_kinetics_comps()[i]);
    if (strcmp_nocase(kinetics_comp_ptr->Get_rate_name().c_str(),
                      kinetics_name) == 0) {
  ...
  error_string =
      sformatf("No data for rate %s in KINETICS keyword.", kinetics_name);
  // if (count_warnings >= 0) // appt debug cvode
  //	warning_msg(error_string);
  return (0);
}
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::kinetics_moles_delta(const char *kinetics_name)
/* ---------------------------------------------------------------------- */
{
  ...
  for (size_t i = 0; i < use.Get_kinetics_ptr()->Get_kinetics_comps().size();
    ...
    if (strcmp_nocase(kinetics_comp_ptr->Get_rate_name().c_str(),
      ...
      if (state != TRANSPORT && state != PHAST) {
        ...
        LDBLE delta_moles =
            kinetics_comp_ptr->Get_m() - kinetics_comp_ptr->Get_initial_moles();
        return delta_moles;
      }
    }
  }
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::log_activity(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE la;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::log_molality(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE lm;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::molality(const char *species_name)
/* ---------------------------------------------------------------------- */
{
  class species *s_ptr;
  LDBLE m;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::pr_pressure(const char *phase_name)
/* ---------------------------------------------------------------------- */
{
  int l;
  ...
  if (gas_phase_ptr != NULL) {
    for (size_t i = 0; i < gas_phase_ptr->Get_gas_comps().size(); i++) {
      ...
      int j;
      class phase *phase_ptr_gas =
          phase_bsearch(gas_comp_ptr->Get_phase_name().c_str(), &j, FALSE);
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::pr_phi(const char *phase_name)
/* ---------------------------------------------------------------------- */
{
  int l;
  class phase *phase_ptr = phase_bsearch(phase_name, &l, FALSE);
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::saturation_ratio(const char *phase_name)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
int Phreeqc::saturation_index(const char *phase_name, LDBLE *iap, LDBLE *si)
/* ---------------------------------------------------------------------- */
{
  class rxn_token *rxn_ptr;
  class phase *phase_ptr;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::sum_match_gases(const char *mytemplate, const char *name)
/* ---------------------------------------------------------------------- */
{
  int i;
  LDBLE tot;
  ...
  tot = 0;
  for (size_t j = 0; j < gas_phase_ptr->Get_gas_comps().size(); j++) {
    class phase *phase_ptr = phase_bsearch(
        gas_phase_ptr->Get_gas_comps()[j].Get_phase_name().c_str(), &i, FALSE);
    if (match_elts_in_species(phase_ptr->formula, mytemplate) == TRUE) {
      if (name == NULL) {
        tot += phase_ptr->moles_x;
      } else {
        for (next_elt = &phase_ptr->next_elt[0]; next_elt->elt != NULL;
             next_elt++) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::sum_match_species(const char *mytemplate, const char *name)
/* ---------------------------------------------------------------------- */
{
  int i;
  LDBLE tot;
  ...
  std::vector<std::string> &species_list =
      (sum_species_map.find(mytemplate))->second;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::sum_match_ss(const char *mytemplate, const char *name)
/* ---------------------------------------------------------------------- */
{
  LDBLE tot;
  const class elt_list *next_elt;
    ...
    if (strcmp_nocase(ss_ptr->Get_name().c_str(), mytemplate) == 0) {
      ...
      for (size_t i = 0; i < ss_ptr->Get_ss_comps().size(); i++) {
        ...
        if (name == NULL) {
          ...
          int l;
          class phase *phase_ptr =
              phase_bsearch(comp_ptr->Get_name().c_str(), &l, FALSE);
          for (next_elt = &phase_ptr->next_elt[0]; next_elt->elt != NULL;
               next_elt++) {
            if (strcmp(next_elt->elt->name, name) == 0) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::list_ss(std::string ss_name, cxxNameDouble &composition)
/* ---------------------------------------------------------------------- */
{
  LDBLE tot = 0;
  composition.clear();
...
/* ---------------------------------------------------------------------- */
int Phreeqc::match_elts_in_species(const char *name, const char *mytemplate)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::extract_bracket(const char **string, char *bracket_string)
/* ---------------------------------------------------------------------- */
{
  const char *cptr;
  char *ptr1;
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::surf_total(const char *total_name, const char *surface_name)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (j = 0; j < (int)this->s_x.size(); j++) {
    ...
    for (int i = 0; s_x[j]->next_elt[i].elt != NULL; i++) {
      if (s_x[j]->next_elt[i].elt->master->type != SURF)
        continue;
    ...
    if (s_x[j]->mole_balance == NULL) {
      for (rxn_ptr = &s_x[j]->rxn_s.token[0] + 1; rxn_ptr->s != NULL;
           rxn_ptr++) {
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::surf_total_no_redox(const char *total_name,
                                   const char *surface_name)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (j = 0; j < (int)this->s_x.size(); j++) {
    ...
    for (i = 0; s_x[j]->next_elt[i].elt != NULL; i++) {
      ...
      if (strcmp(name, surface_name_local) == 0) {
        ...
        // fprintf(stderr, "%15s\t%e\t%s\t%s\n", s_x[j]->name, s_x[j]->moles,
        // name, surface_name_local );
        break;
      }
    }
  }
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::total(const char *total_name)
/* ---------------------------------------------------------------------- */
{
  class master *master_ptr;
  LDBLE t;
  ...
  if (master_ptr == NULL) {
    ...
    /*
                    sprintf (error_string, "Cannot find definition for master
       species, %s.", total_name); warning_msg (error_string);
    */
  }
  /*
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::total_mole(const char *total_name)
/* ---------------------------------------------------------------------- */
{
  class master *master_ptr;
  LDBLE t;
   ...
   */
    ...
    if (master_ptr->s->secondary == NULL) {
      ...
      t = 0;
      for (size_t i = master_ptr->number + 1;
           (i < master.size() && master[i]->elt->primary == master_ptr); i++) {
        t += master[i]->total;
      }
    }
  }
...
/* ---------------------------------------------------------------------- */
int Phreeqc::get_edl_species(cxxSurfaceCharge &charge_ref)
/* ---------------------------------------------------------------------- */
{
  ...
  for (int j = 0; j < (int)this->s_x.size(); j++) {
    if (s_x[j]->type == H2O) {
      ...
      double moles_excess = mass_water_aq_x * molality *
                            charge_ref.Get_g_map()[s_x[j]->z].Get_g();
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::edl_species(const char *surf_name, LDBLE *count, char ***names,
                           LDBLE **moles, LDBLE *area, LDBLE *thickness)
/* ---------------------------------------------------------------------- */
{
  /*
   ...
   */
  if (sys.size() > 1) {
    qsort(&sys[0], sys.size(), sizeof(class system_species),
          system_species_compare);
  }
  /*
   * malloc space
   */
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::system_total(const char *total_name, LDBLE *count, char ***names,
...
/* ---------------------------------------------------------------------- */
{
  ...
  *count = (LDBLE)count_sys;
  if (strcmp_nocase(total_name, "elements") == 0) {
    ...
    ;
    for (i = 0; i < (int)count_sys; i++) {
      if (strcmp(sys[i].type, "dis") == 0 && strstr(sys[i].name, "(") == NULL &&
...
/* ---------------------------------------------------------------------- */
std::string Phreeqc::kinetics_formula(std::string kin_name,
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (use.Get_kinetics_ptr() == NULL)
    return (formula);
  std::vector<cxxKineticsComp> comps =
      use.Get_kinetics_ptr()->Get_kinetics_comps();
  count_elts = 0;
  paren_count = 0;
...
/* ---------------------------------------------------------------------- */
std::string Phreeqc::phase_formula(std::string phase_name,
                                   cxxNameDouble &stoichiometry)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Returns formula of mineral
   *   Also returns arrays of elements and stoichiometry in elts_arg and
   * coef_arg
   */
  stoichiometry.clear();
  std::string formula;
...
/* ---------------------------------------------------------------------- */
std::string Phreeqc::species_formula(std::string phase_name,
                                     cxxNameDouble &stoichiometry)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
std::string Phreeqc::phase_equation(
...
/* ---------------------------------------------------------------------- */
std::string Phreeqc::species_equation(
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_elements(void)
/* ---------------------------------------------------------------------- */
{
  int i;
  LDBLE t;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_si(void)
/* ---------------------------------------------------------------------- */
{
  int i;
  LDBLE si, iap;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_aq(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_ex(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_surf(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_gas(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_equi(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *  Equilibrium phases
  ...
  std::map<std::string, cxxPPassemblageComp> comps =
      use.Get_pp_assemblage_ptr()->Get_pp_assemblage_comps();
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_kin(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *  Equilibrium phases
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_ss(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_elt(const char *total_name)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
  ...
  if (use.Get_surface_ptr() != NULL && dl_type_x != cxxSurface::NO_DL) {
    ...
    for (k = 0; k < count_unknowns; k++) {
      if (x[k]->type != SURFACE_CB)
        continue;
      cxxSurfaceCharge *charge_ptr =
          use.Get_surface_ptr()->Find_charge(x[k]->surface_charge);
      i++;
      /*
       *   Loop through all surface components, calculate each H2O surface
       * (diffuse layer), H2O aq, and H2O bulk (diffuse layers plus aqueous).
       */
  ...
  if (use.Get_pp_assemblage_in() == TRUE &&
    ...
    for (i = 0; i < count_unknowns; i++) {
      ...
      // std::map<std::string, cxxPPassemblageComp>::iterator it;
      // it =
      // pp_assemblage_ptr->Get_pp_assemblage_comps().find(x[i]->pp_assemblage_comp_name);
      cxxPPassemblageComp *comp_ptr =
          (cxxPPassemblageComp *)x[i]->pp_assemblage_comp_ptr;
      ...
      paren_count = 0;
      int j;
      // class phase * phase_ptr = phase_bsearch(x[i]->pp_assemblage_comp_name,
      // &j, FALSE);
    ...
    for (size_t k = 0; k < ss_ptrs.size(); k++) {
      ...
      if (ss_ptr->Get_ss_in()) {
        for (size_t i = 0; i < ss_ptr->Get_ss_comps().size(); i++) {
          ...
          count_elts = 0;
          paren_count = 0;
          add_elt_list(phase_ptr->next_elt, comp_ptr->Get_moles());
          elt_list_combine();
          for (j = 0; j < count_elts; j++) {
            if (strcmp(elt_list[j].elt->name, total_name) == 0) {
              size_t count_sys = sys.size();
  ...
  if (use.Get_gas_phase_ptr() != NULL) {
    ...
    for (size_t i = 0; i < gas_phase_ptr->Get_gas_comps().size(); i++) {
      class phase *phase_ptr = phase_bsearch(
          gas_phase_ptr->Get_gas_comps()[i].Get_phase_name().c_str(), &k,
          FALSE);
      assert(phase_ptr);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_elt_secondary(const char *total_name)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
        ...
        for (l = 0; l < count_elts; l++) {
          if (strcmp(elt_list[l].elt->name, total_name) == 0) {
            ...
            if (s_x[j]->type > H2O)
              continue;
            molality = under(s_x[j]->lm);
            moles_excess = mass_water_aq_x * molality *
    ...
    for (size_t i = 0; i < ss_ptrs.size(); i++) {
      ...
      if (ss_ptr->Get_ss_in()) {
        for (size_t k = 0; k < ss_ptr->Get_ss_comps().size(); k++) {
          ...
          count_elts = 0;
          paren_count = 0;
          add_elt_list(phase_ptr->next_sys_total, comp_ptr->Get_moles());
          elt_list_combine();
          for (j = 0; j < count_elts; j++) {
            if (strcmp(elt_list[j].elt->name, total_name) == 0) {
              size_t count_sys = sys.size();
  ...
  if (use.Get_gas_phase_ptr() != NULL) {
    ...
    for (size_t j = 0; j < gas_phase_ptr->Get_gas_comps().size(); j++) {
      class phase *phase_ptr = phase_bsearch(
          gas_phase_ptr->Get_gas_comps()[j].Get_phase_name().c_str(), &i,
          FALSE);
      assert(phase_ptr);
      if (phase_ptr->in == TRUE) {
        count_elts = 0;
        paren_count = 0;
        add_elt_list(phase_ptr->next_sys_total, phase_ptr->moles_x);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::solution_number(void)
/* ---------------------------------------------------------------------- */
{
...
/* ---------------------------------------------------------------------- */
LDBLE Phreeqc::solution_sum_secondary(const char *total_name)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Provides total moles in system and lists of species/phases in sort order
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_species_compare(const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
...
int Phreeqc::system_species_compare_name(const void *ptr1, const void *ptr2)
/* ---------------------------------------------------------------------- */
{
  const class system_species *a, *b;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::system_total_solids(cxxExchange *exchange_ptr,
...
/* ---------------------------------------------------------------------- */
{
  ...
  if (gas_phase_ptr != NULL) {
    for (size_t j = 0; j < gas_phase_ptr->Get_gas_comps().size(); j++) {
      ...
      add_elt_list(phase_ptr->next_elt,
                   gas_phase_ptr->Get_gas_comps()[j].Get_moles());
    }
  }
  if (pp_assemblage_ptr != NULL) {
    ...
    for (; it != pp_assemblage_ptr->Get_pp_assemblage_comps().end(); it++) {
      ...
      add_elt_list(phase_ptr->next_elt, it->second.Get_moles());
    }
  }
  elt_list_combine();
  return (OK);
...
LDBLE Phreeqc::iso_value(const char *total_name) {
  int j;
  char token[MAX_LENGTH];
...
char *Phreeqc::iso_unit(const char *total_name) {
  int j;
...
int Phreeqc::basic_compile(const char *commands, void **lnbase, void **vbase,
                           void **lpbase) {
  return this->basic_interpreter->basic_compile(commands, lnbase, vbase,
                                                lpbase);
}

int Phreeqc::basic_run(char *commands, void *lnbase, void *vbase,
                       void *lpbase) {
  return this->basic_interpreter->basic_run(commands, lnbase, vbase, lpbase);
}

void Phreeqc::basic_free(void) {
...
double Phreeqc::basic_callback(double x1, double x2, const char *str) {
...
void Phreeqc::register_basic_callback(double (*fcn)(double x1, double x2,
                                                    const char *str,
                                                    void *cookie),
                                      void *cookie1) {
...
#ifdef IPHREEQC_NO_FORTRAN_MODULE
void Phreeqc::register_fortran_basic_callback(
    double (*fcn)(double *x1, double *x2, const char *str, size_t l)) {
  this->basic_fortran_callback_ptr = fcn;
}
#else

void Phreeqc::register_fortran_basic_callback(
    double (*fcn)(double *x1, double *x2, const char *str, int l)) {
  this->basic_fortran_callback_ptr = fcn;
}
#endif
