...
/* ---------------------------------------------------------------------- */
int Phreeqc::step(LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if ((state == TRANSPORT) && (transport_step != 0) && (cell > 0) &&
      (cell != count_cells + 1)) // ** needs potV correction
  {
    ...
    if (use.Get_ss_assemblage_in()) {
      ...
      for (size_t j = 0; j < ss_ptrs.size(); j++) {
        ...
        for (size_t k = 0; k < ss_ptr->Get_ss_comps().size(); k++) {
          ...
          int n;
          class phase *p_ptr =
              phase_bsearch(comp_ptr->Get_name().c_str(), &n, FALSE);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::xsolution_zero(void)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_solution(cxxSolution *solution_ptr, LDBLE extensive,
                          LDBLE intensive)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_exchange(cxxExchange *exchange_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_surface(cxxSurface *surface_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (size_t i = 0; i < surface_ptr->Get_surface_comps().size(); i++) {
    ...
    class element *elt_ptr =
        element_store(comp_ptr->Get_master_element().c_str());
    ...
    for (jit = comp_ptr->Get_totals().begin();
         jit != comp_ptr->Get_totals().end(); jit++) {
      ...
      if (master_j_ptr == NULL) {
        input_error++;
        error_string =
            sformatf("Element not defined in database, %s.", elt_j_ptr->name);
  ...
  for (size_t i = 0; i < surface_ptr->Get_surface_charges().size(); i++) {
     ...
     */
    if (surface_ptr->Get_dl_type() != cxxSurface::NO_DL &&
        !surface_ptr->Get_new_def()) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_mix(cxxMix *mix_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  LDBLE sum_fractions_water = 0, sum_positive_water = 0, intensive_water = 0,
        extensive_water = 0;
  ...
  for (it = mix_ptr->Get_mixComps().begin();
       it != mix_ptr->Get_mixComps().end(); it++) {
    solution_ptr = Utilities::Rxn_find(Rxn_solution_map, it->first);
    if (solution_ptr == NULL) {
      error_string = sformatf("Mix solution not found, %d.", it->first);
...
#endif
/* ---------------------------------------------------------------------- */
int Phreeqc::add_pp_assemblage(cxxPPassemblage *pp_assemblage_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  paren_count = 0;
  /*
   *   Check that all elements are in solution for phases with greater than zero
   * mass
   */
  std::map<std::string, cxxPPassemblageComp>::iterator it;
  it = pp_assemblage_ptr->Get_pp_assemblage_comps().begin();
  for (; it != pp_assemblage_ptr->Get_pp_assemblage_comps().end(); it++) {
    ...
    if (comp_ptr->Get_add_formula().size() > 0) {
      Utilities::strcpy_safe(token, MAX_LENGTH,
                             comp_ptr->Get_add_formula().c_str());
      cptr = &(token[0]);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::check_pp_assemblage(cxxPPassemblage *pp_assemblage_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_reaction(cxxReaction *reaction_ptr, int step_number,
                          LDBLE step_fraction)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (incremental_reactions == FALSE) {
    if (!reaction_ptr->Get_equalIncrements() &&
        reaction_ptr->Get_steps().size() > 0) {
      if (step_number > (int)reaction_ptr->Get_steps().size()) {
        step_x =
            reaction_ptr->Get_steps()[reaction_ptr->Get_steps().size() - 1];
      } else {
        step_x = reaction_ptr->Get_steps()[(size_t)step_number - 1];
      }
    } else if (reaction_ptr->Get_equalIncrements() &&
               reaction_ptr->Get_steps().size() > 0) {
      if (step_number > (int)reaction_ptr->Get_reaction_steps()) {
        step_x = reaction_ptr->Get_steps()[0];
      } else {
        step_x = reaction_ptr->Get_steps()[0] * ((LDBLE)step_number) /
    ...
    if (!reaction_ptr->Get_equalIncrements() &&
      ...
      if (step_number > (int)reaction_ptr->Get_reaction_steps()) {
        step_x =
            reaction_ptr
                ->Get_steps()[(size_t)reaction_ptr->Get_reaction_steps() - 1];
      } else {
        step_x = reaction_ptr->Get_steps()[(size_t)step_number - 1];
      }
    } else if (reaction_ptr->Get_equalIncrements() &&
...
/* ---------------------------------------------------------------------- */
int Phreeqc::reaction_calc(cxxReaction *reaction_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (int i = 0; i < count_elts; i++) {
    if (elt_list[i].elt->master == NULL) {
      error_string = sformatf("Element or phase not defined in database, %s.",
                              elt_list[i].elt->name);
      error_msg(error_string, CONTINUE);
      input_error++;
      return_value = ERROR;
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_gas_phase(cxxGasPhase *gas_phase_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (gas_phase_ptr->Get_type() == cxxGasPhase::GP_PRESSURE &&
      fabs(gas_phase_ptr->Get_total_p() - patm_x) > 0.01) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_ss_assemblage(cxxSSassemblage *ss_assemblage_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::add_kinetics(cxxKinetics *kinetics_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  for (; it != kinetics_ptr->Get_totals().end(); it++) {
    ...
    if (elt_ptr == NULL || (master_ptr = elt_ptr->primary) == NULL) {
      input_error++;
      error_string =
          sformatf("Element %s in kinetic reaction not found in database.",
                   it->first.c_str());
      error_msg(error_string, STOP);
    } else {
      if (master_ptr->s == s_hplus) {
...
/* ---------------------------------------------------------------------- */
int Phreeqc::gas_phase_check(cxxGasPhase *gas_phase_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
  ...
  if (use.Get_pressure_ptr() != NULL &&
      gas_phase_ptr->Get_type() == cxxGasPhase::GP_PRESSURE) {
      ...
      for (int j = 0; j < count_elts; j++) {
        ...
        if (master_ptr->s == s_hplus) {
          ...
          if (state != ADVECTION && state != TRANSPORT && state != PHAST) {
            error_string =
                sformatf("Element %s is contained in gas %s (which has 0.0 "
                         "mass),\nbut is not in solution or other phases.",
                         elt_list[j].elt->name, phase_ptr->name);
...
/* ---------------------------------------------------------------------- */
int Phreeqc::pp_assemblage_check(cxxPPassemblage *pp_assemblage_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Check for missing elements
    ...
    if (comp_ptr->Get_moles() <= 0.0) {
      ...
      for (int i = 0; i < count_elts; i++) {
        ...
        if (master_ptr->s == s_hplus) {
          ...
          if (state != ADVECTION && state != TRANSPORT && state != PHAST) {
            error_string =
                sformatf("Element %s is contained in %s (which has 0.0 mass),"
                         "\t\nbut is not in solution or other phases.",
                         elt_list[i].elt->name, phase_ptr->name);
            warning_msg(error_string);
          }
          /*
           *   Make la's of all master species for the element small, so SI will
           * be small and no mass transfer will be calculated
           */
...
/* ---------------------------------------------------------------------- */
int Phreeqc::ss_assemblage_check(cxxSSassemblage *ss_assemblage_ptr)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Check for missing elements
          ...
          if (master_ptr->s == s_hplus) {
            ...
            if (state != ADVECTION && state != TRANSPORT && state != PHAST) {
              error_string = sformatf(
                  "Element %s is contained in solid solution %s (which has 0.0 "
                  "mass),\nbut is not in solution or other phases.",
                  elt_list[l].elt->name, phase_ptr->name);
              warning_msg(error_string);
            }
          }
          /*
...
/* ---------------------------------------------------------------------- */
int Phreeqc::solution_check(void)
/* ---------------------------------------------------------------------- */
{
  /*
   *   Check for missing elements
  ...
  for (i = 0; i < (int)master.size(); i++) {
    ...
    /*
       ...
       balance occurs as moles are added to produce zero moles.\n\tUsually
       caused by KINETICS, REACTION, or diffuse layer calculation.\n\tMay be due
       to large time steps in early part of KINETICS simulation or negative
       concentrations in the diffuse layer.", master_ptr->elt->name, (LDBLE)
       master_ptr->total);
     */
    if (state != TRANSPORT) {
      error_string =
          sformatf("Negative moles in solution %d for %s, %e. Recovering...",
