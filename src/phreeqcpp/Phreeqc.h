...
public:
  ...
  // basicsubs.cpp -------------------------------
  int basic_compile(const char *commands, void **lnbase, void **vbase,
                    void **lpbase);
  ...
  double basic_callback(double x1, double x2, const char *str);
#endif
  void register_basic_callback(double (*fcn)(double x1, double x2,
                                             const char *str, void *cookie),
                               void *cookie1);
#ifdef IPHREEQC_NO_FORTRAN_MODULE
  void register_fortran_basic_callback(double (*fcn)(double *x1, double *x2,
                                                     const char *str,
                                                     size_t l));
#else
  void register_fortran_basic_callback(double (*fcn)(double *x1, double *x2,
  ...
  LDBLE sa_declercq(double type, double sa, double d, double m, double m0,
                    double gfw);
  LDBLE calc_SC(void);
  /* VP: Density Start */
  ...
  LDBLE edl_species(const char *surf_name, LDBLE *count, char ***names,
  ...
  std::string kinetics_formula(std::string kinetics_name,
                               cxxNameDouble &stoichiometry);
  std::string phase_formula(std::string phase_name,
                            cxxNameDouble &stoichiometry);
  std::string species_formula(std::string phase_name,
                              cxxNameDouble &stoichiometry);
  std::string
  phase_equation(std::string phase_name,
                 std::vector<std::pair<std::string, double>> &stoichiometry);
  std::string
  species_equation(std::string species_name,
                   std::vector<std::pair<std::string, double>> &stoichiometry);
  ...
  // cl1.cpp -------------------------------
  int cl1(int k, int l, int m, int n, int nklmd, int n2d, LDBLE *q, int *kode,
          LDBLE toler, int *iter, LDBLE *x, LDBLE *res, LDBLE *error, LDBLE *cu,
          int *iu, int *s, int check);
  void cl1_space(int check, int n2d, int klm, int nklmd);

  // cl1mp.cpp -------------------------------
  int cl1mp(int k, int l, int m, int n, int nklmd, int n2d, LDBLE *q_arg,
            int *kode, LDBLE toler, int *iter, LDBLE *x_arg, LDBLE *res_arg,
            LDBLE *error, LDBLE *cu_arg, int *iu, int *s, int check,
            LDBLE censor_arg);
...
public:
  ...
  int check_line_impl(const char *string, int allow_empty, int allow_eof,
                      int allow_keyword, int print);
  int get_line(void);
  ...
  LDBLE calc_psi_avg(cxxSurfaceCharge *charge_ptr, LDBLE surf_chrg_eq,
                     LDBLE nDbl, LDBLE f_free, std::vector<LDBLE> &zcorr);
  ...
  int set_ph_c(class inverse *inv_ptr, int i, cxxSolution *soln_ptr_orig,
               int n_user_new, LDBLE d_alk, LDBLE ph_factor, LDBLE alk_factor);
  int shrink(class inverse *inv_ptr, LDBLE *array_in, LDBLE *array_out, int *k,
             int *l, int *m, int *n, unsigned long cur_bits, LDBLE *delta_l,
             int *col_back_l, int *row_back_l);
  ...
  int calculate_isotope_moles(class element *elt_ptr, cxxSolution *solution_ptr,
                              LDBLE total_moles);
...
public:
  ...
  int calc_kinetic_reaction(cxxKinetics *kinetics_ptr, LDBLE time_step);
  ...
  // mainsubs.cpp  -------------------------------
  std::ifstream *open_input_stream(std::string query, std::string &default_name,
                                   std::ios_base::openmode mode, bool batch);
  std::ofstream *open_output_stream(std::string query,
  ...
  LDBLE ss_root(LDBLE a0, LDBLE a1, LDBLE kc, LDBLE kb, LDBLE xcaq, LDBLE xbaq);
  LDBLE ss_halve(LDBLE a0, LDBLE a1, LDBLE x0, LDBLE x1, LDBLE kc, LDBLE kb,
                 LDBLE xcaq, LDBLE xbaq);
  LDBLE ss_f(LDBLE xb, LDBLE a0, LDBLE a1, LDBLE kc, LDBLE kb, LDBLE xcaq,
             LDBLE xbaq);
  ...
  int parse_eq(char *eqn, std::vector<class elt_list> &new_elt_list,
               int association);
...
public:
  ...
  LDBLE G(LDBLE Y);
  LDBLE GP(LDBLE Y);
  int ETHETAS(LDBLE ZJ, LDBLE ZK, LDBLE I, LDBLE *etheta, LDBLE *ethetap);
  ...
  LDBLE calc_PR(std::vector<class phase *> phase_ptrs, LDBLE P, LDBLE TK,
                LDBLE V_m);
  LDBLE calc_PR();
  ...
  std::vector<class master *> get_list_master_ptrs(const char *cptr,
  ...
  int *read_list_ints_range(const char **ptr, int *count_ints, int positive,
                            int *int_list);
  int read_list_ints_range(const char **cptr, bool positive,
  ...
  int read_copy(void);
  int read_debug(void);
  int read_delta_h_only(const char *cptr, LDBLE *delta_h, DELTA_H_UNIT *units);
  int read_aq_species_vm_parms(const char *cptr, LDBLE *delta_v);
  int read_vm_only(const char *cptr, LDBLE *delta_v, DELTA_V_UNIT *units);
  int read_phase_vm(const char *cptr, LDBLE *delta_v, DELTA_V_UNIT *units);
  ...
  int spread_row_to_solution(class spread_row *heading, class spread_row *units,
  ...
  int add_solution(cxxSolution *solution_ptr, LDBLE extensive, LDBLE intensive);
...
public:
  ...
  cxxSurface sum_surface_comp(cxxSurface *source1, LDBLE f1,
                              cxxSurface *source2, std::string charge_name,
                              LDBLE f2, LDBLE new_Dw);
  int reformat_surf(const char *comp_name, LDBLE fraction,
                    const char *new_comp_name, LDBLE new_Dw, int cell);
  ...
  void calc_b_ij(int icell, int jcell, int k, LDBLE b_i, LDBLE b_j, LDBLE g_i,
                 LDBLE g_j, LDBLE free_i, LDBLE free_j, int stagnant);
  ...
  LDBLE moles_from_donnan_layer(cxxSurface *sptr, const char *name,
                                LDBLE moles_needed);
  LDBLE add_MCD_moles(LDBLE moles, LDBLE min_mol, int i, cxxSolution *sptr,
                      const char *name);
  ...
  int mix_stag(int i, LDBLE stagkin_time, int punch, LDBLE step_fraction_kin);
...
public:
  ...
  size_t list_SolidSolutions(std::list<std::string> &list_comps,
                             std::list<std::string> &list_names);
  size_t list_Surfaces(std::list<std::string> &surftype,
                       std::list<std::string> &surf);
  ...
  std::map<int, cxxSolution> &Get_Rxn_solution_map() {
    return this->Rxn_solution_map;
  }
  std::map<int, cxxExchange> &Get_Rxn_exchange_map() {
    return this->Rxn_exchange_map;
  }
  std::map<int, cxxGasPhase> &Get_Rxn_gas_phase_map() {
    return this->Rxn_gas_phase_map;
  }
  std::map<int, cxxKinetics> &Get_Rxn_kinetics_map() {
    return this->Rxn_kinetics_map;
  }
  std::map<int, cxxPPassemblage> &Get_Rxn_pp_assemblage_map() {
    return this->Rxn_pp_assemblage_map;
  }
  std::map<int, cxxSSassemblage> &Get_Rxn_ss_assemblage_map() {
    return this->Rxn_ss_assemblage_map;
  }
  std::map<int, cxxSurface> &Get_Rxn_surface_map() {
    return this->Rxn_surface_map;
  }
  std::map<int, cxxMix> &Get_Rxn_mix_map() { return this->Rxn_mix_map; }
  std::map<int, cxxReaction> &Get_Rxn_reaction_map() {
    return this->Rxn_reaction_map;
  }
  std::map<int, cxxTemperature> &Get_Rxn_temperature_map() {
    return this->Rxn_temperature_map;
  }
  std::map<int, cxxPressure> &Get_Rxn_pressure_map() {
    return this->Rxn_pressure_map;
  }
...
protected:
   ...
   *----------------------------------------------------------------------
   */
  std::vector<class list0>
      sum_jacob0; /* array of pointers to targets and coefficients for array */

  std::vector<class list1> sum_mb1; /* array of pointers to sources and targets
                                       for mass balance summations with coef
                                       = 1.0 */
  std::vector<class list1> sum_jacob1; /* array of pointers to sources and
                                          targets for array equations with coef
                                          = 1.0 */
  std::vector<class list2> sum_mb2;    /* array of coefficients and pointers to
                                          sources and    targets for mass balance
                                          summations with coef != 1.0 */
  ...
  int interlayer_Dflag; /* multicomponent diffusion and diffusion through
                           interlayer porosity */
  int implicit;         /* implicit calculation of diffusion */
  LDBLE max_mixf;   /* the maximum value of the implicit mixfactor = De * Dt /
                       (Dx^2) */
  LDBLE min_dif_LM; /* the minimal log10(molality) for including a species in
                       multicomponent diffusion */
  LDBLE default_Dw; /* default species diffusion coefficient in water at 25oC,
                       m2/s */
  int correct_Dw;   /* if true, Dw is adapted in calc_SC */
  LDBLE multi_Dpor; /* uniform porosity of free porewater in solid medium */
  LDBLE interlayer_Dpor;     /* uniform porosity of interlayer space of
                                montmorillonite in solid medium */
  LDBLE multi_Dpor_lim;      /* limiting free porewater porosity where transport
                                stops */
  LDBLE interlayer_Dpor_lim; /* limiting interlayer porosity where transport
                                stops */
   ...
   *   Tidy data
   *---------------------------------------------------------------------- */
  int new_model, new_exchange, new_pp_assemblage, new_surface, new_reaction,
      new_temperature, new_mix, new_solution, new_gas_phase, new_inverse,
      new_punch, new_ss_assemblage, new_kinetics, new_copy, new_pitzer;
   ...
   *   Reaction work space
   *---------------------------------------------------------------------- */
  class reaction_temp trxn; /* structure array of working space while reading
                               equations species names are in "temp_strings" */
...
public:
  ChartHandler &Get_chart_handler(void) { return chart_handler; }
  const ChartHandler &Get_chart_handler(void) const { return chart_handler; }
...
protected:
  ...
  int print_viscosity;
  LDBLE viscos, viscos_0, viscos_0_25; // viscosity of the solution, of pure
                                       // water, of pure water at 25 C
  LDBLE density_x;
  ...
  LDBLE QBrn; // Born function d(ln(eps_r))/dP / eps_r * 41.84004, for supcrt
              // calc'n of molal volume
  LDBLE ZBrn; // Born function (-1/eps_r + 1) * 41.84004, for supcrt calc'n of
              // molal volume
  LDBLE dgdP; // dg / dP, pressure derivative of g-function, for supcrt calc'n
              // of molal volume
...
#ifdef IPHREEQC_NO_FORTRAN_MODULE
  double (*basic_fortran_callback_ptr)(double *x1, double *x2, const char *str,
                                       size_t l);
#else
  double (*basic_fortran_callback_ptr)(double *x1, double *x2, const char *str,
                                       int l);
  ...
  size_t col_phases, col_redox, col_epsilon, col_ph, col_water, col_isotopes,
      col_phase_isotopes;
  size_t row_mb, row_fract, row_charge, row_carbon, row_isotopes, row_epsilon,
      row_isotope_epsilon, row_water;
  std::vector<double> inv_zero, array1, inv_res, inv_delta1, delta2, delta3,
      inv_cu, delta_save;
...
protected:
  ...
  std::vector<int> s_list, cation_list, neutral_list, anion_list, ion_list,
      param_list;
  ...
  LDBLE current_x, current_A,
      fix_current; // current: coulomb / s, Ampere, fixed current (Ampere)
...
#else
#define PHR_ISFINITE(x)                                                        \
  std::isfinite(x) /* changed when <math.h> was changed to <cmath> */
#endif
#elif defined(HAVE_FINITE)
#define PHR_ISFINITE(x) finite(x)
#elif defined(HAVE_ISNAN)
#define PHR_ISFINITE(x)                                                        \
  (((x) == 0.0) || ((!std::isnan(x)) && ((x) != (2.0 * (x)))))
#else
...
template <typename T>
void Rxn_dump_raw_range(const T &b, std::ostream &s_oss, int start, int end,
                        unsigned int indent) {
  typename T::const_iterator it;
...
template <typename T> T *Rxn_find(std::map<int, T> &b, int i) {
  if (b.find(i) != b.end()) {
...
template <typename T> int Rxn_next_user_number(std::map<int, T> &b) {
  int ret = 0;
  if (b.size() != 0) {
...
template <typename T> T *Rxn_copy(std::map<int, T> &b, int i, int j) {
...
template <typename T>
int Rxn_read_raw(std::map<int, T> &m, std::set<int> &s,
                 Phreeqc *phreeqc_cookie) {
  typename std::map<int, T>::iterator it;
...
template <typename T>
int Rxn_read_modify(std::map<int, T> &m, std::set<int> &s,
                    Phreeqc *phreeqc_cookie) {
  typename std::map<int, T>::iterator it;
  ...
  if (!entity_ptr) {
    ...
    errstr << "Could not find " << key_name << " " << nk.Get_n_user()
           << ", ignoring modify data.\n";
...
template <typename T> int SB_read_modify(std::map<int, T> &m, CParser &parser) {
  typename std::map<int, T>::iterator it;
...
template <typename T>
void Rxn_mix(std::map<int, cxxMix> &mix_map, std::map<int, T> &entity_map,
             Phreeqc *phreeqc_cookie) {
  std::map<int, cxxMix>::iterator mix_it;
  for (mix_it = mix_map.begin(); mix_it != mix_map.end(); mix_it++) {
    T entity(entity_map, mix_it->second, mix_it->second.Get_n_user(),
             phreeqc_cookie->Get_phrq_io());
    entity_map[mix_it->second.Get_n_user()] = entity;
    Utilities::Rxn_copies(entity_map, mix_it->second.Get_n_user(),
                          mix_it->second.Get_n_user_end());
  }
  mix_map.clear();
}
...
namespace std {
__inline bool isnan(double num) { return _isnan(num) != 0; }
} // namespace std
#endif
#include <stdarg.h>
